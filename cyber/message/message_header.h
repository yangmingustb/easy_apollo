...
public:
    ...
    {
        memcpy(magic_num_, "BDACBDAC", sizeof(magic_num_));
    }

    uint64_t seq() const { return ConvertArrayTo64(seq_); }
    void set_seq(uint64_t seq)
    {
        Convert64ToArray(seq, const_cast<char*>(seq_));
    }
    ...
    {
        memset(timestamp_ns_, 0, sizeof(timestamp_ns_));
    }
    ...
    {
        memset(content_size_, 0, sizeof(content_size_));
    }
...
private:
    void Convert32ToArray(uint32_t input, char* output)
    {
        ...
        memcpy(static_cast<void*>(output), static_cast<const void*>(&n),
               sizeof(n));
    }
    ...
    {
        uint32_t n = 0;
        memcpy(static_cast<void*>(&n), static_cast<const void*>(input),
               sizeof(n));
        return ntohl(n);
    }
