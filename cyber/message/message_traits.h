...
template <typename T> class HasSerializer
{
public:
...
template <typename T> constexpr bool HasSerializer<T>::value;

template <typename T,
          typename std::enable_if<
                  HasType<T>::value && std::is_member_function_pointer<
...
template <typename T> int FullByteSize(const T& message)
{
        ...
        const void* data, int size, T* message)
{
    ...
    return message->ParseFromArray(
            static_cast<const void*>(static_cast<const char*>(data) +
                                     header_size),
            header->content_size());
}
...
template <typename T, typename Descriptor,
          typename std::enable_if<!HasFullName<Descriptor>::value, bool>::type =
                  0>
std::string GetFullName()
{
    return typeid(T).name();
