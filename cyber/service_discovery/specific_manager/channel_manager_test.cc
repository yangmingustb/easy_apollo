...
protected:
    ChannelManagerTest() : channel_num_(10)
    {
        ...
        for (int i = 0; i < channel_num_; ++i)
        {
            ...
            uint64_t channel_id =
                    common::GlobalData::Instance()->RegisterChannel(
...
TEST_F(ChannelManagerTest, get_proto_desc)
{
    ...
    uint64_t channel_id =
            common::GlobalData::Instance()->RegisterChannel("wasd");
    role_attr.set_channel_id(channel_id);
...
TEST_F(ChannelManagerTest, get_writers_attr)
{
    ...
    for (int i = 0; i < channel_num_; ++i)
    {
        ...
        writers.clear();
        channel_manager_.GetWritersOfNode("node_" + std::to_string(i),
                                          &writers);
        EXPECT_EQ(writers.size(), 1);
        writers.clear();
    }
...
TEST_F(ChannelManagerTest, get_readers_attr)
{
    ...
    for (int i = 0; i < channel_num_; ++i)
    {
        ...
        readers.clear();
        channel_manager_.GetReadersOfNode("node_" + std::to_string(i),
                                          &readers);
        EXPECT_EQ(readers.size(), 1);
        readers.clear();
    }
...
TEST_F(ChannelManagerTest, get_upstream_downstream)
{
    ...
    for (int i = 0; i < channel_num_; ++i)
    {
        ...
        nodes.clear();
        channel_manager_.GetDownstreamOfNode("node_" + std::to_string(i),
                                             &nodes);
        EXPECT_EQ(nodes.size(), 1);
        nodes.clear();
    }
    ...
    channel_id = common::GlobalData::Instance()->RegisterChannel(
            role_attr.channel_name());
    role_attr.set_channel_id(channel_id);
    role_attr.set_id(transport::Identity().HashValue());
    channel_manager_.Join(role_attr, RoleType::ROLE_WRITER);
...
TEST_F(ChannelManagerTest, is_message_type_matching)
{
    ...
    const std::string change_msg_type =
            message::MessageType<proto::ChangeMsg>();
    ...
    EXPECT_TRUE(channel_manager_.IsMessageTypeMatching(chatter_msg_type,
                                                       py_msg_type));
    EXPECT_TRUE(channel_manager_.IsMessageTypeMatching(raw_msg_type_1,
                                                       py_msg_type));
}
