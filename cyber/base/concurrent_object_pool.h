...
template <typename T> CCObjectPool<T>::~CCObjectPool()
{
    std::free(node_arena_);
}

template <typename T> bool CCObjectPool<T>::FindFreeHead(Head *head)
{
    Head new_head;
...
template <typename T> std::shared_ptr<T> CCObjectPool<T>::GetObject()
{
    Head free_head;
    ...
    return std::shared_ptr<T>(
            reinterpret_cast<T *>(free_head.node),
...
template <typename T> void CCObjectPool<T>::ReleaseObject(T *object)
{
    Head new_head;
