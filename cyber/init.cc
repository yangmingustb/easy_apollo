...
bool Init(const char* binary_name)
{
    ...
    if (global_data->IsMockTimeMode())
    {
        ...
        auto cb = [](const std::shared_ptr<const apollo::cyber::proto::Clock>&
                             msg) {
            if (msg->has_clock())
            {
        ...
        clock_node->CreateReader<apollo::cyber::proto::Clock>(kClockChannel,
                                                              cb);
    }

    AINFO << "cyber init finish";
    return true;
}
