...
bool Recorder::Start()
{
    for (const auto& channel_name : white_channels_)
    {
        ...
        {
            AERROR << "find channel in both of white list and black list, "
                      "channel: "
                   << channel_name;
            return false;
        }
...
void Recorder::FindNewChannel(const RoleAttributes& role_attr)
{
        ...
        channel_reader_map_.end())
    {
        ...
        {
            AERROR << "write channel fail, channel:"
...
bool Recorder::InitReadersImpl()
{
    ...
    change_conn_ = channel_manager->AddChangeListener(std::bind(
            &Recorder::TopologyCallback, this, std::placeholders::_1));
                              ...
                              const std::string& message_type)
{
    try
    {
        ...
        auto callback =
                [weak_this,
                 channel_name](const std::shared_ptr<RawMessage>& raw_message) {
