        ...
        switch (option)
        {
            ...
            case 'p':
                std::cout << "\t-p, --preload <seconds>\t\t\t" << command
                          << " after trying to preload n second(s)"
                          << std::endl;
                break;
            case 'i':
...
int main(int argc, char** argv)
{
    ...
    do
    {
        int opt = getopt_long(argc, argv, short_opts.c_str(), long_opts,
                              &long_index);
        if (opt == -1)
        {
            break;
        ...
        switch (opt)
        {
            ...
            case 'r':
                ...
                {
                    std::cout << "Invalid argument: -r/--rate "
                ...
                break;
            case 'b':
                opt_begin = StringToUnixSeconds(std::string(optarg)) * 1000 *
                            1000 * 1000ULL;
                break;
            case 'e':
                opt_end = StringToUnixSeconds(std::string(optarg)) * 1000 *
                          1000 * 1000ULL;
                break;
            case 's':
                try
                ...
                {
                    std::cout << "Invalid argument: -s/--start "
                              << std::string(optarg) << std::endl;
                    return -1;
                }
                catch (const std::out_of_range& e)
            ...
            case 'd':
                ...
                {
                    std::cout << "Invalid argument: -d/--delay "
                              << std::string(optarg) << std::endl;
                    return -1;
                }
                catch (const std::out_of_range& e)
            ...
            case 'p':
                ...
                {
                    std::cout << "Invalid argument: -p/--preload "
                              << std::string(optarg) << std::endl;
                    return -1;
                }
                catch (const std::out_of_range& e)
            ...
            case 'i':
                try
                {
                    ...
                    if (interval_s < 0)
                    {
                        std::cout << "Argument is less than zero: "
                                     "-i/--segment-interval "
                                  << std::string(optarg) << std::endl;
                        return -1;
                    }
                ...
                {
                    std::cout << "Argument is out of range: "
                                 "-i/--segment-interval "
                              << std::string(optarg) << std::endl;
                    return -1;
                }
    ...
    else if (command == "recover")
    {
        ...
        {
            std::cout << "TOO many input/output file option (-f/-o)."
                      << std::endl;
            return -1;
        }
    ...
    if (command == "play")
    {
        ...
        play_param.files_to_play.insert(opt_file_vec.begin(),
                                        opt_file_vec.end());
    ...
    else if (command == "record")
    {
        ...
        {
            std::cout << "MUST specify channels option (-c) or all channels "
                         "option (-a)."
                      << std::endl;
            return -1;
        }
        ...
        auto recorder = std::make_shared<Recorder>(
                opt_output_vec[0], opt_all, opt_white_channels,
