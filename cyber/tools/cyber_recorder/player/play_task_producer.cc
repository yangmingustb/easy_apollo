...
bool PlayTaskProducer::Init()
{
    ...
    play_state_writer_ = node_->CreateWriter<apollo::cyber::proto::PlayInfo>(
            "channel/play_info");
...
bool PlayTaskProducer::UpdatePlayParam()
{
    ...
    total_progress_time_s_ = static_cast<double>(play_param_.end_time_ns -
                                                 play_param_.begin_time_ns) /
                                     1e9 +
                             static_cast<double>(play_param_.start_time_s);
...
int PlayTaskProducer::publish_play_info(double processed_time)
{
    double current_time = apollo::cyber::Time::Now().ToSecond();

    // 0.2秒发送一次
    if (current_time - history_time_ < 0.2)
    {
        return 0;
    }
    if (total_progress_time_s_ < 0.1)
    {
        return 0;
    }

    double ratio = (processed_time / total_progress_time_s_) * 100;

    play_info_.set_ratio(static_cast<int64_t>(ratio));
    play_state_writer_->Write(play_info_);

    history_time_ = current_time;
