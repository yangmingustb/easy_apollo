        ...
        const std::string& channel_name)
{
    ...
    auto callback =
            [this](const std::shared_ptr<apollo::cyber::message::RawMessage>&
                           raw_msg) { UpdateRawMessage(raw_msg); };
                                              ...
                                              int* line_no)
{
    if (has_message_come())
    {
        ...
        {
            auto rawFactory =
                    apollo::cyber::message::ProtobufFactory::Instance();
        ...
        else
        {
            ...
            if (channel_msg->message.size())
            {
                ...
                {
                    out_str << " ("
                            << static_cast<float>(channel_msg->message.size()) /
                                       kGB
                            << " GB)";
                }
                ...
                if (raw_msg_class_->ParseFromString(channel_msg->message))
                {
                    ...
                    GeneralMessageBase::PrintMessage(
                            this, *raw_msg_class_, &jump_lines, s, line_no, 0);
                }
                else
                {
