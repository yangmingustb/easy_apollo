                    ...
                    item_fd->type())
        {
            ...
            for (int i = 0; i < size; ++i)
            {
                ...
                const std::string key(
                        item.GetReflection()->GetString(item, item_fd));
...
int GeneralMessage::Render(const Screen* s, int key)
{
    ...
    int line_no = 0;
    {
        ...
        if (message_ptr_ && reflection_ptr_)
        {
            ...
            const std::vector<int> indices(SortProtobufMapByKeys(
                    *message_ptr_, field_, *reflection_ptr_, size));
            if (is_folded_)
            {
                GeneralMessageBase::PrintField(this, *message_ptr_, &jump_lines,
                                               s, &line_no, 0, reflection_ptr_,
                                               field_, indices[item_index_]);
            }
            else
            {
                for (const int index : indices)
                {
                    GeneralMessageBase::PrintField(
                            this, *message_ptr_, &jump_lines, s, &line_no, 0,
