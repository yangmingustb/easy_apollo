...
void RecordViewer::Init()
{
    ...
    for (auto& reader : readers_)
    {
        ...
        std::set_intersection(
                all_channel.begin(), all_channel.end(), channels_.begin(),
                channels_.end(),
    ...
    while (curr_begin_time_ <= end_time_ && msg_buffer_.size() < kBufferMinSize)
    {
        ...
        for (size_t i = 0; i < readers_.size(); ++i)
        {
            ...
            while (true)
            {
                ...
                msg_buffer_.emplace(
                        std::make_pair(record_msg->time, record_msg));
            }
        }

        // because ReadMessage of RecordReader is closed interval, so we add 1
        // here
