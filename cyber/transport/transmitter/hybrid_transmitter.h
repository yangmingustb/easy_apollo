...
template <typename M> class HybridTransmitter : public Transmitter<M>
{
public:
    ...
    using ReceiverMap = std::unordered_map<OptionalMode, std::set<uint64_t>,
                                           std::hash<int>>;
...
template <typename M> HybridTransmitter<M>::~HybridTransmitter()
{
    ClearReceivers();
    ClearTransmitters();
}

template <typename M> void HybridTransmitter<M>::Enable()
{
...
template <typename M> void HybridTransmitter<M>::Disable()
{
    std::lock_guard<std::mutex> lock(mutex_);
    for (auto& item : transmitters_)
    {
...
template <typename M> void HybridTransmitter<M>::InitMode()
{
...
template <typename M> void HybridTransmitter<M>::ObtainConfig()
{
...
template <typename M> void HybridTransmitter<M>::InitHistory()
{
...
template <typename M> void HybridTransmitter<M>::InitTransmitters()
{
...
template <typename M> void HybridTransmitter<M>::ClearTransmitters()
{
    for (auto& item : transmitters_)
    {
        item.second->Disable();
...
template <typename M> void HybridTransmitter<M>::InitReceivers()
{
...
template <typename M> void HybridTransmitter<M>::ClearReceivers()
{
    receivers_.clear();
}
        ...
        const std::vector<typename History<M>::CachedMessage>& msgs)
{
    ...
    std::string new_channel_name = std::to_string(this->attr_.id()) +
