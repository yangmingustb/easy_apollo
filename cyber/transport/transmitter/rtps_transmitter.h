...
namespace transport
{
template <typename M> class RtpsTransmitter : public Transmitter<M>
{
public:
...
template <typename M> RtpsTransmitter<M>::~RtpsTransmitter() { Disable(); }

template <typename M> void RtpsTransmitter<M>::Enable()
{
    if (this->enabled_)
    {
        return;
...
template <typename M> void RtpsTransmitter<M>::Disable()
{
    if (this->enabled_)
    {
        publisher_ = nullptr;
