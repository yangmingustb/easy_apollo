...
namespace transport
{
template <typename M> class IntraTransmitter : public Transmitter<M>
{
public:
...
template <typename M> IntraTransmitter<M>::~IntraTransmitter() { Disable(); }

template <typename M> void IntraTransmitter<M>::Enable()
{
    if (!this->enabled_)
    {
...
template <typename M> void IntraTransmitter<M>::Disable()
{
    if (this->enabled_)
    {
        dispatcher_ = nullptr;
