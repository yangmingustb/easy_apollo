...
namespace transport
{
template <typename M> class ShmReceiver : public Receiver<M>
{
public:
...
template <typename M> ShmReceiver<M>::~ShmReceiver() { Disable(); }

template <typename M> void ShmReceiver<M>::Enable()
{
    if (this->enabled_)
    {
        return;
...
template <typename M> void ShmReceiver<M>::Disable()
{
    if (!this->enabled_)
    {
        return;
