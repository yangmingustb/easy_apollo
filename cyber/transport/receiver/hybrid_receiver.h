...
template <typename M> class HybridReceiver : public Receiver<M>
{
public:
...
template <typename M> HybridReceiver<M>::~HybridReceiver()
{
    ClearTransmitters();
    ClearReceivers();
}

template <typename M> void HybridReceiver<M>::Enable()
{
...
template <typename M> void HybridReceiver<M>::Disable()
{
    std::lock_guard<std::mutex> lock(mutex_);
    for (auto& item : receivers_)
    {
...
template <typename M> void HybridReceiver<M>::InitMode()
{
...
template <typename M> void HybridReceiver<M>::ObtainConfig()
{
...
template <typename M> void HybridReceiver<M>::InitHistory()
{
...
template <typename M> void HybridReceiver<M>::InitReceivers()
{
    ...
    for (auto& mode : modes)
    {
        switch (mode)
        {
            case OptionalMode::INTRA:
                receivers_[mode] = std::make_shared<IntraReceiver<M>>(
                        this->attr_, listener);
                break;
...
template <typename M> void HybridReceiver<M>::ClearReceivers()
{
    receivers_.clear();
}

template <typename M> void HybridReceiver<M>::InitTransmitters()
{
...
template <typename M> void HybridReceiver<M>::ClearTransmitters()
{
...
{
    std::string channel_name = std::to_string(opposite_attr.id()) +
                               std::to_string(this->attr_.id());
