...
namespace transport
{
template <typename M> class IntraReceiver : public Receiver<M>
{
public:
...
template <typename M> IntraReceiver<M>::~IntraReceiver() { Disable(); }

template <typename M> void IntraReceiver<M>::Enable()
{
    if (this->enabled_)
    {
        return;
...
template <typename M> void IntraReceiver<M>::Disable()
{
    if (!this->enabled_)
    {
        return;
