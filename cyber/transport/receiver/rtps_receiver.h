...
namespace transport
{
template <typename M> class RtpsReceiver : public Receiver<M>
{
public:
...
template <typename M> RtpsReceiver<M>::~RtpsReceiver() { Disable(); }

template <typename M> void RtpsReceiver<M>::Enable()
{
    if (this->enabled_)
    {
        return;
...
template <typename M> void RtpsReceiver<M>::Disable()
{
    if (!this->enabled_)
    {
        return;
