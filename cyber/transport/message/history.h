...
namespace transport
{
template <typename MessageT> class History
{
public:
    ...
    enabled_(false), max_depth_(1000)
{
    ...
    {
        max_depth_ = global_conf.transport_conf()
                             .resource_limit()
                             .max_history_depth();
    }
...
template <typename MessageT> History<MessageT>::~History() { Clear(); }
...
template <typename MessageT> void History<MessageT>::Clear()
{
    std::lock_guard<std::mutex> lock(msgs_mutex_);
    msgs_.clear();
}
...
template <typename MessageT> size_t History<MessageT>::GetSize() const
{
    std::lock_guard<std::mutex> lock(msgs_mutex_);
    return msgs_.size();
}
