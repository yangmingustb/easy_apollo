...
using MessageListener = std::function<void(const std::shared_ptr<MessageT>&,
                                           const MessageInfo&)>;
...
private:
            ...
            BaseHandlersType* handlers)
    {
        ...
        {
            (*handlers)[channel_id] =
                    std::map<std::string, ListenerHandlerBasePtr>();
        }
        ...
        if (handlers->find(channel_id) != handlers->end())
        {
                ...
                (*handlers)[channel_id].end())
            {
                ...
                ADEBUG << "remove " << GlobalData::GetChannelById(channel_id)
                       << "'s " << message_type << " ListenerHandler";
             ...
             const MessageInfo& message_info)
    {
        ...
        {
            AERROR << "Cant find channel "
                   << GlobalData::GetChannelById(channel_id) << " in Chain";
            return;
        }
        ...
        for (const auto& ele : channel_handlers)
        {
            ...
            if (message_type == ele.first)
            {
                ADEBUG << "Run handler for message type: " << ele.first
                       << " directly";
                auto handler =
                        std::static_pointer_cast<ListenerHandler<MessageT>>(
                                handler_base);
                if (handler == nullptr)
                {
                    continue;
            ...
            else
            {
                ...
                if (msg.empty())
                {
                    ...
                    msg.resize(msg_size);
                    if (!message::SerializeToHC(*message,
                                                const_cast<char*>(msg.data()),
                                                msg_size))
                    {
                        AERROR << "Chain Serialize error for channel id: "
                               << channel_id;
                        msg.clear();
                    }
                }
                                ...
                                const MessageInfo& message_info)
{
    ...
    {
        auto handler = std::dynamic_pointer_cast<ListenerHandler<MessageT>>(
                *handler_base);
        if (handler)
        {
            handler->Run(message, message_info);
    ...
    if (handler && created)
    {
        auto listener_wrapper =
                [this, self_id, channel_id, message_type](
                        const std::shared_ptr<MessageT>& message,
                        const MessageInfo& message_info) {
                    this->chain_->Run<MessageT>(self_id, channel_id,
                                  ...
                                  const MessageListener<MessageT>& listener)
{
    ...
    bool created = chain_->AddListener(self_id, oppo_id, channel_id,
                                       message_type, listener);
