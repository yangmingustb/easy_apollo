    ...
    auto& bucket = work_wheel_[current_work_wheel_index_];
    {
        ...
        while (ite != bucket.task_list().end())
        {
            ...
            if (task)
            {
                ...
                auto* callback = reinterpret_cast<std::function<void()>*>(
                        &(task->callback));
                          ...
                          const uint64_t current_work_wheel_index)
{
    ...
    auto work_wheel_index =
            ...
            static_cast<uint64_t>(
                    std::ceil(static_cast<double>(task->next_fire_duration_ms) /
                              TIMER_RESOLUTION_MS));
        ...
        {
            {
                std::lock_guard<std::mutex> lock(
                        current_assistant_wheel_index_mutex_);
                current_assistant_wheel_index_ = GetAssistantWheelIndex(
                        current_assistant_wheel_index_ + 1);
            }
