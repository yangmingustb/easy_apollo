 ...
 */
template <typename MessageT> class Writer : public WriterBase
{
public:
...
template <typename MessageT> Writer<MessageT>::~Writer() { Shutdown(); }

template <typename MessageT> bool Writer<MessageT>::Init()
{
    {
...
template <typename MessageT> void Writer<MessageT>::Shutdown()
{
    {
        std::lock_guard<std::mutex> g(lock_);
        if (!init_)
...
template <typename MessageT> bool Writer<MessageT>::Write(const MessageT& msg)
{
...
template <typename MessageT> void Writer<MessageT>::JoinTheTopology()
{
    // add listener
...
template <typename MessageT> void Writer<MessageT>::LeaveTheTopology()
{
...
template <typename MessageT> bool Writer<MessageT>::HasReader()
{
    RETURN_VAL_IF(!WriterBase::IsInit(), false);
