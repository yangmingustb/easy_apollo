...
public:
        ...
        is_reality_mode_(true), node_name_(node_name)
    {
        ...
        if (is_reality_mode_)
        {
            node_manager_ = service_discovery::TopologyManager::Instance()
                                    ->node_manager();
...
private:
    ...
    template <typename MessageT> void FillInAttr(proto::RoleAttributes* attr);
        ...
        -> std::shared_ptr<Reader<MessageT>>
{
    ...
    if (!is_reality_mode_)
    {
        reader_ptr = std::make_shared<blocker::IntraReader<MessageT>>(
...
void NodeChannelImpl::FillInAttr(proto::RoleAttributes* attr)
{
    ...
    if (!attr->has_proto_desc())
    {
        ...
        message::GetDescriptorString<MessageT>(attr->message_type(),
                                               &proto_desc);
