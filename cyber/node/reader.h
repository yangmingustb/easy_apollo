...
public:
     ...
     * @param reader_func is the callback function, when the message is
     * received.
...
template <typename MessageT> Reader<MessageT>::~Reader() { Shutdown(); }
...
template <typename MessageT> void Reader<MessageT>::Observe()
{
    blocker_->Observe();
}
...
template <typename MessageT> void Reader<MessageT>::Shutdown()
{
...
template <typename MessageT> void Reader<MessageT>::JoinTheTopology()
{
    // add listener
...
template <typename MessageT> void Reader<MessageT>::LeaveTheTopology()
{
...
template <typename MessageT> bool Reader<MessageT>::HasReceived() const
{
    return !blocker_->IsPublishedEmpty();
}

template <typename MessageT> bool Reader<MessageT>::Empty() const
{
    return blocker_->IsObservedEmpty();
}

template <typename MessageT> double Reader<MessageT>::GetDelaySec() const
{
...
template <typename MessageT> uint32_t Reader<MessageT>::PendingQueueSize() const
{
...
template <typename MessageT> void Reader<MessageT>::ClearData()
{
...
template <typename MessageT> uint32_t Reader<MessageT>::GetHistoryDepth() const
{
    return static_cast<uint32_t>(blocker_->capacity());
}

template <typename MessageT> bool Reader<MessageT>::HasWriter()
{
    if (!init_.load())
    {
        return false;
