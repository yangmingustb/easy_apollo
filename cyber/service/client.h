 ...
 */
template <typename Request, typename Response> class Client : public ClientBase
{
public:
                                            ...
                                            CallbackType&& cb)
{
    if (IsInit())
    {
        ...
        pending_requests_[info.seq_num()] = std::make_tuple(
                call_promise, std::forward<CallbackType>(cb), f);
        return f;
    }
    else
    {
