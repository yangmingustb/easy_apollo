...
ClassFactoryVector GetAllClassFactoryObjects()
{
    ...
    {
        ClassFactoryVector objs =
                GetAllClassFactoryObjects(baseclass_map.second);
        all_class_factory_objs.insert(all_class_factory_objs.end(),
                                      objs.begin(), objs.end());
    }
...
bool IsLibraryLoaded(const std::string& library_path, ClassLoader* class_loader)
{
    ...
    auto num_lib_loader_class_factory_objs =
            lib_loader_class_factory_objs.size();
...
bool LoadLibrary(const std::string& library_path, ClassLoader* loader)
{
    ...
    std::lock_guard<std::recursive_mutex> lck(GetLibPathSharedLibMutex());
    LibPathSharedLibVector& opened_libraries = GetLibPathSharedLibVector();
    opened_libraries.emplace_back(std::pair<std::string, SharedLibraryPtr>(
...
{
    {
        ...
        try
        {
            ...
            {
                AWARN << "ClassFactory objects still remain in memory, meaning "
                         "other"
