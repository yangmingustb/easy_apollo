...
{
public:
    template <typename M> using Reader = cyber::Reader<M>;
    ...
    template <typename T> bool GetProtoConfig(T* config) const
    {
...
protected:
    ...
    void LoadConfigFiles(const ComponentConfig& config)
    {
        if (!config.config_file_path().empty())
        {
            ...
            {
                config_file_path_ = common::GetAbsolutePath(
                        common::WorkRoot(), config.config_file_path());
            }
            else
            {
        ...
        if (!config.flag_file_path().empty())
        {
            ...
            if (flag_file_path[0] != '/')
            {
                flag_file_path = common::GetAbsolutePath(common::WorkRoot(),
                                                         flag_file_path);
            }
