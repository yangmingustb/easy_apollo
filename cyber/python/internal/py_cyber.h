...
public:
    ...
    {
        auto message =
                std::make_shared<message::PyMessageWrap>(data, data_type_);
...
public:
        ...
        channel_name_(channel), data_type_(type), node_(node), func_(nullptr)
    {
        ...
        {
            auto f = [this](const std::shared_ptr<const message::PyMessageWrap>&
                                    request) { this->cb(request); };
            reader_ = node_->CreateReader<message::PyMessageWrap>(channel, f);
        }
        else
        {
            auto f = [this](const std::shared_ptr<const message::RawMessage>&
                                    request) { this->cb_rawmsg(request); };
            reader_rawmsg_ =
...
public:
        ...
        func_(nullptr)
    {
        ...
        service_ =
                node_->CreateService<message::PyMessageWrap,
                                     message::PyMessageWrap>(service_name, f);
    }
...
public:
    ...
    {
        client_ = node_->CreateClient<message::PyMessageWrap,
                                      message::PyMessageWrap>(name);
    }
...
public:
    ...
    static std::string get_msgtype_by_channelname(
            const std::string& channel_name, uint8_t sleep_s = 0)
    {
