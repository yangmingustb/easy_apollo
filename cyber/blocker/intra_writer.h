...
template <typename MessageT> IntraWriter<MessageT>::~IntraWriter()
{
    Shutdown();
}

template <typename MessageT> bool IntraWriter<MessageT>::Init()
{
    {
...
template <typename MessageT> void IntraWriter<MessageT>::Shutdown()
{
    {
        std::lock_guard<std::mutex> g(this->lock_);
        if (!this->init_)
