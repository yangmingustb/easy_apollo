...
template <typename MessageT> IntraReader<MessageT>::~IntraReader()
{
    Shutdown();
}

template <typename MessageT> bool IntraReader<MessageT>::Init()
{
    ...
    return BlockerManager::Instance()->Subscribe<MessageT>(
            ...
            this->role_attr_.qos_profile().depth(),
            this->role_attr_.node_name(),
            std::bind(&IntraReader<MessageT>::OnMessage, this,
                      std::placeholders::_1));
}

template <typename MessageT> void IntraReader<MessageT>::Shutdown()
{
...
template <typename MessageT> void IntraReader<MessageT>::ClearData()
{
...
template <typename MessageT> void IntraReader<MessageT>::Observe()
{
    auto blocker = BlockerManager::Instance()->GetBlocker<MessageT>(
            this->role_attr_.channel_name());
    if (blocker != nullptr)
...
template <typename MessageT> bool IntraReader<MessageT>::Empty() const
{
    auto blocker = BlockerManager::Instance()->GetBlocker<MessageT>(
            this->role_attr_.channel_name());
    if (blocker != nullptr)
...
template <typename MessageT> bool IntraReader<MessageT>::HasReceived() const
{
    auto blocker = BlockerManager::Instance()->GetBlocker<MessageT>(
            this->role_attr_.channel_name());
    if (blocker != nullptr)
...
{
    BlockerManager::Instance()->Publish<MessageT>(
            this->role_attr_.channel_name(), msg);
}
...
template <typename MessageT> auto IntraReader<MessageT>::End() const -> Iterator
{
    auto blocker = BlockerManager::Instance()->GetBlocker<MessageT>(
            this->role_attr_.channel_name());
    ACHECK(blocker != nullptr);
