...
bool LatController::LoadControlConf(const ControlConf *control_conf)
{
    ...
    max_lat_acc_ =
            control_conf->lat_controller_conf().max_lateral_acceleration();
                           ...
                           const ControlConf *control_conf)
{
    ...
    */
    ...
    if (enable_leadlag_)
    {
        leadlag_controller_.Init(lat_controller_conf.reverse_leadlag_conf(),
                                 ts_);
    }
    ...
    enable_look_ahead_back_control_ = control_conf_->lat_controller_conf()
        ...
        ControlCommand *cmd)
{
        ...
        FLAGS_enable_navigation_mode_position_update)
    {
        ...
        else
        {
            ...
            auto x_diff_veh =
                    cos_map_veh * x_diff_map + sin_map_veh * y_diff_map;
            auto y_diff_veh =
                    -sin_map_veh * x_diff_map + cos_map_veh * y_diff_map;

            auto cos_theta_diff = std::cos(-theta_diff);
            auto sin_theta_diff = std::sin(-theta_diff);

            auto tx = -(cos_theta_diff * x_diff_veh -
                        sin_theta_diff * y_diff_veh);
            auto ty = -(sin_theta_diff * x_diff_veh +
                        cos_theta_diff * y_diff_veh);
            ...
            std::for_each(ptr_trajectory_points->begin(),
                          ...
                          [&cos_theta_diff, &sin_theta_diff, &tx, &ty,
                           &theta_diff](common::TrajectoryPoint &p) {
                              ...
                              auto x_new = cos_theta_diff * x -
                                           sin_theta_diff * y + tx;
                              auto y_new = sin_theta_diff * x +
                                           cos_theta_diff * y + ty;
    ...
    // Transform the coordinate of the planning trajectory from the center of
    // the rear-axis to the center of mass, if conditions matched
    ...
    */
    ...
    if (FLAGS_enable_gain_scheduler)
    {
        ...
        common::math::SolveLQRProblem(matrix_adc_, matrix_bdc_,
                                      matrix_q_updated_, matrix_r_, lqr_eps_,
    ...
    const double steer_angle_feedforward =
            ComputeFeedForward(debug->curvature());
    ...
    if (enable_leadlag_)
    {
            ...
            std::fabs(vehicle_state->linear_velocity()) < low_speed_bound_)
        {
                ...
                low_speed_bound_ - low_speed_window_)
            {
                ...
                // interplolate the augment control gain for "soft" control
                // switch
                steer_angle_feedback_augment = common::math::lerp(
                        steer_angle_feedback_augment,
                        low_speed_bound_ - low_speed_window_, 0.0,
                        low_speed_bound_,
                        std::fabs(vehicle_state->linear_velocity()));
            }
        }
    ...
    // std::cout << "steer_angle Clamp 0: " << steer_angle << "\n";

    std::cout << "vel: " << vehicle_state->linear_velocity() << "\n";
    ...
    debug->set_steer_angle_lateral_contribution(
            steer_angle_lateral_contribution);
    debug->set_steer_angle_lateral_rate_contribution(
            steer_angle_lateral_rate_contribution);
    debug->set_steer_angle_heading_contribution(
            steer_angle_heading_contribution);
...
{
    double v;
    // At reverse driving, replace the lateral translational motion dynamics
    // with the corresponding kinematic models
    if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE)
    {
        ...
        steer_angle_feedforwardterm =
                (wheelbase_ * ref_curvature + kv * v * v * ref_curvature -
                 matrix_k_(0, 2) * (lr_ * ref_curvature -
                                    lf_ * mass_ * v * v * ref_curvature / 2 /
                                            cr_ / wheelbase_)) *
                180 / M_PI * steer_ratio_ / steer_single_direction_max_degree_ *
                100;
    }
        ...
        SimpleLateralDebug *debug)
{
    ...
    {
        lookahead_station =
                common::math::lerp(lookahead_station_low_speed_,
                                   low_speed_bound_ - low_speed_window_,
                                   lookahead_station_high_speed_,
                                   low_speed_bound_, std::fabs(linear_v));
        lookback_station =
                common::math::lerp(lookback_station_low_speed_,
                                   low_speed_bound_ - low_speed_window_,
                                   lookback_station_high_speed_,
                                   low_speed_bound_, std::fabs(linear_v));
    }
