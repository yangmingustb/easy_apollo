    ...
    name_(ControlConf_ControllerType_Name(ControlConf::LON_CONTROLLER))
{
    if (FLAGS_enable_csv_debug)
    {
        ...
        if (speed_log_file_ != nullptr)
        {
            fprintf(speed_log_file_, "station_reference,"
                                     "station_error,"
                           ...
                           const ControlConf *control_conf)
{
    ...
    vehicle_param_.CopyFrom(common::VehicleConfigHelper::Instance()
                                    ->GetConfig()
                                    .vehicle_param());
        ...
        control::ControlCommand *cmd)
{
    ...
    {
        station_error_limited =
                common::math::Clamp(debug->station_error(),
                                    -station_error_limit, station_error_limit);
    }
    ...
    if (enable_leadlag)
    {
        acceleration_cmd_closeloop = speed_leadlag_controller_.Control(
                acceleration_cmd_closeloop, ts);
    ...
    if (FLAGS_enable_csv_debug && speed_log_file_ != nullptr)
    {
        fprintf(speed_log_file_,
                ...
                debug->preview_acceleration_reference(),
                acceleration_cmd_closeloop, acceleration_cmd,
                debug->acceleration_lookup(), debug->speed_lookup(),
                calibration_value, throttle_cmd, brake_cmd,
                debug->is_full_stop());
    }
        ...
        SimpleLongitudinalDebug *debug)
{
    ...
    double lon_speed =
            vehicle_state->linear_velocity() * std::cos(heading_error);
    ...
    debug->set_preview_station_error(preview_point.path_point().s() -
                                     s_matched);
