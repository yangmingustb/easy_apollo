...
public:
    ...
    /**
     * @brief compute steering target and throttle/ brake based on current
     * vehicle status and target trajectory
...
protected:
    ...
    void ComputeLongitudinalErrors2(const TrajectoryAnalyzer* trajectory,
                                   SimpleMPCDebug* debug);
    ...
    int limit_steer_angle_by_trajectory(
    ...
    // 方向盘和路径曲率
    std::unique_ptr<Interpolation1D> steer_kappa_interpolation_;
    ...
    // Enable the feedback-gain-related compensation components in the
    // feedfoward term for steering control
