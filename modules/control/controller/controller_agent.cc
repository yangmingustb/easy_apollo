...
{
    // AINFO << "Only support MPC controller or Lat + Lon controllers as of
    // now";
                             ...
                             const ControlConf *control_conf)
{
    ...
    AINFO << "ControllerAgent init finish";
...
Status ControllerAgent::ComputeControlCommand(
        ...
        const planning::ADCTrajectory *trajectory, control::ControlCommand *cmd)
{
    for (auto &controller : controller_list_)
    {
        if (debug_mpc)
        ...
        controller->ComputeControlCommand(localization, chassis, trajectory,
                                          cmd);
