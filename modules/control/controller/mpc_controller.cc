...
bool MPCController::LoadControlConf(const ControlConf* control_conf)
{
    ...
    throttle_lowerbound_ = std::max(
            vehicle_param_.throttle_deadzone(),
            control_conf->mpc_controller_conf().throttle_minimum_action());
    brake_lowerbound_ = std::max(
            vehicle_param_.brake_deadzone(),
    ...
    enable_mpc_feedforward_compensation_ =
            control_conf->mpc_controller_conf()
                    .enable_mpc_feedforward_compensation();

    unconstrained_control_diff_limit_ =
            control_conf->mpc_controller_conf()
                    .unconstrained_control_diff_limit();
                           ...
                           const ControlConf* control_conf)
{
    ...
    enable_mrac_ =
            control_conf->lat_controller_conf().enable_steer_mrac_control();
    if (enable_mrac_)
    {
        mrac_controller_.Init(
                control_conf->lat_controller_conf().steer_mrac_conf(),
    ...
    // TODO
    {
        ...
        xy5.push_back(std::make_pair(0.0, 2.0));
        xy5.push_back(std::make_pair(0.001, 4.0));
        xy5.push_back(std::make_pair(0.002, 6.0));
        xy5.push_back(std::make_pair(0.01, 200.0));
        xy5.push_back(std::make_pair(0.02, 400.0));
        // xy5.push_back(std::make_pair(0.1, 500.0));
        steer_kappa_interpolation_.reset(new Interpolation1D);
        steer_kappa_interpolation_->Init(xy5);
    }
...
// 根据轨迹的曲率来限制方向盘角度, 曲率越大限制越小
int MPCController::limit_steer_angle_by_trajectory(
        const planning::ADCTrajectory* planning_published_trajectory)
{
    // auto tra_pts = planning_published_trajectory->trajectory_point_size();
    auto tra_pts = trajectory_analyzer_.trajectory_points();
    for (int i = 0; i < tra_pts.size(); ++i)
    {
        std::cout << tra_pts[i].path_point().kappa() << ","
                  << tra_pts[i].relative_time() << "\n";
    }
        ...
        ControlCommand* cmd)
{
    ...
    // ComputeLongitudinalErrors2(&trajectory_analyzer_, debug);

    // std::cout << chassis->DebugString();

    // std::cout << "can state steer"
    //           << chassis->steering_percentage() * 5.05 / 15.7 << "\n ";

    // std::cout << injector_->vehicle_state()->vehicle_state().DebugString();
    ...
    // Add gain scheduler for higher speed steering, default is 0
    ...
    if (enable_mpc_feedforward_compensation_)
    {
        ...
        if (fabs(unconstrained_control_diff) <=
            unconstrained_control_diff_limit_)
        {
            steer_angle_ff_compensation = Wheel2SteerPct(
                    debug->curvature() *
                    (control_gain[0](0, 2) *
        ...
        else
        {
            control_gain_truncation_ratio =
                    control[0](0, 0) / unconstrained_control;
            steer_angle_ff_compensation = Wheel2SteerPct(
                    debug->curvature() *
                    (control_gain[0](0, 2) *
                             (lr_ - lf_ / cr_ * mass_ * v * v / wheelbase_) -
    ...
    // front wheel angle
    // std::cout << "origin steer"
    //           << steer_angle_feedback * 5.05 / 15.7 << " "
    //           << "feed for "
    //           << steer_angle_feedforwardterm_updated_ * 5.05 / 15.7
    //           << "com "
    //           << steer_angle_ff_compensation * 5.05 / 15.7
    //           << "steer_angle end " << steer_angle * 5.05 / 15.7 ;

    // Re-compute the steering command if the MRAC control is enabled, with
    // steer angle limitation and steer rate . default value is 0
    if (enable_mrac_)
    {
        ...
        {
            steer_state(1, 0) =
                    (steering_position - pre_steering_position_) / ts_;
        }
        ...
        const double steer_limit =
                std::atan(max_lat_acc_ * wheelbase_ /
                          (vehicle_state->linear_velocity() *
                           vehicle_state->linear_velocity())) *
                steer_ratio_ * 180 / M_PI / steer_single_direction_max_degree_ *
                100;

        const double steer_diff_with_max_rate =
                FLAGS_enable_maximum_steer_rate_limit
                        ? vehicle_param_.max_steer_angle_rate() * ts_ * 180 /
                                  M_PI / steer_single_direction_max_degree_ *
                                  100
                        : 100.0;
    ...
    {
        // Clamp the steer angle by trajectory
        TrajectoryPoint pt =
                trajectory_analyzer_.QueryNearestPointByRelativeTime(0.8);
        double steer_limit_kappa = steer_kappa_interpolation_->Interpolate(
                std::abs(pt.path_point().kappa()));

        // std::cout << "kappa: " << pt.path_point().kappa()
        //           << "\nsteer_limit: " << steer_limit_kappa << "\n";

        steer_limit_kappa =
                steer_limit_kappa / steer_single_direction_max_degree_ * 100;
        steer_angle = common::math::Clamp(steer_angle, -steer_limit_kappa,
                                          steer_limit_kappa);
        // limit_steer_angle_by_trajectory(planning_published_trajectory);
    ...
    // std::cout << "limit steer" << steer_angle * 5.05 / 15.7 << "\n";

    steer_angle = digital_filter_.Filter(steer_angle);

    // std::cout << "filter steer" << steer_angle * 5.05 / 15.7 << "\n";

    // Clamp the steer angle to -100.0 to 100.0
    steer_angle = common::math::Clamp(steer_angle, -100.0, 100.0);

    // std::cout << "clamp steer" << steer_angle * 5.05 / 15.7 << "\n";
    ...
    {
        calibration_value = control_interpolation_->Interpolate(
                std::make_pair(injector_->vehicle_state()->linear_velocity(),
                               acceleration_cmd));
    }
    ...
    debug->set_steer_angle_feedforward_compensation(
            steer_angle_ff_compensation);
        ...
        const TrajectoryAnalyzer& trajectory_analyzer, SimpleMPCDebug* debug)
{
    ...
    debug->set_lateral_jerk(
            (debug->lateral_acceleration() - previous_lateral_acceleration_) /
            ts_);
    ...
    debug->set_heading_jerk(
            ...
            ts_);
    debug->set_ref_heading_jerk((debug->ref_heading_acceleration() -
        ...
        const TrajectoryAnalyzer* trajectory_analyzer, SimpleMPCDebug* debug)
{
    ...
    trajectory_analyzer->ToTrajectoryFrame(
            injector_->vehicle_state()->x(), injector_->vehicle_state()->y(),
            injector_->vehicle_state()->heading(),
            injector_->vehicle_state()->linear_velocity(), matched_point,
    ...
    double one_minus_kappa_lat_error =
            1 - reference_point.path_point().kappa() * linear_v *
    ...
    double jerk_reference = (debug->acceleration_reference() -
    ...
    debug->set_jerk_error(jerk_reference -
                          lon_jerk / one_minus_kappa_lat_error);
    previous_acceleration_reference_ = debug->acceleration_reference();
    previous_acceleration_ = debug->acceleration_feedback();
}

void MPCController::ComputeLongitudinalErrors2(
        const TrajectoryAnalyzer* trajectory_analyzer, SimpleMPCDebug* debug)
{
    // the decomposed vehicle motion onto Frenet frame
    // s: longitudinal accumulated distance along reference trajectory
    ...
    const auto position_matched_point =
            trajectory_analyzer->QueryMatchedPathPoint(
                    injector_->vehicle_state()->x(),
                    injector_->vehicle_state()->y());

    // update vehicle state : s, ds, l, dl
    trajectory_analyzer->ToTrajectoryFrame(
            injector_->vehicle_state()->x(), injector_->vehicle_state()->y(),
            injector_->vehicle_state()->heading(),
            injector_->vehicle_state()->linear_velocity(),
            position_matched_point, &s_matched, &s_dot_matched, &d_matched,
            &d_dot_matched);

    // get absolute time
    const double current_control_time = Clock::NowInSeconds();

    // 时间参考点还是位置参考点，位置参考点比较准确
    TrajectoryPoint reference_point;
    ...
    // else
    {
        ...
        double front_dist =
                injector_->vehicle_state()->linear_velocity() * 0.02;

        reference_point = trajectory_analyzer->QueryNearestFrontPointByPosition(
                injector_->vehicle_state()->x(),
                injector_->vehicle_state()->y(), front_dist);
    }

    ADEBUG << "matched point:" << position_matched_point.DebugString();
    ADEBUG << "reference point:" << reference_point.DebugString();

    const double linear_v = injector_->vehicle_state()->linear_velocity();
    const double linear_a = injector_->vehicle_state()->linear_acceleration();
    double heading_error =
            common::math::NormalizeAngle(injector_->vehicle_state()->heading() -
                                         position_matched_point.theta());
