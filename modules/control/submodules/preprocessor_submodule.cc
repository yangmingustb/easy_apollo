...
bool PreprocessorSubmodule::Proc(const std::shared_ptr<LocalView> &local_view)
{
    ...
    //    control_preprocessor.header().lidar_timestamp(), start_time,
    //    end_time);
        ...
        Preprocessor *control_preprocessor)
{
    ...
    if (!status.ok())
    {
        AERROR_EVERY(100) << "Control input data failed: "
                          << status.error_message();
        auto mutable_engage_advice =
                control_preprocessor->mutable_engage_advice();
    ...
    {
        return Status(ErrorCode::CONTROL_ESTOP_ERROR,
                      absl::StrCat("estop from planning : ",
                                   control_preprocessor->local_view()
                                           .trajectory()
                                           .estop()
                                           .reason()));
    ...
    if (FLAGS_enable_gear_drive_negative_speed_protection)
    {
        ...
        auto first_trajectory_point = control_preprocessor->local_view()
                                              .trajectory()
                                              .trajectory_point(0);
        ...
        {
            return Status(ErrorCode::CONTROL_ESTOP_ERROR,
                          absl::StrCat("estop for empty planning trajectory, "
                                       "planning headers: ",
                                       control_preprocessor->local_view()
                                               .trajectory()
                                               .header()
