                                                    ...
                                                    const double y) const
{
    ...
    size_t index_end = index_min + 1 == trajectory_points_.size()
                               ? index_min
                               : index_min + 1;
...
TrajectoryPoint TrajectoryAnalyzer::QueryNearestFrontPointByPosition(
        const double x, const double y, const double move_dist) const
{
    double d_min = PointDistanceSquare(trajectory_points_.front(), x, y);
    size_t index_min = 0;
    ...
    // find front point
    double neasrest_point_s  = trajectory_points_[index_min].path_point().s();

    double accumulate_s = 0.0;

    size_t index_front = index_min;

    for (size_t i = index_min; i < trajectory_points_.size(); ++i)
    {
        double delta_s =
                trajectory_points_[i].path_point().s() - neasrest_point_s;

        if (delta_s >= move_dist)
        {
            break;
        }
        index_front = i;
    ...
    return trajectory_points_[index_front];
}
                                                   ...
                                                   const double y) const
{
    ...
    double s = common::math::GoldenSectionSearch(
            dist_square, p0.path_point().s(), p1.path_point().s());
    p.set_s(s);
    p.set_x(common::math::lerp(p0.path_point().x(), p0.path_point().s(),
                               p1.path_point().x(), p1.path_point().s(), s));
    p.set_y(common::math::lerp(p0.path_point().y(), p0.path_point().s(),
                               p1.path_point().y(), p1.path_point().s(), s));
    p.set_theta(common::math::slerp(
            p0.path_point().theta(), p0.path_point().s(),
