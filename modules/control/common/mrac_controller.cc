                               ...
                               const double input_rate_limit)
{
    ...
    {
        AERROR << "MRAC: model build failed; will work as a unity compensator. "
                  "The "
...
Status MracController::SetReferenceModel(const MracConf& mrac_conf)
{
    ...
    if (((mrac_conf.reference_time_constant() < Epsilon &&
          model_order_ == 1)) ||
        ((mrac_conf.reference_natural_frequency() < Epsilon &&
          model_order_ == 2)))
    {
        const auto error_msg = absl::StrCat(
                "mrac controller error: reference model time-constant "
                "parameter: ",
...
Status MracController::SetAdaptionModel(const MracConf& mrac_conf)
{
    ...
    {
        const auto error_msg = absl::StrCat(
                "mrac controller error: adaption matrix p element number: ",
                p_size, ", state gain number: ", x_size,
                ", and anti-windup compensation gain number: ", aw_size,
                " in configuration file do not match the model number: ",
                model_order_);
        AERROR << error_msg;
        return Status(ErrorCode::CONTROL_INIT_ERROR, error_msg);
    }
...
Status MracController::BuildAdaptionModel()
{
    ...
    {
        const std::string error_msg = "Solution of the algebraic Lyapunov "
                                      "equation is not symmetric positive "
                                      "definite";
        AERROR << error_msg;
        return Status(ErrorCode::CONTROL_INIT_ERROR, error_msg);
    }
...
void MracController::UpdateReference()
{
    ...
    state_reference_.col(0) =
            ...
            ((matrix_i + ts_ * 0.5 * matrix_a_reference_) *
                     state_reference_.col(1) +
                                    ...
                                    const Matrix gain_adp)
{
    ...
    law_adp->col(0) =
            ...
            0.5 * ts_ * gain_adp *
                    (state_adp.col(0) *
                             (state_error.col(0).transpose() +
                              compensation_anti_windup_.col(0).transpose()) +
                     state_adp.col(1) *
                             (state_error.col(1).transpose() +
                                            ...
                                            const double previous_command)
{
    ...
    offset_windup(0, 0) = ((control_command > bound_command_)
                                   ? bound_command_ - control_command
                                   : 0.0) +
                          ((control_command < -bound_command_)
                                   ? -bound_command_ - control_command
                                   : 0.0);
    if (model_order_ > 1)
    {
        offset_windup(1, 0) =
                ((control_command >
                  previous_command + bound_command_rate_ * ts_)
                         ? bound_command_rate_ -
                                   (control_command - previous_command) / ts_
                         : 0.0) +
                ((control_command <
                  previous_command - bound_command_rate_ * ts_)
                         ? -bound_command_rate_ -
                                ...
                                const double previous_output, double* output)
{
    ...
    {
        *output =
                (-bound_command_ > previous_output - bound_command_rate_ * ts_)
                        ? -bound_command_
                        : previous_output - bound_command_rate_ * ts_;
    ...
    else
    {
        *output = output_unbounded;
        // if output does not violate neither bound nor rate bound, then status
        // = 0
        status = 0;
    }
    return status;
        ...
        const Matrix& state_reference_init)
{
        ...
        state_reference_init.cols() != 1)
    {
        AWARN << "failed to set the initial reference states, due to the given "
                 "state size: "
              << state_reference_init.rows() << " x "
              << state_reference_init.cols()
        ...
        const Matrix& gain_state_adaption_init)
{
    ...
    {
        AWARN << "failed to set the initial state adaption gains, due to the "
                 "given "
                 "state size: "
...
void MracController::SetStateAdaptionRate(const double ratio_state)
{
    if (ratio_state < 0.0)
    {
        AWARN << "failed to set the state adaption rate, due to new ratio < 0; "
                 "the "
...
void MracController::SetInputAdaptionRate(const double ratio_input)
{
    if (ratio_input < 0.0)
    {
        AWARN << "failed to set the input adaption rate, due to new ratio < 0; "
                 "the "
                 "current ratio is still: "
              << gamma_ratio_input_;
    }
...
void MracController::SetNonlinearAdaptionRate(const double ratio_nonlinear)
{
    ...
    {
        AWARN << "failed to set the nonlinear adaption rate, due to new ratio "
                 "< 0; "
