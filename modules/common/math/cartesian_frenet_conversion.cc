    ...
    ptr_s_condition->at(2) =
            (a * cos_delta_theta -
             ptr_s_condition->at(1) * ptr_s_condition->at(1) *
                     (ptr_d_condition->at(1) * delta_theta_prime -
                      kappa_r_d_prime)) /
            one_minus_kappa_r_d;
}
...
double CartesianFrenetConverter::CalculateLateralDerivative(const double rtheta,
                                                            const double theta,
                                                            const double l,
                                                            const double rkappa)
{
        ...
        const double kappa, const double rdkappa, const double l)
{
    ...
    const double res =
            -(rdkappa * l + rkappa * dl) * std::tan(theta - rtheta) +
