...
void VehicleConfigHelper::Init(const VehicleConfig &vehicle_params)
{
    ...
    if (vehicle_config_.vehicle_param().max_steer_angle() < 0.0001)
    {
        AERROR << "vehicle params error";
    }
    if (vehicle_config_.vehicle_param().steer_ratio() < 0.0001)
    {
        AERROR << "vehicle params error";
    }
    if (vehicle_config_.vehicle_param().wheel_base() < 0.0001)
    {
        AERROR << "vehicle params error";
    }
