        ...
        autotuning::TrajectoryPointwiseFeature* const point_feature) const
{
    ...
    for (int i = 0;
         i < raw_point_feature.speed_feature().virtual_decision_size(); ++i)
    {
    ...
    {
        const auto& obj_decision =
                raw_point_feature.speed_feature().overtake(i);
        ...
        const
{
    ...
    input_feature->set_lateral_distance(
            std::fabs(obj_raw_feature.relative_l()));
}
