...
HybridAStar::HybridAStar(const PlannerOpenSpaceConfig& open_space_conf)
{
    ...
    reed_shepp_generator_ = std::make_unique<ReedShepp>(
            vehicle_param_, planner_open_space_config_);
    ...
    traj_forward_penalty_ = planner_open_space_config_.warm_start_config()
                                    .traj_forward_penalty();
    traj_back_penalty_ =
            planner_open_space_config_.warm_start_config().traj_back_penalty();
    traj_gear_switch_penalty_ = planner_open_space_config_.warm_start_config()
        ...
        const std::shared_ptr<ReedSheppPath> reeds_shepp_to_end)
{
    std::shared_ptr<Node3d> node = std::shared_ptr<Node3d>(new Node3d(
            reeds_shepp_to_end->x, reeds_shepp_to_end->y,
            reeds_shepp_to_end->phi, XYbounds_, planner_open_space_config_));
...
bool HybridAStar::ValidityCheck(std::shared_ptr<Node3d> node)
{
    ...
    for (size_t i = check_start_index; i < node_step_size; ++i)
    {
        ...
        Box2d bounding_box =
                Node3d::GetBoundingBox(vehicle_param_, traversed_x[i],
        ...
        for (const auto& obstacle_linesegments : obstacles_linesegments_vec_)
        {
                 ...
                 obstacle_linesegments)
            {
                ...
                {
                    ADEBUG << "collision start at x: "
                           << linesegment.start().x();
                    ADEBUG << "collision start at y: "
                           << linesegment.start().y();
        ...
        std::shared_ptr<Node3d> current_node, size_t next_node_index)
{
    ...
    for (size_t i = 0; i < arc / step_size_; ++i)
    {
        ...
        const double next_phi = common::math::NormalizeAngle(
                last_phi + traveled_distance / vehicle_param_.wheel_base() *
                                   std::tan(steering));
    ...
    std::shared_ptr<Node3d> next_node = std::shared_ptr<Node3d>(
            new Node3d(intermediate_x, intermediate_y, intermediate_phi,
        ...
        std::vector<HybridAStartResult>* partitioned_result)
{
    ...
    if (x.size() != y.size() || x.size() != phi.size())
    {
        AERROR << "states sizes are not equal when do trajectory partitioning "
                  "of "
    ...
    bool current_gear = std::abs(common::math::NormalizeAngle(
    ...
    for (size_t i = 0; i < horizon - 1; ++i)
    {
        ...
        bool gear = std::abs(common::math::NormalizeAngle(
                            tracking_angle - heading_angle)) < (M_PI_2);
                       ...
                       HybridAStartResult* result)
{
    ...
    for (const auto& obstacle_vertices : obstacles_vertices_vec)
    {
        ...
        for (size_t i = 0; i < vertices_num - 1; ++i)
        {
            common::math::LineSegment2d line_segment =
                    common::math::LineSegment2d(obstacle_vertices[i],
    ...
    while (!open_pq_.empty())
    {
        ...
        {
            std::shared_ptr<Node3d> next_node =
                    Next_node_generator(current_node, i);
