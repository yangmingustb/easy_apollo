...
ReedShepp::ReedShepp(const common::VehicleParam& vehicle_param,
    ...
    vehicle_param_(vehicle_param), planner_open_space_config_(open_space_conf)
{
                      ...
                      RSPParam* param)
{
    ...
    if (rho <= 1.0 && rho >= 0.0)
    {
        ...
        if (u >= 0 && u <= 0.5 * M_PI)
        {
            std::pair<double, double> tau_omega =
                    calc_tau_omega(u, -u, xi, eta, phi);
                      ...
                      RSPParam* param)
{
    ...
    if (rho >= 2.0)
    {
        ...
        if (u <= 0.0)
        {
            t = common::math::NormalizeAngle(atan2(
        ...
        const std::shared_ptr<Node3d> end_node, ReedSheppPath* shortest_path)
{
    ...
    size_t point_num = static_cast<size_t>(std::floor(
            shortest_path->total_length / step_scaled +
    ...
    for (size_t i = 0; i < px.size(); ++i)
    {
        ...
        shortest_path->y.push_back(-std::sin(-start_node->GetPhi()) * px.at(i) +
                                   ...
                                   start_node->GetY());
        shortest_path->phi.push_back(common::math::NormalizeAngle(
...
#pragma omp parallel for schedule(dynamic, 2) num_threads(8)
    for (int i = 0; i < RSP_nums; ++i)
    {
        ...
        {  // CCC, LRL case, backward
            LRL(x_param * xb, y_param * yb, x_param * y_param * dphi,
                &RSP_param);
            if (y_param > 0)
            {
                rd_type = "LRL";
        ...
        {  // CCCC, LRLRn
            LRLRn(x_param * x, y_param * y, x_param * y_param * dphi,
                  &RSP_param);
            if (y_param > 0.0)
            {
                rd_type = "LRLR";
        ...
        {  // CCCC, LRLRp
            LRLRp(x_param * x, y_param * y, x_param * y_param * dphi,
                  &RSP_param);
            if (y_param > 0.0)
            {
                rd_type = "LRLR";
        ...
        {  // CCSCC, LRSLR
            tmp_length = 5;
            LRSLR(x_param * x, y_param * y, x_param * y_param * dphi,
                  &RSP_param);
            if (y_param > 0.0)
            {
                rd_type = "LRSLR";
        ...
        if (tmp_length > 0)
        {
            if (RSP_param.flag && !SetRSPPar(tmp_length, RSP_lengths, rd_type,
