        ...
        std::shared_ptr<Node2d> current_node)
{
    ...
    std::shared_ptr<Node2d> up = std::make_shared<Node2d>(
            current_node_x, current_node_y + 1.0, XYbounds_);
    ...
    std::shared_ptr<Node2d> right = std::make_shared<Node2d>(
            current_node_x + 1.0, current_node_y, XYbounds_);
        ...
        for (auto& next_node : next_nodes)
        {
            ...
            if (open_set.find(next_node->GetIndex()) == open_set.end())
            {
                ...
                next_node->SetHeuristic(EuclidDistance(
                        next_node->GetGridX(), next_node->GetGridY(),
                ...
                obstacles_linesegments_vec)
{
    ...
    max_grid_y_ =
            std::round((XYbounds_[3] - XYbounds_[2]) / xy_grid_resolution_);
    max_grid_x_ =
            std::round((XYbounds_[1] - XYbounds_[0]) / xy_grid_resolution_);
    std::shared_ptr<Node2d> end_node =
            std::make_shared<Node2d>(ex, ey, xy_grid_resolution_, XYbounds_);
    obstacles_linesegments_vec_ = obstacles_linesegments_vec;
    ...
    while (!open_pq.empty())
    {
        ...
        for (auto& next_node : next_nodes)
        {
            ...
            {
                if (open_set[next_node->GetIndex()]->GetCost() >
                    next_node->GetCost())
                {
                    open_set[next_node->GetIndex()]->SetCost(
                            next_node->GetCost());
...
{
    std::string index =
            Node2d::CalcIndex(sx, sy, xy_grid_resolution_, XYbounds_);
