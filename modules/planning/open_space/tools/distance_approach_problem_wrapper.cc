...
public:
    ...
    bool HPresentationObstacle()
    {
        ...
        if (!ObsHRep(obstacles_num_, obstacles_edges_num_,
                     obstacles_vertices_vec_, &obstacles_A_, &obstacles_b_))
        {
...
extern "C"
{
    ...
    bool DistanceSmoothing(const apollo::planning::PlannerOpenSpaceConfig&
                                   planner_open_space_config,
                           const ObstacleContainer& obstacles, double sx,
    ...
    {
        // load Warm Start result(horizon is the "N", not the size of step
        // points)
            ...
            planner_open_space_config.enable_linear_interpolation())
        {
            ...
            std::vector<double> x_extend = VectorLinearInterpolation(
                    hybrid_a_star_result->x, extend_size);
            x = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(x_extend.data(),
                                                              horizon_ + 1);

            std::vector<double> y_extend = VectorLinearInterpolation(
                    hybrid_a_star_result->y, extend_size);
            y = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(y_extend.data(),
                                                              horizon_ + 1);

            std::vector<double> phi_extend = VectorLinearInterpolation(
                    hybrid_a_star_result->phi, extend_size);
            phi = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(
                    phi_extend.data(), horizon_ + 1);

            std::vector<double> v_extend = VectorLinearInterpolation(
            ...
            {
                ADEBUG << "i: " << i
                       << ", val: " << hybrid_a_star_result->steer[i];
            }
        ...
        ego_ << front_to_center, right_to_center, back_to_center,
                left_to_center;

        // result for distance approach problem
        Eigen::MatrixXd l_warm_up;
        Eigen::MatrixXd n_warm_up;
        Eigen::MatrixXd s_warm_up = Eigen::MatrixXd::Zero(
                obstacles.GetObstaclesNum(), horizon_ + 1);
        ...
        {
            bool dual_variable_warm_start_status =
                    dual_variable_warm_start_ptr->Solve(
                            horizon_, ts_, ego_, obstacles.GetObstaclesNum(),
                            obstacles.GetObstaclesEdgesNum(),
                            obstacles.GetAMatrix(), obstacles.GetbMatrix(), xWS,
        ...
        bool status = distance_approach_ptr->Solve(
                x0, xF, last_time_u, horizon_, ts_, ego_, xWS, uWS, l_warm_up,
                n_warm_up, s_warm_up, XYbounds, obstacles.GetObstaclesNum(),
    ...
    bool DistancePlan(HybridAStar* hybridA_ptr,
                      ...
                      double* XYbounds)
    {
        ...
        if (FLAGS_enable_parallel_trajectory_smoothing)
        {
            ...
            //                    piece_wise_ey, piece_wise_ephi,
            //                    std::ref(XYbounds_), ith_trajectories,
            //                    ith_state_result, ith_control_result,
            //                    ith_time_result, ith_dual_l_result,
            //                    ith_dual_n_result));
            // }
            ...
            for (size_t i = 0; i < size; ++i)
            {
                ...
                if (planner_open_space_config_
                            .enable_check_parallel_trajectory())
                {
                    AINFO << "trajectory idx: " << i;
                    AINFO << "trajectory pt number: "
                          << partition_trajectories[i].x.size();
                }

                if (!DistanceSmoothing(
                            planner_open_space_config_, *obstacles_ptr,
            ...
            state_result_ds.col(k) = state_result_ds_vec.back().col(
                    state_result_ds_vec.back().cols() - 1);

            const uint64_t control_dimension =
                    control_result_ds_vec.front().rows();
            Eigen::MatrixXd control_result_ds;
            control_result_ds.resize(control_dimension,
        ...
        else
        {
            ...
            if (!DistanceSmoothing(planner_open_space_config_, *obstacles_ptr,
                                   sx, sy, sphi, ex, ey, ephi, XYbounds_,
                                   &hybrid_astar_result, &state_result_ds,
                                   &control_result_ds, &time_result_ds,
                                   &dual_l_result_ds, &dual_n_result_ds,
                                   dual_total, ipopt_total))
            {
                return false;
            }
            *(result_ptr->PrepareHybridAResult()) = hybrid_astar_result;
    ...
    void DistanceGetResult(ResultContainer* result_ptr,
                           ObstacleContainer* obstacles_ptr, double* x,
                           double* y, double* phi, double* v, double* a,
                           double* steer, double* opt_x, double* opt_y,
                           double* opt_phi, double* opt_v, double* opt_a,
                           double* opt_steer, double* opt_time,
                           double* opt_dual_l, double* opt_dual_n,
