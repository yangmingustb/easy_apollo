...
public:
    bool OpenSpaceROI()
    {
        // left or right of the parking lot is decided when viewing the parking
        // spot open upward
        ...
        if (!(nearby_path_->GetProjection(left_top, &left_top_s, &left_top_l) &&
              nearby_path_->GetProjection(right_top, &right_top_s,
                                          &right_top_l)))
        {
        ...
        double start_s =
                center_line_s - planner_open_space_config_.roi_config()
                                        .roi_longitudinal_range_start();
        double end_s = center_line_s + planner_open_space_config_.roi_config()
        ...
        // rotate the points to have the lane to be horizontal to x axis and
        // scale them base on the origin point
        ...
        double y_min =
                std::min({left_down.y(), start_right.y(), start_left.y()});
        double y_max =
                std::max({left_down.y(), start_right.y(), start_left.y()});
    ...
    bool LoadMap(const std::string& lane_id, const std::string& parking_id)
    {
        ...
        std::cout << "the lane found is " << nearby_lane->id().id()
                  << std::endl;
        ...
        for (const auto parking_overlap : parking_space_overlaps)
        {
            if (parking_overlap.object_id != parking_id)
            {
                target_parking_spot_ =
                        map_ptr->GetParkingSpaceById(target_lane_id);
                std::cout << "parking_overlap.object_id is "
                          << target_lane_id.id() << std::endl;
            }
        }
        if (target_parking_spot_ == nullptr)
        {
            std::cout << "No such parking spot found " << parking_id
                      << std::endl;
            return false;
        }
...
extern "C"
{
    ...
    bool ROITest(OpenSpaceROITest* test_ptr, char* lane_id, char* parking_id,
                 ...
                 double* unrotated_roi_boundary_y, double* roi_boundary_x,
                 double* roi_boundary_y, double* parking_spot_x,
