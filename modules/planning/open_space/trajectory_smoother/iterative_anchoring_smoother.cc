        ...
        DiscretizedTrajectory* discretized_trajectory)
{
    ...
    double delta_s =
            path_length / std::ceil(path_length / interpolated_delta_s);
    ...
    {
        AERROR << "Generate initial bounds failed, path point to close to "
                  "obstacle";
        return false;
    }
                                 ...
                                 &input_colliding_point_index_))
    {
        ...
        // if (!ReAnchoring(colliding_point_index,
        // &interpolated_warm_start_path)) {
        ...
        std::vector<std::pair<double, double>>* const point2d)
{
    ...
    // the point and the other point equal to the given warm start initial or
    // end heading
    const double first_to_second_dx =
            point2d->at(1).first - point2d->at(0).first;
    ...
    const double second_last_to_last_dy = point2d->at(path_size - 1).second -
                                          point2d->at(path_size - 2).second;
    ...
    end_vec.SelfRotate(gear_ ? NormalizeAngle(end_heading + M_PI)
                             : end_heading);
    end_vec += last_point;
        ...
        DiscretizedPath* path_points)
{
    ...
    for (const auto index : colliding_point_index)
    {
        ...
        {
            AERROR << "Initial and end points collision avoid condition "
                      "failed.";
            return false;
        }
        if (index == 1 || index == path_points->size() - 2)
        {
            AERROR << "second to last point or second point pos reanchored. "
                      "Heading "
                      "discontinuity might "
                      "happen";
        }
            ...
            for (size_t j = index - 1; j < index + 2; ++j)
            {
                const double heading =
                        gear_ ? path_points->at(j).theta()
                              : NormalizeAngle(path_points->at(j).theta() +
                                               M_PI);
                Box2d ego_box(
                        {path_points->at(j).x() +
                ...
                for (const auto& obstacle_linesegments :
            ...
            if (is_colliding)
            {
                ...
                if (index == 1)
                {
                    const double adjust_theta =
                            path_points->at(index - 1).theta();
                    const double delta_s =
                            std::abs(path_points->at(index).s() -
                                     path_points->at(index - 1).s());
                    double rand_dev =
                            common::math::Clamp(length_dis(gen), 0.8, -0.8);
                    double adjusted_delta_s = delta_s * (1.0 + rand_dev);
                    path_points->at(index).set_x(
                            path_points->at(index - 1).x() +
                            adjusted_delta_s * std::cos(adjust_theta));
                    path_points->at(index).set_y(
                            path_points->at(index - 1).y() +
                            adjusted_delta_s * std::sin(adjust_theta));
                }
                else if (index == path_points->size() - 2)
                {
                    const double adjust_theta = NormalizeAngle(
                            path_points->at(index + 1).theta() + M_PI);
                    const double delta_s =
                            std::abs(path_points->at(index + 1).s() -
                ...
                else
                {
                    double rand_dev_x = common::math::Clamp(
                            pos_dis(gen), 2.0 * reanchoring_pos_stddev,
        ...
        const DiscretizedPath& path_points, std::vector<double>* initial_bounds)
{
    ...
    for (const auto& path_point : path_points)
    {
        ...
        for (const auto& obstacle_linesegments : obstacles_linesegments_vec_)
        {
            ...
            {
                min_bound = std::min(min_bound,
                                     linesegment.DistanceTo(
        ...
        DiscretizedPath* smoothed_path_points)
{
    ...
    while (!is_collision_free)
    {
        ...
        if (!fem_pos_smoother.Solve(raw_point2d, flexible_bounds, &opt_x,
                                    &opt_y))
        {
        ...
        {
            AERROR << "Return by fem_pos_smoother is wrong. Size smaller than "
                      "2 ";
            return false;
        }
        ...
        std::vector<size_t>* colliding_point_index)
{
    ...
    for (size_t i = 0; i < path_points_size; ++i)
    {
        ...
        const double heading =
                gear_ ? path_points[i].theta()
                      : NormalizeAngle(path_points[i].theta() + M_PI);
        Box2d ego_box({path_points[i].x() +
                               center_shift_distance_ * std::cos(heading),
                       path_points[i].y() +
                               center_shift_distance_ * std::sin(heading)},
                      heading, ego_length_, ego_width_);
                                             ...
                                             SpeedData* smoothed_speeds)
{
    ...
    for (size_t i = 1; i < num_of_knots; ++i)
    {
        ...
        smoothed_speeds->AppendSpeedPoint(
                s[i], delta_t * static_cast<double>(i), ds[i], dds[i],
    ...
    std::for_each(
            ...
            [](TrajectoryPoint& trajectory_point) {
                trajectory_point.mutable_path_point()->set_theta(NormalizeAngle(
                        trajectory_point.path_point().theta() + M_PI));
...
bool IterativeAnchoringSmoother::GenerateStopProfileFromPolynomial(
        const double init_acc, const double init_speed,
        const double stop_distance, SpeedData* smoothed_speeds)
{
    ...
    for (double t = 2.0; t <= kMaxT; t += kUnitT)
    {
        ...
        for (double curve_t = 0.0; curve_t <= t; curve_t += kUnitT)
        {
            ...
            smoothed_speeds->AppendSpeedPoint(curve_s, curve_t, curve_v,
                                              curve_a, curve_da);
        }
        return true;
    }
