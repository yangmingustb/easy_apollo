...
public:
    ...
    bool eval_grad_f(int n, const double* x, bool new_x,
     ...
     * nullptr) 2) The values of the hessian of the lagrangian (if "values" is
     * not nullptr)
     */
    ...
    template <class T> bool eval_obj(int n, const T* x, T* obj_value);

    /** Template to compute constraints */
    template <class T> bool eval_constraints(int n, const T* x, int m, T* g);
