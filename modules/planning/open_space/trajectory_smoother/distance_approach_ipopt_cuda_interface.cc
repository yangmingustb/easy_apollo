    ...
    obstacles_b_(obstacles_b)
{
    ...
    weight_stitching_steer_ =
            distance_approach_config_.weight_steer_stitching();
    ...
    max_steer_rate_ = vehicle_param_.max_steer_angle_rate() /
                                                         ...
                                                         double* g_u)
{
    ...
    variable_index += 4;
    ADEBUG << "variable_index after adding state variables : "
           << variable_index;
    ...
    ADEBUG << "constraint_index after adding Euler forward dynamics "
              "constraints: "
           << constraint_index;
    ...
    else
    {
        ...
        for (int i = 0; i < horizon_; ++i)
        {
            ...
            grad_f[control_index + 1] +=
                    2 * weight_input_a_ * x[control_index + 1];
            control_index += 2;
        }
        ...
        double last_time_a_rate = (x[control_index + 1] - last_time_u_(1, 0)) /
                                  x[time_index] / ts_;

        grad_f[control_index] +=
                2.0 * last_time_steer_rate *
                (weight_stitching_steer_ / x[time_index] / ts_);
        grad_f[control_index + 1] +=
                2.0 * last_time_a_rate *
                (weight_stitching_a_ / x[time_index] / ts_);
        grad_f[time_index] +=
                -2.0 *
                (weight_stitching_steer_ * last_time_steer_rate *
                         last_time_steer_rate +
        ...
        for (int i = 0; i < horizon_ - 1; ++i)
        {
            double steering_rate = (x[control_index + 2] - x[control_index]) /
                                   x[time_index] / ts_;
            ...
            double a_rate = (x[control_index + 3] - x[control_index + 1]) /
                            x[time_index] / ts_;
                                                        ...
                                                        double* values)
{
    ...
    if (values == nullptr)
    {
        ...
        // 4. Three obstacles related equal constraints, one equality
        // constraints, [0, horizon_] * [0, obstacles_num_-1] * 4
        state_index = state_start_index_;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(4)
    ...
    else
    {
        ...
        ADEBUG << "After fulfilled control rate constraints derivative, "
                  "nz_index : "
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(4)
        for (int iter = 0; iter < (horizon_ + 1) * obstacles_num_; iter++)
        {
            ...
            // With respect to x
            values[nz_index_tmp] =
                    -std::sin(x[state_index_tmp + 2]) * tmp1 +
            ...
            // With respect to x
            values[nz_index_tmp] =
                    -std::cos(x[state_index_tmp + 2]) * tmp1 -
            ...
            for (int k = 0; k < current_edges_num; ++k)
            {
                values[nz_index_tmp] =
                        (x[state_index_tmp] +
                         std::cos(x[state_index_tmp + 2]) * offset_) *
                                Aj(k, 0) +
                        (x[state_index_tmp + 1] +
                                                        ...
                                                        double* values)
{
    ...
    else
    {
        ...
        for (int i = 0; i < horizon_ + 1; ++i)
        {
            ...
            {
                int current_edges_num = obstacles_edges_num_(j, 0);
                Eigen::MatrixXd Aj = obstacles_A_.block(edges_counter, 0,
                                                        current_edges_num, 2);
                Eigen::MatrixXd bj = obstacles_b_.block(edges_counter, 0,
                                                        current_edges_num, 1);
                ...
                {
                    values[nz_index] =
                            std::cos(x[state_index + 2]) * Aj(k, 0) +
                ...
                {
                    values[nz_index] =
                            -std::sin(x[state_index + 2]) * Aj(k, 0) +
                ...
                for (int k = 0; k < current_edges_num; ++k)
                {
                    values[nz_index] =
                            (x[state_index] +
                             std::cos(x[state_index + 2]) * offset_) *
                                    Aj(k, 0) +
                            (x[state_index + 1] +
                             std::sin(x[state_index + 2]) * offset_) *
                                    Aj(k, 1) -
                            bj(k, 0);  // ddk
                    ++nz_index;
                }
                                                ...
                                                double* values)
{
    ...
    else
    {
        ...
        sparse_hess(tag_L, n, 1, const_cast<double*>(x), &nnz_L, &rind_L,
                    &cind_L, &hessval, options_L);
        ...
        Eigen::MatrixXd* dual_n_result) const
{
    ...
    *dual_n_result = dual_n_result_;
    ...
    for (int i = 0; i < horizon_ + 1; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            state_diff_max = std::max(
                    std::abs(xWS_(j, i) - state_result_(j, i)), state_diff_max);
        }
    }
    ...
    for (int i = 0; i < horizon_; ++i)
    {
        control_diff_max = std::max(
                std::abs(uWS_(0, i) - control_result_(0, i)), control_diff_max);
        control_diff_max = std::max(
                std::abs(uWS_(1, i) - control_result_(1, i)), control_diff_max);
    }
    ...
    for (int i = 0; i < horizon_ + 1; ++i)
    {
        ...
        {
            l_diff_max =
                    std::max(std::abs(l_warm_up_(j, i) - dual_l_result_(j, i)),
                             l_diff_max);
        }
    }
    ...
    for (int i = 0; i < horizon_ + 1; ++i)
    {
        ...
        {
            n_diff_max =
                    std::max(std::abs(n_warm_up_(j, i) - dual_n_result_(j, i)),
                             n_diff_max);
        }
    }
                                                  ...
                                                  T* obj_value)
{
    ...
    *obj_value = 0.0;
    ...
    {
        *obj_value +=
                weight_input_steer_ * x[control_index] * x[control_index] +
    ...
    *obj_value += weight_stitching_steer_ * last_time_steer_rate *
                          last_time_steer_rate +
                  weight_stitching_a_ * last_time_a_rate * last_time_a_rate;
    ...
    for (int i = 0; i < horizon_ - 1; ++i)
    {
        ...
        T a_rate = (x[control_index + 3] - x[control_index + 1]) /
                   x[time_index] / ts_;
                                                          ...
                                                          int m, T* g)
{
    ...
    for (int i = 0; i < horizon_; ++i)
    {
        ...
        // x4
        g[constraint_index + 3] = x[state_index + 7] -
                                  (x[state_index + 3] +
                                   ts_ * x[time_index] * x[control_index + 1]);
    ...
    for (int i = 0; i < horizon_ + 1; ++i)
    {
        ...
        for (int j = 0; j < obstacles_num_; ++j)
        {
            ...
            g[constraint_index + 3] =
                    tmp3 +
                    (x[state_index] + cos(x[state_index + 2]) * offset_) *
                            tmp1 +
                    (x[state_index + 1] + sin(x[state_index + 2]) * offset_) *
                            tmp2 -
                    tmp4;
