        ...
        Eigen::MatrixXd* n_warm_up, Eigen::MatrixXd* s_warm_up)
{
                ...
                .qp_format() == OSQP)
    {
        DualVariableWarmStartOSQPInterface ptop =
                DualVariableWarmStartOSQPInterface(
                        horizon, ts, ego, obstacles_edges_num, obstacles_num,
                        obstacles_A, obstacles_b, xWS,
                     ...
                     .qp_format() == IPOPTQP)
    {
            ...
            status == Ipopt::Solved_To_Acceptable_Level)
        {
            ...
            ADEBUG << "*** The problem solved in " << iter_count
                   << " iterations!";
    ...
    {  // debug mode
            ...
            status == Ipopt::Solved_To_Acceptable_Level)
        {
            ...
            ADEBUG << "*** IPOPTQP: The problem solved in " << iter_count
                   << " iterations!";
            Ipopt::Number final_obj = app->Statistics()->FinalObjective();
            ADEBUG << "*** IPOPTQP: The final value of the objective function "
                      "is "
                   << final_obj << '.';
        }
        else
        ...
        ptop_ipopt->get_optimization_results(&l_warm_up_ipopt,
                                             &n_warm_up_ipopt);
        ...
        for (int c = 0; c < l_warm_up->cols(); ++c)
        {
            ...
            {
                l_max_diff1 =
                        std::max(l_max_diff1, std::abs(l_warm_up->coeff(r, c) -
                                                       l_warm_up_ipopt(r, c)));
                l_max_diff2 = std::max(l_max_diff2,
                                       std::abs(l_warm_up->coeff(r, c) -
                                                l_warm_up_ipoptqp(r, c)));
                l_max_diff3 =
                        std::max(l_max_diff3, std::abs(l_warm_up_ipoptqp(r, c) -
                                                       l_warm_up_ipopt(r, c)));
            }
        }
        ...
        for (int c = 0; c < n_warm_up->cols(); ++c)
        {
            ...
            {
                n_max_diff1 =
                        std::max(n_max_diff1, std::abs(n_warm_up->coeff(r, c) -
                                                       n_warm_up_ipopt(r, c)));
                n_max_diff2 = std::max(n_max_diff2,
                                       std::abs(n_warm_up->coeff(r, c) -
                                                n_warm_up_ipoptqp(r, c)));
                n_max_diff3 =
                        std::max(n_max_diff3, std::abs(n_warm_up_ipoptqp(r, c) -
                                                       n_warm_up_ipopt(r, c)));
            }
        }
