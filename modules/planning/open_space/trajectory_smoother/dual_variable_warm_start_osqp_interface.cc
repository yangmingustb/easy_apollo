    ...
    xWS_(xWS)
{
    ...
    min_safety_distance_ =
            planner_open_space_config.dual_variable_warm_start_config()
                    .min_safety_distance();
    check_mode_ = planner_open_space_config.dual_variable_warm_start_config()
                          .debug_osqp();
    osqp_config_ = planner_open_space_config.dual_variable_warm_start_config()
                           .osqp_config();
}
        ...
        const Eigen::MatrixXd& l_warm_up, const Eigen::MatrixXd& n_warm_up)
{
    ...
    for (int idx = r2_index; idx < r3_index; ++idx)
    {
        ...
        {
            AERROR << "-g' * mu + (A * t - b) * lambda) >= d_min constraint "
                      "fails, "
                   << "constraint_index: " << idx << ", g: " << g(idx, 0);
        }
    }
