    ...
    xWS_(xWS)
{
    ...
    weight_d_ = planner_open_space_config.dual_variable_warm_start_config()
                        .weight_d();
}
        ...
        IndexStyleEnum& index_style)
{
    ...
    num_of_constraints_ =
            4 * obstacles_num_ * (horizon_ + 1) + num_of_variables_;
    ...
    else
    {
        ...
        for (int i = 0; i < horizon_ + 1; ++i)
        {
            ...
            for (int j = 0; j < obstacles_num_; ++j)
            {
                ...
                {
                    values[nz_index] =
                            -(xWS_(0, i) + std::cos(xWS_(2, i)) * offset_) *
                                    Aj(k, 0) -
                            (xWS_(1, i) + std::sin(xWS_(2, i)) * offset_) *
                                    Aj(k, 1) +
                            bj(k, 0);  // ddk
                    ++nz_index;
                }
