            ...
            for (size_t i = path_data.discretized_path().size(); i >= 1; --i)
            {
                ...
                scenario::util::PullOverStatus status =
                        scenario::util::CheckADCPullOverPathPoint(
                                reference_line_info, scenario_config_,
                                path_point, injector_->planning_context());
    ...
    if (path_fail)
    {
            ...
            pull_over_status.position().has_y())
        {
            ...
            planning::util::BuildStopDecision(
                    ...
                    wait_for_obstacle_ids, "PULL-OVER-scenario", frame,
                    &(frame->mutable_reference_line_info()->front()));

            ADEBUG << "Build a stop fence to pause ADC at a better position: "
                      "id["
