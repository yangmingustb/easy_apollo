        ...
        const ReferenceLineInfo::OverlapType& overlap_type)
{
    ...
    {
        // stop_sign_overlap
        const auto& stop_sign_overlaps = reference_line_info.reference_line()
                                                 .map_path()
                                                 .stop_sign_overlaps();
    ...
    else if (overlap_type == ReferenceLineInfo::PNC_JUNCTION)
    {
        ...
        const auto& pnc_junction_overlaps = reference_line_info.reference_line()
                                                    .map_path()
    ...
    {
        // yield_sign_overlap
        const auto& yield_sign_overlaps = reference_line_info.reference_line()
                                                  .map_path()
                                                  .yield_sign_overlaps();
        ...
        const PlanningContext* planning_context)
{
    ...
    const bool position_check = CheckPullOverPositionBySL(
            ...
            target_position, pull_over_status.theta(),
        ...
        Frame* frame, const ScenarioParkAndGoConfig& scenario_config)
{
    ...
    common::math::Vec2d adc_position = {vehicle_status->x(),
                                        vehicle_status->y()};
                     ...
                     const double heading_diff_to_reference_line)
{
    ...
    double angle_difference =
            common::math::NormalizeAngle(adc_heading - path_point.theta());
    if (angle_difference > -1.0 * (heading_diff_to_reference_line -
                                   kReducedHeadingBuffer) &&
