...
#define debug_lane_follow_stage (0)
        ...
        const TrajectoryPoint& planning_start_point, Frame* frame)
{
    ...
    // {
    //     AINFO << "\nNo: [" << count << "] Reference Line.";
    //     AINFO << "\nIsChangeLanePath: "
    //           << reference_line_info.is_change_lane_ref_line() << "\n"
    //           << "is neighbour ref line "
    //           << reference_line_info.IsNeighborLanePath() << ",\tis on "
    //           << reference_line_info.IsDrivable() << "\n";
    //     AINFO << reference_line_info.Lanes().Id();
    // }

    // 原始的apollo:车辆所在车道的reference line默认第一位
    // 更新后： 优先车道的ref line在第一位

    std::list<ReferenceLineInfo>* ref_line_list =
            frame->mutable_reference_line_info();

    for (ReferenceLineInfo& reference_line_info : *ref_line_list)
    {
...
#if debug_lane_follow_stage
        AINFO << "No: [" << count << "] Reference Line.";
        AINFO << "Reference Line size: "
              << frame->mutable_reference_line_info()->size();

        AINFO << "IsChangeLanePath: "
              << reference_line_info.is_change_lane_ref_line()
              << ", is neighbour ref line: "
              << reference_line_info.IsNeighborLanePath()
              << ", is drivable: " << reference_line_info.IsDrivable();

        AINFO << " ref line id " << reference_line_info.Lanes().Id();
        AINFO << " lane id, "
              << reference_line_info.get_start_lane_id().DebugString();
        AINFO << " veh l to ref line: "
              << reference_line_info.get_dist_from_plan_start_to_ref_line();

        AINFO << "has_drivable_reference_line " << has_drivable_reference_line;
        ...
        // 每条ref line计算多条轨迹，在path assessment 选择一个best
        auto cur_status = PlanOnReferenceLine(planning_start_point, frame,
                                              &reference_line_info);
        ...
        if (cur_status.ok())
        {
            ...
            {
                // std::cout << "\nreference line is lane change ref.";
                // std::cout << "\nFLAGS_enable_smarter_lane_change: " <<
                // FLAGS_enable_smarter_lane_change;


                planning::ChangeLaneStatus* lane_change_status =
                ...
                {
                    // If the path and speed optimization succeed on target lane
                    // while under smart lane-change or IsClearToChangeLane
                    // under older version
                    ...
                    AINFO << " lane change target lane id "
                          << reference_line_info.get_start_lane_id()
                                     .DebugString();
                }
                else
                {
                    ...
                    AINFO << "forbiden lane change";
                }
            }
            else
            {
                // std::cout << "\nreference line is NOT lane change ref.";
                has_drivable_reference_line = true;
            }
        }
        else
        ...
        ReferenceLineInfo* reference_line_info)
{
    ...
    // std::cout << "planning start point:" <<
    // planning_start_point.DebugString();
    //   std::cout << "Current reference_line_info is is_change_lane_ref_line: " <<
    //   reference_line_info->is_change_lane_ref_line() << "\n";

    // AINFO << "start s:" << planning_start_point.DebugString();
    ...
    for (auto* task : task_list_)
    {
        ...
        // std::cout << task->Name() << " time spend: " << time_diff_ms
        //           << " ms. \n\n";

        AINFO << "after task: " << task->Name()
              << " time spend: " << time_diff_ms << " ms.";

#if debug_lane_follow_stage
        AINFO << "is_change_lane_ref_line: "
              << reference_line_info->is_change_lane_ref_line() << "\n"
              << "is neighbour ref line "
              << reference_line_info->IsNeighborLanePath() << ", is drivable "
              << reference_line_info->IsDrivable();

        AINFO << reference_line_info->Lanes().Id();
        AINFO << "path max s "
              << reference_line_info->path_data().discretized_path().max_s();
        AINFO << "max t: " << reference_line_info->speed_data().TotalTime()
              << ", max s: " << reference_line_info->speed_data().TotalLength();
        ...
        // ADEBUG << "Current reference_line_info is is_change_lane_ref_line: "
        //        << reference_line_info->is_change_lane_ref_line();
    }
    ...
    if (!ret.ok())
    {
        PlanFallbackTrajectory(planning_start_point, frame,
                               reference_line_info);
    }
         ...
         reference_line_info->path_decision()->obstacles().Items())
    {
        ...
        if (obstacle->LongitudinalDecision().has_stop())
        {
            ...
            {
                SLPoint stop_sl =
                        GetStopSL(obstacle->LongitudinalDecision().stop(),
                                  reference_line_info->reference_line());
    ...
    if (FLAGS_enable_trajectory_check)
    {
        ...
        ConstraintChecker::Result ret = ConstraintChecker::ValidTrajectory(
                trajectory, adc_speed, reference_line_info->trajectory_type());

        if (ret != ConstraintChecker::Result::VALID)
        {
    ...
    // AINFO << "cost " << reference_line_info->Cost();
    // AINFO << "adc sl " << reference_line_info->AdcSlBoundary().DebugString();
    ...
    // AINFO << "traj max s " << reference_line_info->trajectory().GetMaxS();
...
void LaneFollowStage::PlanFallbackTrajectory(
        ...
        ReferenceLineInfo* reference_line_info)
{
    // no path，generate fallback path
    ...
    // 如果只有speed 计算失败，也会使用self lane path
        ...
        PathData* path_data)
{
    ...
    {
        AERROR << "Last frame doesn't succeed, fail to retrieve last frame "
                  "path data";
        return false;
    }
    ...
    if (last_frame_discretized_path.size() < 2)
    {
        AERROR << "history path point is null";
        return false;
    }
...
#if 0
    AINFO << "history path size: " << last_frame_discretized_path.size();
    AINFO << "history cartesian path s: "
          << last_frame_discretized_path.Length();
    AINFO << "history cartesian path end point s: "
          << last_frame_discretized_path.max_s();
#endif
    ...
    // 从traj stitch point开始，沿着history path生成新的path
    ...
    // check history path length, if it is short, return false
    double adc_speed = reference_line_info->get_veh_linear_velocity();

    double k_deceleration = 1.0;

    double stop_dist = adc_speed * adc_speed / k_deceleration * 0.5;

    if (path_data->discretized_path().Length() < stop_dist)
    {
        AERROR << "history path is short";
    ...
    path_data->type_ = last_frame_discretized_path.get_path_boundary_type();


#if 0
    path_data->discretized_path().debug_string();
#endif
    return true;
}
