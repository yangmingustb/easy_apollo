        ...
        ScenarioConfig::ScenarioType scenario_type)
{
    ...
    switch (scenario_type)
    {
        case ScenarioConfig::BARE_INTERSECTION_UNPROTECTED:
            ptr.reset(new scenario::bare_intersection::
                              BareIntersectionUnprotectedScenario(
                                      config_map_[scenario_type],
                                      &scenario_context_, injector_));
            break;
        ...
        case ScenarioConfig::DEADEND_TURNAROUND:
            ptr.reset(
                    new scenario::deadend_turnaround::DeadEndTurnAroundScenario(
                            config_map_[scenario_type], &scenario_context_,
                            injector_));
            break;
        default:
        ...
        const Frame& frame)
{
    ...
    bool pull_over_scenario =
            (frame.reference_line_info().size() ==
                     1 &&  // NO, while changing lane
    ...
    if (pull_over_scenario)
    {
        ...
        for (const auto& overlap : first_encountered_overlap_map_)
        {
                ...
                overlap.first == ReferenceLineInfo::YIELD_SIGN)
            {
                ...
                const double distance_passed =
                        dest_sl.s() - overlap.second.end_s;
                if ((distance_to > 0.0 &&
                     distance_to < kDistanceToAvoidJunction) ||
                    (distance_passed > 0.0 &&
        ...
        while (check_s < dest_sl.s())
        {
                 ...
                 lane->lane().right_neighbor_forward_lane_id())
            {
                ...
                const auto neighbor_lane =
                        hdmap_ptr->GetLaneById(neighbor_lane_id);
        ...
        const Frame& frame)
{
    ...
    else if (pnc_junction_overlap)
    {
        ...
        {
            scenario_type = SelectBareIntersectionScenario(
                    frame, *pnc_junction_overlap);
        }
    }
        ...
        const Frame& frame, const hdmap::PathOverlap& stop_sign_overlap)
{
    ...
    switch (current_scenario_->scenario_type())
    {
        ...
        case ScenarioConfig::PULL_OVER:
            ...
            {
                return stop_sign_all_way
                               ? ScenarioConfig::STOP_SIGN_PROTECTED
                             ...
                             const Frame& frame)
{
    ...
    AINFO << "default secenario: "
          << ScenarioConfig::ScenarioType_Name(default_scenario_type_);

    if (current_scenario_ != nullptr)
    {
        AINFO << "cur secenario: "
              << ScenarioConfig::ScenarioType_Name(
                         current_scenario_->scenario_type());
        AINFO << "cur secenario stage: "
              << ScenarioConfig::StageType_Name(current_scenario_->GetStage());
    }
...
{
    double distance_to_vehicle = (car_position.x() - target_point.x()) *
        ...
        const Frame& frame)
{
    ...
    if (scenario_type == default_scenario_type_)
    {
        ...
        switch (current_scenario_->scenario_type())
        {
            ...
            case ScenarioConfig::VALET_PARKING:
            case ScenarioConfig::DEADEND_TURNAROUND:
                // transfer dead_end to lane follow, should enhance transfer
                // logic
        ...
        const Frame& frame, const ScenarioConfig::ScenarioType& scenario_type)
{
    ...
    const auto map_itr = first_encountered_overlap_map_.find(
        ...
        const Frame& frame, const ScenarioConfig::ScenarioType& scenario_type)
{
    ...
    for (const auto& traffic_light_overlap : traffic_light_overlaps)
    {
        const double dist = traffic_light_overlap.start_s -
                            current_traffic_light_overlap_start_s;
        if (fabs(dist) <= kTrafficLightGroupingMaxDist)
        {
            injector_->planning_context()
                    ->mutable_planning_status()
            ...
            ADEBUG << "Update PlanningContext with first_encountered "
                      "traffic_light["
