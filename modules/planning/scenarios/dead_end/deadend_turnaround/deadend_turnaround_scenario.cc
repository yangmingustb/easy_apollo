...
bool DeadEndTurnAroundScenario::IsTransferable(
        const Frame& frame, const PointENU& dead_end_point,
        const double dead_end_start_range)
{
    ...
    if (!CheckDistanceToDeadEnd(vehicle_state, nearby_path,
                                dead_end_start_range, &junction))
    {
...
bool DeadEndTurnAroundScenario::CheckDistanceToDeadEnd(
        ...
        const double dead_end_start_range, JunctionInfoConstPtr* junction)
{
