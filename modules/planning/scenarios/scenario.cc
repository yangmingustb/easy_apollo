        ...
        const common::TrajectoryPoint& planning_init_point, Frame* frame)
{
...
#if debug_scenario
    std::cout << "\nselect stage: "
              << ScenarioConfig::StageType_Name(current_stage_->stage_type())
              << "\n";
    ...
    switch (ret)
    {
        ...
        case Stage::FINISHED:
        {
            ...
            {
                AINFO << "switch stage from " << current_stage_->Name()
                      << " to " << ScenarioConfig::StageType_Name(next_stage);
                if (next_stage == ScenarioConfig::NO_STAGE)
                {
                    scenario_status_ = STATUS_DONE;
                    return scenario_status_;
                }
                if (stage_config_map_.find(next_stage) ==
