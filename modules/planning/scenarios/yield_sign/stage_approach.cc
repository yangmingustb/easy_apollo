            ...
            scenario_config_.max_valid_stop_distance())
        {
            ...
            for (const auto* obstacle : path_decision.obstacles().Items())
            {
                ...
                double obstacle_traveled_s =
                        obstacle->reference_line_st_boundary()
                                .bottom_left_point()
                                .s() -
                        obstacle->reference_line_st_boundary()
                                .bottom_right_point()
                ...
                // ignore the obstacle which is already on reference line and
                // moving along the direction of ADC max st_min_t(sec) to ignore
...
Stage::StageStatus YieldSignStageApproach::FinishStage()
{
    ...
    for (const auto& yield_sign_overlap_id :
         GetContext()->current_yield_sign_overlap_ids)
    {
        yield_sign_status->add_done_yield_sign_overlap_id(
