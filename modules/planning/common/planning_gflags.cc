...
DEFINE_string(scenario_narrow_street_u_turn_config_file,
              "./../modules/planning/conf/scenario/narrow_street_u_turn_config.pb.txt",
...
DEFINE_string(scenario_emergency_pull_over_config_file,
              "./../modules/planning/conf/scenario/emergency_pull_over_config.pb.txt",
              "The emergency_pull_over scenario configuration file");
DEFINE_string(scenario_emergency_stop_config_file,
              "./../modules/planning/conf/scenario/emergency_stop_config.pb.txt",
...
DEFINE_string(scenario_deadend_turnaround_config_file,
              "./../modules/planning/conf/scenario/deadend_turnaround_config.pb.txt",
              "deadend_turnaround scenario config file");
DEFINE_string(scenario_yield_sign_config_file,
              "./../modules/planning/conf/scenario/yield_sign_config.pb.txt",
...
DEFINE_string(traffic_rule_config_filename, "traffic_rule_config.pb.txt",
              "Traffic rule config filename");

DEFINE_string(smoother_config_filename, "qp_spline_smoother_config.pb.txt",
              "The configuration file for qp_spline smoother");

DEFINE_string(rtk_trajectory_filename, "", "Loop rate for planning node");
...
DEFINE_double(trajectory_time_min_interval, 0.02,
              "(seconds) Trajectory time interval when publish. The is the min "
              "value.");
DEFINE_double(trajectory_time_max_interval, 0.1,
              "(seconds) Trajectory time interval when publish. The is the max "
              "value.");
DEFINE_double(
...
DEFINE_double(longitudinal_jerk_lower_bound, -6.0,
...
DEFINE_double(static_obstacle_nudge_l_buffer, 0.8,
...
DEFINE_bool(enable_speed_lattice_search, true,
              "enable_speed_lattice_search");

DEFINE_bool(enable_lane_borrow_for_blocking, true,
              "enable_lane_borrow_for_blocking");
DEFINE_double(lane_borrow_max_speed, 22.2,
...
// QpSt optimizer
DEFINE_double(slowdown_profile_deceleration, -0.3,
...
DEFINE_double(fallback_total_time, 4.0, "total fallback trajectory time");
DEFINE_double(fallback_time_unit, 0.1,
              "fallback trajectory unit time in seconds");

DEFINE_double(speed_bump_speed_limit, 34.4704,
              "the speed limit when passing a speed bump, m/s. The default "
              "speed limit is 10 mph.");
DEFINE_double(default_city_road_speed_limit, 35.67,
              "default speed limit (m/s) for city road. 35 mph.");
DEFINE_double(default_highway_speed_limit, 39.06,
...
DEFINE_bool(enable_parallel_trajectory_smoothing, false,
            "Whether to partition the trajectory first and do smoothing in "
            "parallel");
...
DEFINE_double(default_front_clear_distance, 300.0,
              "default front clear distance value in case there is no obstacle "
              "around.");
...
DEFINE_bool(use_smoothed_dp_guide_line, false,
            "True to penalize speed optimization result to be close to dp "
            "guide line");
...
DEFINE_string(planning_data_dir, "", "Prefix of files to store feature data");
...
DEFINE_bool(skip_path_reference_in_change_lane, true,
            "skipping using learning model output as path reference in change "
            "lane");
...
DEFINE_bool(debug_st_graph_plot, false, "debug st ");
DEFINE_bool(debug_path, false, "debug path planning ");
