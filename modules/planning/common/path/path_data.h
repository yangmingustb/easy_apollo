...
private:
    ...
    // no collision point, s is reference line
    // path上无碰撞的最远点。no used for now
    common::FrenetFramePoint end_frenet_point_;
    common::PathPoint end_path_point_;

    // path 和 reference line的theta diff
    // point_theta - ref_point_theta;
    // std::vector<double> path_point_delta_theta_;

    /**
     * @brief speed decision generated by path analyzer for guiding speed limit
     * generation in speed bounds decider，记录every path point是否越过车道。if
     * lane borrow, reverse driving, then update this value.
    ...
    // fallback path没有 blocking obs id
    // id in obstacle
     ...
     *
     */
    // wheter this PathData is a path reference serving as an optimization
    // target for later modules
    bool is_valid_path_reference_ = false;

    /**
     * @brief Given a trajectory reference, whether this PathData is optimized
     * according to the "path" part of the trajectory so that "speed" part of
     * the trajectory could be used in later modules accordingly。 Enabled by
     * learning model.
     *
     */
    bool is_optimized_towards_trajectory_reference_ = false;

    // path reference, update by learning base method.
    // fow now, it is null
