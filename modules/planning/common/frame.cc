        ...
        const std::map<std::string, uint32_t> &id_to_priority)
{
    for (const auto &pair : id_to_priority)
    {
        ...
        auto ref_line_info_itr = std::find_if(
                reference_line_info_.begin(), reference_line_info_.end(),
        ...
        const std::string &obstacle_id, const double obstacle_s)
{
    ...
    const double box_center_s =
            obstacle_s + FLAGS_virtual_stop_wall_length / 2.0;
                                                   ...
                                                   const Box2d &box)
{
    ...
    auto *ptr = obstacles_.Add(
            id, *Obstacle::CreateStaticVirtualObstacles(id, box));
    ...
    if (planning_start_point_.v() < 1e-3)
    {
        ...
        if (collision_obstacle != nullptr)
        {
            const std::string msg =
                    absl::StrCat("Found collision with obstacle: ",
...
{
    Obstacle *is_old = Find(obstacle.Id());
    ...
    {
        AERROR << " add same obstacle, id: " << obstacle.PerceptionId();
    }
...
void Frame::ReadTrafficLights()
{
    ...
    {
        AERROR << "traffic signals msg is expired, delay = " << delay
               << " seconds.";
        return;
    }
...
{
    const auto *result = apollo::common::util::FindPtrOrNull(traffic_lights_,
                                                             traffic_light_id);
    if (result == nullptr)
    {
