                             ...
                             perception_obstacle_.width())
{
    is_caution_level_obstacle_ =
            (obstacle_priority == ObstaclePriority::CAUTION);
    ...
    drive_dir_ = agent_drive_direction::UNKNOWN;
}
        ...
        const double relative_time) const
{
    ...
    else
    {
        ...
        auto it_lower = std::lower_bound(points.begin(), points.end(),
                                         relative_time, comp);
...
Polygon2D Obstacle::get_local_polygon() const
{

    Polygon2D local_polygon;

    local_polygon.vertexes[0].x = perception_obstacle_.width() / 2.0;
    local_polygon.vertexes[0].y = perception_obstacle_.length() / 2.0;

    local_polygon.vertexes[1].x = -perception_obstacle_.width() / 2.0;
    local_polygon.vertexes[1].y = perception_obstacle_.length() / 2.0;

    local_polygon.vertexes[2].x = -perception_obstacle_.width() / 2.0;
    local_polygon.vertexes[2].y = -perception_obstacle_.length() / 2.0;

    local_polygon.vertexes[3].x = perception_obstacle_.width() / 2.0;
    local_polygon.vertexes[3].y = -perception_obstacle_.length() / 2.0;
    local_polygon.vertex_num = 4;

    local_polygon.type = POLYGON_RECTANGLE;

    update_polygon_value(
            &local_polygon, NULL, false, false, 100000.0);

    return local_polygon;
}

int Obstacle::get_trajectory_point_polygon(
        ...
        const common::TrajectoryPoint& point) const
{
    ...
    obs_pose.pos.x = point.path_point().x();
    obs_pose.pos.y = point.path_point().y();
    obs_pose.theta = point.path_point().theta();

    cvt_local_polygon_to_global(global_polygon, &local_polygon, &obs_pose);
        ...
        const prediction::PredictionObstacles& predictions)
{
    ...
    for (const auto& prediction_obstacle : predictions.prediction_obstacle())
    {
        ...
        for (const auto& trajectory : prediction_obstacle.trajectory())
        {
            ...
            for (const auto& point : trajectory.trajectory_point())
            {
                if (!IsValidTrajectoryPoint(point))
                {
                    AERROR << "obj:" << perception_id << " TrajectoryPoint: "
                           << trajectory.ShortDebugString() << " is NOT valid.";
            ...
            obstacles.emplace_back(new Obstacle(
                    obstacle_id, prediction_obstacle.perception_obstacle(),
...
bool Obstacle::IsValidTrajectoryPoint(const common::TrajectoryPoint& point)
{
    return !((!point.has_path_point()) || std::isnan(point.path_point().x()) ||
             ...
             std::isnan(point.v()) || std::isnan(point.a()) ||
                                         ...
                                         STBoundary* const st_boundary)
{
    ...
    for (int i = 1; i < trajectory_points.size(); ++i)
    {
        ...
        double object_moving_box_length =
                object_length +
                common::util::DistanceXY(first_point, second_point);
        ...
        const double distance_xy = common::util::DistanceXY(
                trajectory_points[last_index].path_point(),
        ...
        const double st_boundary_delta_s =
                (std::fabs(object_boundary.start_s() - adc_start_s) >
                 kSparseMappingS)
        ...
        const double delta_t = second_traj_point.relative_time() -
                               first_traj_point.relative_time();
        double low_s =
                std::max(object_boundary.start_s() - adc_half_length, 0.0);
        bool has_low = false;
        ...
        if (has_low && has_high)
        {
            ...
            double low_t = (first_traj_point.relative_time() +
                            std::fabs((low_s - object_boundary.start_s()) /
                                      object_s_diff) *
                                    delta_t);
            ...
            double high_t = (first_traj_point.relative_time() +
                             std::fabs((high_s - object_boundary.start_s()) /
                                       object_s_diff) *
                                     delta_t);
    ...
    if (!polygon_points.empty())
    {
        ...
        auto last =
                std::unique(polygon_points.begin(), polygon_points.end(),
                            [](const std::pair<STPoint, STPoint>& a,
                               const std::pair<STPoint, STPoint>& b) {
...
bool Obstacle::IsLongitudinalDecision(const ObjectDecisionType& decision)
{
    return decision.has_ignore() || decision.has_stop() ||
           decision.has_yield() || decision.has_follow() ||
        ...
        const ObjectDecisionType& lhs, const ObjectDecisionType& rhs)
{
    ...
    else
    {
        ...
        {
            return lhs.stop().distance_s() < rhs.stop().distance_s() ? lhs
                                                                     : rhs;
        }
        else if (lhs.has_yield())
        {
            return lhs.yield().distance_s() < rhs.yield().distance_s() ? lhs
                                                                       : rhs;
        }
        else if (lhs.has_follow())
        {
            return lhs.follow().distance_s() < rhs.follow().distance_s() ? lhs
                                                                         : rhs;
        }
        else if (lhs.has_overtake())
        {
            return lhs.overtake().distance_s() > rhs.overtake().distance_s()
                           ? lhs
                           : rhs;
        }
        else
...
int cvt_box2d_to_polygon(Polygon2D *poly, const common::math::Box2d &box)
{
    ...
    int point_size = corners.size();

    if (point_size != 4)
    {
        AERROR << "box is not 4 points";
        return 0;
    }

    for (int i = 0; i < corners.size(); i++)
    {
        poly->vertexes[i].x = corners[i].x();
        poly->vertexes[i].y = corners[i].y();
    }

    poly->vertex_num  = 4;
    poly->type = POLYGON_RECTANGLE;

    update_polygon_value(poly, NULL, false, false,
                         100000);
