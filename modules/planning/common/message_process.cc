...
void MessageProcess::Close()
{
    ...
    if (FLAGS_planning_offline_learning)
    {
        ...
        const std::string msg =
                absl::StrCat("Total learning_data_frame number: ",
...
{
    chassis_feature_.set_message_timestamp_sec(
...
void MessageProcess::OnLocalization(const LocalizationEstimate& le)
{
    ...
    const double time_diff = le.header().timestamp_sec() -
                             last_localization_message_timestamp_sec_;
    ...
    if (GenerateLearningDataFrame(&learning_data_frame))
    {
        ...
        {
            // offline
            FeatureOutput::InsertLearningDataFrame(record_file_,
                                                   learning_data_frame);
        }
        else
        {
    ...
    for (const auto& m : prediction_obstacles_map_)
    {
        ...
        else
        {
            ...
            const std::string msg = absl::StrCat(
                    ...
                    obstacle_history_map_[m.first].back().timestamp_sec(),
                    "] timestamp_sec[",
                    obstacle_trajectory_point.timestamp_sec(), "] time_diff[",
                    time_diff, "]");
            AERROR << msg;
            if (FLAGS_planning_offline_learning)
            {
        ...
        while (!obstacle_history.empty())
        {
            const double time_distance =
                    obstacle_history.back().timestamp_sec() -
        ...
        for (int i = 0; i < routing_response_.road_size(); ++i)
        {
            *adc_passage_s = 0;
            for (int j = 0; j < routing_response_.road(i).passage_size(); ++j)
            {
                double passage_s = 0;
                for (int k = 0;
                     k < routing_response_.road(i).passage(j).segment_size();
                     ++k)
                {
                    const auto& segment =
...
int MessageProcess::GetADCCurrentInfo(ADCCurrentInfo* adc_curr_info)
{
    ...
    adc_curr_info->adc_cur_position_ = std::make_pair(
            adc_cur_pose.position().x(), adc_cur_pose.position().y());
    ...
    for (const auto& obj_traj_point : obstacle_history)
    {
        ...
        for (int i = 0; i < obj_traj_point.polygon_point_size(); ++i)
        {
            ...
            const auto& relative_point = util::WorldCoordToObjCoord(
                    ...
                    adc_curr_info.adc_cur_heading_);
            auto polygon_point =
                    perception_obstacle_history->add_polygon_point();
        ...
        const ADCCurrentInfo& adc_curr_info, ObstacleFeature* obstacle_feature)
{
    ...
    for (int i = 0; i < prediction_obstacle.trajectory_size(); ++i)
    {
        ...
        for (int j = 0; j < obstacle_trajectory.trajectory_point_size(); ++j)
        {
            ...
            if (trajectory->trajectory_point_size() > 0)
            {
                const auto last_relative_time =
                        trajectory
                                ...
                                .trajectory_point()
                                .relative_time();
                if (obstacle_trajectory_point.relative_time() <
                    last_relative_time)
                {
                    const std::string msg = absl::StrCat(
                            "DISCARD prediction trajectory point: frame_num[",
            ...
            path_point->set_lane_id(
                    obstacle_trajectory_point.path_point().lane_id());
        ...
        std::vector<std::string>* local_routing_lane_ids)
{
    ...
    {
        const std::string msg =
                absl::StrCat("DISCARD: invalid routing_response. frame_num[",
                             frame_num, "]");
        AERROR << msg;
        if (FLAGS_planning_offline_learning)
        {
    ...
    for (int i = 0; i < routing_response_.road_size(); ++i)
    {
        ADEBUG << "road_id[" << routing_response_.road(i).id()
               << "] passage_size[" << routing_response_.road(i).passage_size()
               << "]";
    ...
    */
        ...
        else
        {
            ...
            local_routing_end_road_s =
                    (i == adc_road_index ? adc_passage_s + forwardward_length
                                         : forwardward_length);
            ADEBUG << "local_routing_end_road_index["
                   << local_routing_end_road_index
    ...
    ADEBUG << "local_routing: start_road_index["
           << local_routing_start_road_index << "] start_road_s["
           << local_routing_start_road_s << "] end_road_index["
           << local_routing_end_road_index << "] end_road_s["
           << local_routing_end_road_s << "]";
         ...
         i <= local_routing_end_road_index; ++i)
    {
        ...
        for (int j = 0; j < road.passage_size(); ++j)
        {
            ...
            local_routing_passage->set_change_lane_type(
            ...
            for (int k = 0; k < passage.segment_size(); ++k)
            {
                ...
                ADEBUG << "ADD road[" << i << "] id[" << road.id()
                       << "] passage[" << j << "] id[" << lane_segment.id()
                       << "] length["
                        ...
                        if (i < adc_road_index)
                        {
                            ...
                            if (j == adc_passage_index ||
                                (j == road.passage_size() - 1 &&
                                 j < adc_passage_index))
                            {
                                ADEBUG << "ADD local_routing_lane_ids: road["
                                       << i << "] passage[" << j
                                       << "] adc_passage_index["
                                       << adc_passage_index << "] passage_size["
                                       << road.passage_size()
                                       << "]: " << lane_segment.id();
                                local_routing_lane_ids->push_back(
                                        lane_segment.id());
                            }
                        }
                        else
    ...
    if (FLAGS_planning_offline_learning)
    {
        for (size_t i = 0; i < local_routing_lane_ids->size(); ++i)
        {
            ...
            const auto& lane = hdmap::HDMapUtil::BaseMap().GetLaneById(
                    hdmap::MakeMapId(lane_id));
            if (lane == nullptr)
            {
                const std::string msg =
                        absl::StrCat("DISCARD: fail to find local_routing_lane "
                                     "on map. frame_num[",
        ...
        LearningDataFrame* learning_data_frame)
{
    ...
    const int frame_num = learning_data_frame->frame_num();
    const int local_routing_lane_id_size =
    ...
    {
        const std::string msg =
                absl::StrCat("LARGE local_routing. frame_num[", frame_num,
                             "] local_routing_lane_id_size[",
                             local_routing_lane_id_size, "]");
        AERROR << msg;
        if (FLAGS_planning_offline_learning)
        {
        ...
        LearningDataFrame* learning_data_frame)
{
    ...
    for (const auto& localization_sample : localization_samples)
    {
        ...
        const double a = std::sqrt(pose.linear_acceleration().x() *
        ...
        if (FLAGS_planning_offline_learning)
        {
            ...
            if (trajectory_point_index > 0)
            {
                auto& next_point =
                        adc_trajectory_points[trajectory_point_index - 1]
                                ...
                                .path_point();
                point_distance =
                        common::util::DistanceXY(next_point, cur_point);
            }
            ...
            if (!clear_area_id.empty())
            {
                ...
                planning_tag->mutable_clear_area()->set_distance(
                        clear_area_distance);
            }
            ...
            if (!crosswalk_id.empty())
            {
                ...
                planning_tag->mutable_crosswalk()->set_distance(
                        crosswalk_distance);
            }
            ...
            if (!stop_sign_id.empty())
            {
                ...
                planning_tag->mutable_stop_sign()->set_distance(
                        stop_sign_distance);
            }
            ...
            if (!yield_sign_id.empty())
            {
                ...
                planning_tag->mutable_yield_sign()->set_distance(
                        yield_sign_distance);
            }
        }
...
void MessageProcess::GeneratePlanningTag(LearningDataFrame* learning_data_frame)
{
