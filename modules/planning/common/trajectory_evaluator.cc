    ...
    for (const auto& tp : updated_trajectory)
    {
        ...
        else
        {
            const std::string msg = absl::StrCat(
                    "DISCARD trajectory point: frame_num[", frame_num,
                    "] obstacle_id[", obstacle_id, "] last_relative_time[",
                    last_relative_time, "] relatice_time[", tp.relative_time(),
                    "] relative_time_diff[",
        ...
        std::vector<TrajectoryPointFeature>* evaluated_adc_future_trajectory)
{
    ...
    if (trajectory.size() <= 1)
    {
        const std::string msg =
                absl::StrCat("too short adc_future_trajectory. frame_num[",
                             frame_num, "] size[", trajectory.size(), "]");
        WriteLog(msg);
        return;
    }
    ...
    {
        const double time_range = evaluated_trajectory.back().timestamp_sec() -
                                  start_point_timestamp_sec;
        if (time_range < FLAGS_trajectory_time_length)
        {
            const std::string msg = absl::StrCat(
                    "WARNING: adc_future_trajectory not long enough. ",
                    "frame_num[", frame_num, "] size[",
                    evaluated_trajectory.size(), "] time_range[", time_range,
                    "]");
            WriteLog(msg);
        }
    }
    ...
    for (const auto& tp : evaluated_trajectory)
    {
        ...
        {
            const std::string msg = absl::StrCat(
                    "DISCARD adc_future_trajectory_point. frame_num[",
        ...
        LearningDataFrame* learning_data_frame)
{
    for (int i = 0; i < learning_data_frame->obstacle_size(); ++i)
    {
             ...
             obstacle_trajectory.perception_obstacle_history())
        {
            ...
            const double a =
                    std::sqrt(perception_obstacle.acceleration().x() *
            ...
            trajectory.push_back(std::make_pair(
                    perception_obstacle.timestamp_sec(), trajectory_point));
        }
        ...
        LearningDataFrame* learning_data_frame)
{
    for (int i = 0; i < learning_data_frame->obstacle_size(); ++i)
    {
        ...
        for (int j = 0; j < obstacle_prediction.trajectory_size(); ++j)
        {
            ...
            std::vector<std::pair<double, CommonTrajectoryPointFeature>>
                    trajectory;
                 ...
                 obstacle_prediction_trajectory.trajectory_point())
            {
                ...
                trajectory.push_back(std::make_pair(
                        timestamp_sec, trajectory_point.trajectory_point()));
            }
            ...
            ADEBUG << "frame_num[" << learning_data_frame->frame_num()
                   << "] obstacle_id[" << obstacle_id
                   << "orig obstacle_prediction_trajectory["
                   << trajectory.size() << "] evaluated_trajectory_size["
                   << evaluated_trajectory.size() << "]";
