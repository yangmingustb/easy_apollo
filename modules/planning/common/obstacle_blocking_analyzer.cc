                          ...
                          const Obstacle& obstacle)
{
    ...
    double adc_v = reference_line_info.get_veh_linear_velocity();
    double k_deceleration = 2.0;

    // 最好在停车距离内完成side pass
    double adc_stop_dist = adc_v * adc_v / 2.0 / k_deceleration;

    double side_pass_lon_dist = std::max(kAdcDistanceThreshold, adc_stop_dist);

    if (obstacle.PerceptionSLBoundary().start_s() >
        adc_sl_boundary.end_s() + side_pass_lon_dist)
    {
        ADEBUG << " - It is too far ahead and we are not so sure of its "
                  "status.";
        return false;
    }
                     ...
                     const Obstacle* obstacle)
{
    ...
    bool is_at_road_edge =
            std::abs(obstacle->PerceptionSLBoundary().start_l()) >
