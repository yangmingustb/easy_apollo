...
hdmap::LaneInfoConstPtr ReferenceLineInfo::LocateLaneInfo(const double s) const
{
...
bool ReferenceLineInfo::has_neighbour_lane(adc_direction direction,
                                           const double s) const
{
    ...
    if(direction == adc_direction::LEFT)
    {
        do
        {
            if (!ptr_lane_info->lane().left_neighbor_forward_lane_id().empty())
    ...
    double ptr_lane_width = ptr_neighbor_lane->GetWidth(neighbor_s);

    if (ptr_lane_width < 1.0)
    {
        return false;
    }
...
double ReferenceLineInfo::get_neighbour_lane_width(adc_direction direction,
                                           const double s) const
{
    auto ptr_lane_info = LocateLaneInfo(s);
    if (ptr_lane_info == nullptr)
...
void ReferenceLineInfo::InitFirstOverlaps()
{
    ...
    {
        first_encounter_overlaps_.emplace_back(PNC_JUNCTION,
                                               pnc_junction_overlap);
    }
...
Obstacle* ReferenceLineInfo::AddObstacle(const Obstacle* obstacle)
{
...
#if debug_ref_line_info
    if (mutable_obstacle->IsLaneBlocking())
    {
        AINFO << "obstacle [" << obstacle->Id() << "] is lane blocking.";
    }
    else
    {
        AINFO << "obstacle [" << obstacle->Id() << "] is NOT lane blocking.";
    }
#endif

    if (is_irrelevant_obstacle(*mutable_obstacle))
    {
    ...
    else
    {
        ...
        AINFO << "reference line st boundary: t["
        ...
        const std::vector<const Obstacle*>& obstacles)
{
    if (FLAGS_use_multi_thread_to_add_obstacles)
    {
        ...
        {
            results.push_back(cyber::Async(&ReferenceLineInfo::AddObstacle,
                                           this, obstacle));
        }
...
bool ReferenceLineInfo::is_irrelevant_obstacle(const Obstacle& obstacle)
{
    ...
    bool is_backside_obs = false;
    bool is_backside_obs_ignore = false;

    if (obstacle_boundary.end_s() < adc_sl_boundary_.start_s())
    {
        is_backside_obs = true;

        is_backside_obs_ignore = is_ignore_backside_obstacle(
                is_change_lane_ref_line(), is_on_reference_line_, obstacle,
                adc_sl_boundary_, reference_line_);

        if (is_backside_obs_ignore)
        {
            return true;
        }
    }
...
SpeedData* ReferenceLineInfo::mutable_emergency_brake_speed_data()
{
...
#if debug_ref_line_info
    path_data_.discretized_path().debug_string();

    speed_data_.log_speed_data();
        ...
        DiscretizedTrajectory* adjusted_trajectory)
{
    ...
    const double start_point_relative_time =
    ...
    for (size_t i = 0; i < trajectory.size(); ++i)
    {
        ...
        const double tracking_heading = std::atan2(cur_point_y - start_point_y,
                                                   cur_point_x - start_point_x);
        if (std::fabs(common::math::AngleDiff(
                    start_point_heading, tracking_heading)) < kMaxAngleDiff)
        {
            insert_idx = i;
            break;
        }
    ...
    for (size_t i = 1; i < cut_trajectory.size(); ++i)
    {
        ...
        accumulated_s +=
                std::sqrt((cur_path_point->x() - pre_path_point.x()) *
...
bool ReferenceLineInfo::is_change_lane_ref_line() const
{
        ...
        common::VehicleSignal* vehicle_signal) const
{
    ...
    for (const auto& seg : Lanes())
    {
        ...
        else if (turn == hdmap::Lane::U_TURN)
        {
            ...
            auto start_xy = PointFactory::ToVec2d(
                    seg.lane->GetSmoothPoint(seg.start_s));
            auto middle_xy = PointFactory::ToVec2d(
                    seg.lane->GetSmoothPoint((seg.start_s + seg.end_s) / 2.0));
            auto end_xy =
                    PointFactory::ToVec2d(seg.lane->GetSmoothPoint(seg.end_s));
                                     ...
                                     PlanningContext* planning_context) const
{
    ...
    // 检查是否到达目的地，这里有bug。判断的不准确，如环岛路，uturn路，cycle路
    ...
    // lane borrow state
    planning::LaneBorrowStatus* lane_borrow =
            decision_result->mutable_lane_borrow_decider();

    lane_borrow->CopyFrom(
            planning_context->planning_status().lane_borrow_decider());

    // lane change
    planning::ChangeLaneStatus* lane_change =
            decision_result->mutable_change_lane();

    lane_change->CopyFrom(
            planning_context->planning_status().change_lane());
        ...
        PlanningContext* planning_context) const
{
    ...
    // todo
    auto mission_complete = decision_result->mutable_main_decision()
    ...
    {
        mission_complete->mutable_stop_point()->CopyFrom(
                main_stop.stop_point());
        mission_complete->set_stop_heading(main_stop.stop_heading());


        // AINFO << "not reach destination";
    }
}
        ...
        DecisionResult* decision_result) const
{
    ...
    for (const auto* obstacle : path_decision_.obstacles().Items())
    {
        ...
        apollo::common::PointENU stop_point =
                object_decision.stop().stop_point();
    ...
    if (stop_obstacle != nullptr)
    {
        ...
        ADEBUG << " main stop obstacle id:" << stop_obstacle->Id()
               ...
               << stop_decision->stop_point().x()
               << stop_decision->stop_point().y()
...
void ReferenceLineInfo::set_close_following_obstacle(
        const std::string& obstacle_id)
{
    close_following_obstacle_ = path_decision_.Find(obstacle_id);
}

void ReferenceLineInfo::set_close_stop_obstacle(
        const std::string& obstacle_id)
{
    close_stop_obstacle_ = path_decision_.Find(obstacle_id);
}

void ReferenceLineInfo::clear_following_obstacle()
{
    close_following_obstacle_ = nullptr;
}

void ReferenceLineInfo::clear_stop_obstacle()
{
    close_stop_obstacle_ = nullptr;
}
