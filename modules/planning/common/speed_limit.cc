...
int SpeedLimit::get_index_by_s(double s)
{
    ...
    if (s > speed_limit_points_.end()->first)
    {
    ...
    for (size_t i = 0; i < speed_limit_points_.size() - 1; i++)
    {
        if (s >= speed_limit_points_[i].first &&
            s <= speed_limit_points_[i + 1].first)
        {
            return i;
        }
    }
