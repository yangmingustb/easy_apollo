...
#define debug_on_lane_planning_time (1)
                                 ...
                                 const VehicleState& vehicle_state)
{
    ...
    // ref line的顺序是route优先,还是车辆位置优先？车辆位置优先。
    // ref line的数量和车道数量无关，而和route有关系，当route一直是直行，那么ref
    // line的数量就一直是1.
    ...
    for (auto& seg : segments)
    {
     ...
     auto status =
             frame_->Init(injector_->vehicle_state(), reference_lines, segments,
    ...
    // reset planning internal state

    if (local_view_.chassis->driving_mode() == canbus::Chassis::COMPLETE_MANUAL)
    {
        auto* mutable_path_decider_status = injector_->planning_context()
                                                ->mutable_planning_status()
                                                ->mutable_lane_borrow_decider();

        mutable_path_decider_status->Clear();
    }
                                   ...
                                   const VehicleState& vehicle_state)
{
    ...
    {
        // std::cout << "\nFail to get junctions from base_map.\n";
        return false;
    }
    if (junctions.size() <= 0)
    {
        // std::cout << "\nNo junction from map\n";
        return false;
    }
    Vec2d car_position;
                             ...
                             ADCTrajectory* const ptr_trajectory_pb)
{
    ...
    // AINFO << local_view_.localization_estimate->DebugString();

    // chassis
    // std::cout << "Get chassis:" << local_view_.chassis->DebugString();
    ...
    // std::cout << vehicle_state.DebugString();
    ...
    if (util::IsDifferentRouting(last_routing_, *local_view_.routing))
    {
        ...
        // AINFO << "last_routing_:" << last_routing_.ShortDebugString();
    ...
    // plan_start.set_x(stitching_trajectory.back().path_point().x());
    // plan_start.set_y(stitching_trajectory.back().path_point().y());

    // double kappa = stitching_trajectory.back().path_point().kappa();


    // const double l = common::VehicleConfigHelper::GetConfig()
    //                          .vehicle_param()
    //                          .wheel_base();

    // double angle = std::atan(kappa * l) * 180.0 / M_PI * 15.7;

    // AINFO << "print_start_angle:"
    //       << "(" << angle << ","
    //       << ")";

    // veh_position.set_x(vehicle_state.x());
    // veh_position.set_y(vehicle_state.y());

    // double dist = plan_start.DistanceTo(veh_position);

    // AINFO << "start dist " << dist;

    // AINFO << local_view_.localization_estimate->DebugString();
    // AINFO << vehicle_state.DebugString();
    // AINFO << stitching_trajectory.back().path_point().DebugString();
    // AINFO << "traj point size " << stitching_trajectory.size();
    // AINFO << stitching_trajectory.back().DebugString();
    ...
    // AINFO << "ref line list size " << frame_->reference_line_info().size();
    // for (auto& ref_line_info : frame_->reference_line_info())
    // {
    //     auto& ref_line = ref_line_info.reference_line();

    //     AINFO << "ref line len:" << ref_line.Length()
    //           << " m, points size: " << ref_line.reference_points().size();
    // }
    ...
    if (!status.ok())
    {
        ...
        {
            // "estop" signal check in function
            // "Control::ProduceControlCommand()" estop_ = estop_ ||
            // local_view_.trajectory.estop().is_estop(); we should add more
            // information to ensure the estop being triggered.
    ...
    for (auto& ref_line_info : *frame_->mutable_reference_line_info())
    {
        ...
        auto traffic_status = traffic_decider.Execute(
                frame_.get(), &ref_line_info, injector_);

        if (!traffic_status.ok() || !ref_line_info.IsDrivable())
        {
            ref_line_info.SetDrivable(false);

            AERROR << "Reference line " << ref_line_info.Lanes().Id()
                   << " traffic decider failed";
        }
    }

    const auto pre_process_time_end =
            std::chrono::duration<double>(
                    std::chrono::system_clock::now().time_since_epoch())
                    .count();

    auto planning_pre_process_diff =
            pre_process_time_end - start_system_timestamp;

    AINFO << "print_planning_pre_process_time:"
    ...
    // std::cout << "Planning latency: " <<
    // ptr_trajectory_pb->latency_stats().DebugString();

    if (!status.ok())
    {
        status.Save(ptr_trajectory_pb->mutable_header()->mutable_status());
        // std::cout << "Planning failed:" << status.ToString();
        if (FLAGS_publish_estop)
        {
            std::cout << "Planning failed and set estop";
            // "estop" signal check in function
            // "Control::ProduceControlCommand()" estop_ = estop_ ||
            // local_view_.trajectory.estop().is_estop(); we should add more
            // information to ensure the estop being triggered.
    ...
    if (ptr_trajectory_pb->is_replan())
    {
        ...
        AINFO << "replan reason: " << replan_reason;
    }

    if (frame_->open_space_info().is_on_open_space_trajectory())
    {
        FillPlanningPb(start_timestamp, ptr_trajectory_pb);
        std::cout << "Planning pb:"
                  << ptr_trajectory_pb->header().DebugString();
        frame_->set_current_frame_planned_trajectory(*ptr_trajectory_pb);
    }
    else
    {
...
void OnLanePlanning::ExportReferenceLineDebug(planning_internal::Debug* debug)
{
    ...
    for (auto& reference_line_info : *frame_->mutable_reference_line_info())
    {
        ...
        rl_debug->set_is_change_lane_path(
        ...
        for (const auto& reference_point : reference_points)
        {
            ...
            dkappa_max_abs =
                    dkappa_max_abs < dkappa_sq ? dkappa_sq : dkappa_max_abs;
        }
        double reference_points_size =
        ...
        const double adc_half_width = common::VehicleConfigHelper::GetConfig()
                                              .vehicle_param()
                                              .width() /
                                      2.0;
        ...
        ADCTrajectory* const ptr_trajectory_pb)
{
    ...
    else
    {
...
#if 0
        AINFO << "debug stitch traj";
        current_frame_planned_path.debug_string();
#endif

        // 如果速度规划失败，path是会被history path覆盖的
        const auto& best_ref_path =
                best_ref_info->path_data().discretized_path();

        std::copy(best_ref_path.begin() + 1, best_ref_path.end(),
                  std::back_inserter(current_frame_planned_path));

#if 0
        best_ref_path.debug_string();
#endif

#if 0
        AINFO << "path size: " << best_ref_path.size();
        AINFO << "cartesian path s: " << best_ref_path.Length();
        AINFO << "cartesian path end point s: " << best_ref_path.max_s();

        AINFO << "full path size: " << current_frame_planned_path.size();
        AINFO << "full cartesian path s: "
              << current_frame_planned_path.Length();
        AINFO << "full cartesian path end point s: "
              << current_frame_planned_path.max_s();
#endif

        current_frame_planned_path.set_path_boundary_type(
                best_ref_info->path_data().type_);
        ...
        else
        {
            ...
            if (failed_ref_info)
            {
                ExportFailedLaneChangeSTChart(failed_ref_info->debug(),
                                              ptr_debug);
            }
        }
        ...
        planning::SpeedDecision* speed_decision =
                ptr_trajectory_pb->mutable_speed_decision();

        *speed_decision = best_ref_info->get_speed_decision();
        ...
        ptr_trajectory_pb->set_path_type(
                current_frame_planned_path.get_path_boundary_type());
        ...
        best_ref_info->publish_ref_line_to_cyber(ptr_debug);
...
#if 0
        for (size_t i = 0; i < ptr_trajectory_pb->trajectory_point_size(); i++)
        {
            const apollo::common::TrajectoryPoint& point =
                    ptr_trajectory_pb->trajectory_point(i);
        ...
        // lateral path
        ptr_trajectory_pb->clear_path_point();
        auto* traj_path_points = ptr_trajectory_pb->mutable_path_point();

        for (auto& path_point : current_frame_planned_path)
        {
            auto* traj_point = traj_path_points->Add();

            traj_point->CopyFrom(path_point);
        }

#if debug_on_lane_planning
        AINFO << best_ref_info->debug().planning_data().st_graph_size();
        for (size_t i = 0;
             i < best_ref_info->debug().planning_data().st_graph_size(); i++)
        {
            AINFO << best_ref_info->debug().planning_data().st_graph(i).name();
        }

        for (size_t i = 0;
             i < best_ref_info->debug().planning_data().speed_plan_size(); i++)
        {
            AINFO << best_ref_info->debug().planning_data().speed_plan(i).name();
        }
...
{
    // TODO(Jinyun): use the same method in trajectory stitching for forward
    // prediction
