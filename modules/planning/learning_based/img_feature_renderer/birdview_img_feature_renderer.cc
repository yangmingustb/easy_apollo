...
bool BirdviewImgFeatureRenderer::Init(const PlanningSemanticMapConfig& config)
{
    ...
    // a redundant call to HDMapUtil::BaseMap() to save time for renderering
    // when the basemap is not yet initialized in HDMapUtil
        ...
        const LearningDataFrame& learning_data_frame, cv::Mat* img_feature)
{
    ...
    cv::Mat obs_future =
            cv::Mat(config_.height(), config_.width(), CV_8UC1, cv::Scalar(0));
    cv::Mat road_map = cv::Mat(config_.height(), config_.width(), CV_8UC3,
                               cv::Scalar(0, 0, 0));
    cv::Mat routing =
            cv::Mat(config_.height(), config_.width(), CV_8UC1, cv::Scalar(0));
    cv::Mat speed_limit = cv::Mat(config_.height(), config_.width(), CV_8UC3,
                                  cv::Scalar(0, 0, 0));
    ...
    if (!RenderRouting(learning_data_frame, current_x, current_y,
                       current_heading, &routing))
    {
        ...
        const LearningDataFrame& learning_data_frame, cv::Mat* img_feature)
{
    ...
    cv::Mat bgr_canvas = cv::Mat(config_.height(), config_.width(), CV_8UC3,
                                 cv::Scalar(0, 0, 0));
...
bool BirdviewImgFeatureRenderer::LoadSpeedlimitMap(const std::string& map_file)
{
        ...
        const cv::Scalar& bgr_color)
{
    ...
    (*img_feature).at<uchar>(config_.ego_idx_y(), config_.ego_idx_x()) =
            color[0];
    return true;
}
        ...
        const cv::Scalar& gray_scale, const cv::Scalar& bgr_color)
{
    ...
    {
        if (obstacle.obstacle_trajectory().evaluated_trajectory_point_size() ==
            0)
        {
            continue;
        }
        ...
        for (const auto& traj_point : past_traj)
        {
            ...
            std::vector<cv::Point2i> box_corner_points = GetAffinedBoxImgIdx(
                    ...
                    0.0, 0.0, M_PI_2);
            cv::fillPoly(*img_feature,
                         std::vector<std::vector<cv::Point>>(
                                 {std::move(box_corner_points)}),
                         color);
        }
    }
    ...
    for (const auto& obstacle : learning_data_frame.obstacle())
    {
        ...
        if (obstacle.obstacle_prediction().is_static())
        {
            const auto& past_traj_points_size =
                    obstacle.obstacle_trajectory()
                            .evaluated_trajectory_point_size();
        ...
        for (int i = 0; i < predicted_traj_size; ++i)
        {
            const auto& trajectory =
                    obstacle.obstacle_prediction().trajectory(i);
        ...
        const cv::Scalar& gray_scale, const cv::Scalar& bgr_color)
{
         ...
         learning_data_frame.traffic_light_detection().traffic_light())
    {
        ...
        {
            AERROR << "traffic_light [" << traffic_light_status.id()
                   << "] not found";
            return false;
        }
        ...
        {
            const auto& overlap =
                    apollo::hdmap::HDMapUtil::BaseMap().GetOverlapById(
                            overlap_id);
            if (overlap == nullptr)
            {
            ...
            for (const auto& overlap_object : overlap->overlap().object())
            {
                ...
                const auto& lane =
                        apollo::hdmap::HDMapUtil::BaseMap().GetLaneById(
                                overlap_object.id());
                if (lane == nullptr)
                {
                    AERROR << "lane [" << overlap_object.id().id()
                           << "] not found";
                    return false;
                }
                for (const auto& segment :
                     lane->lane().central_curve().segment())
                {
                    const int segment_point_size =
                            segment.line_segment().point_size();
        ...
        const cv::Scalar& gray_scale, const cv::Scalar& bgr_color)
{
    ...
    for (int i = 0; i < routing_lanes_size; ++i)
    {
        ...
        const auto& lane = apollo::hdmap::HDMapUtil::BaseMap().GetLaneById(
                hdmap::MakeMapId(
                        learning_data_frame.routing().local_routing_lane_id(
                                i)));
        if (lane == nullptr)
        {
            AERROR << "lane ["
                                            ...
                                            cv::Mat* img_feature)
{
    ...
    cv::Point2i ego_img_idx = GetPointImgIdx(ego_x - map_bottom_left_point_x_,
                                             ego_y - map_bottom_left_point_y_,
                                             0, base_roadmap_img_.size[0]);
    ...
    const int rough_radius =
            static_cast<int>(sqrt(config_.height() * config_.height() +
                                  config_.width() * config_.width()));
