...
bool TrajectoryImitationLibtorchInference::LoadCNNLSTMModel()
{
    ...
    {
        AERROR << "Fail to do initial inference on "
                  "HISTORY_UNCONDITIONED_CNN_LSTM "
                  "Model";
        return false;
    }
    return true;
        ...
        LearningDataFrame* const learning_data_frame)
{
    ...
    ADEBUG << "cur_x[" << cur_x << "], cur_y[" << cur_y << "], cur_heading["
           << cur_heading << "], cur_v["
           << cur_traj_point.trajectory_point().v() << "]";
    ...
    for (int i = 0; i < torch_output_tensor.size(1); ++i)
    {
        ...
        ADEBUG << "dx[" << dx << "], dy[" << dy << "], dtheta[" << dtheta
               << "], v[" << v << "]";
        ...
        LearningDataFrame* const learning_data_frame)
{
    ...
    {
        input_feature_tensor[0][i] =
                input_feature_tensor[0][i].sub(0.5).div(0.5);
    }
