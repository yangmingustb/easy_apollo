...
#define debug_ref_line_priority (0)
    ...
    vehicle_state_provider_(vehicle_state_provider)
{
    ...
    {
        smoother_.reset(
                new DiscretePointsReferenceLineSmoother(smoother_config_));
    }
    else
    {
    ...
    // std::cout<< smoother_config_.DebugString();
    return;
}
        ...
        const std::list<hdmap::RouteSegments> &route_segments)
{
    if (reference_lines.size() != route_segments.size())
    {
        AERROR << "The calculated reference line size("
               << reference_lines.size() << ") and route_segments size("
    ...
    else
    {
             ...
             ++iter, ++segment_iter, ++internal_iter, ++internal_segment_iter)
        {
            ...
            if (common::util::SamePointXY(
                ...
                common::util::SamePointXY(
                        iter->reference_points().back(),
        ...
        std::list<hdmap::RouteSegments> *segments)
{
    ...
    if (FLAGS_use_navigation_mode)
    {
        ...
        bool result =
                GetReferenceLinesFromRelativeMap(reference_lines, segments);
        if (!result)
        {
    ...
    {
        std::cout << "Failed to use reference line latest history";
        return false;
    }
        ...
        std::list<hdmap::RouteSegments> *route_segments)
{
    ...
    while (iter != route_segments->end())
    {
        if (!iter->IsOnSegment())
        {
            route_segments->splice(route_segments->begin(), *route_segments,
                                   iter);
            break;
        }
        ++iter;
        ...
        std::list<hdmap::RouteSegments> *segments)
{
    ...
    if (is_lane_change_needed)
    {
        ...
        if (left_neighbor_lane_ids.end() !=
            std::find(left_neighbor_lane_ids.begin(),
                      left_neighbor_lane_ids.end(), target_lane_pair.first))
        {
        ...
        hdmap::LaneWaypoint *waypoint)
{
    ...
    std::copy_if(lanes.begin(), lanes.end(), std::back_inserter(valid_lanes),
                 [&](hdmap::LaneInfoConstPtr ptr) {
                     return navigation_lane_ids.count(ptr->lane().id().id()) >
                            0;
                 });
        ...
        std::list<hdmap::RouteSegments> *segments)
{
...
#if debug_ref_line_priority
    AINFO << "debug route segment, ref line size: " << segments->size();
    for (auto &s : *segments)
    {
        debug_route_segment(s);
    }
    ...
    if (segments->size() > 1)
    {
        ...
        {
            // 车辆不在优先车道
            if (!iter->IsOnSegment() && iter->is_reference_line_priority())
            {
                segments->splice(segments->begin(), *segments, iter);
                break;
            }
            ++iter;
        }
...
#if debug_ref_line_priority
    AINFO << "route segment sequence is updated";
    for (auto &s : *segments)
    {
        debug_route_segment(s);
    }
        ...
        std::list<hdmap::RouteSegments> *segments)
{
    ...
    // FLAGS_enable_reference_line_stitching is 1.
                                                ...
                                                ReferenceLine *reference_line)
{
    ...
    if (prev_segment == route_segments_.end())
    {
        ...
        {
            AWARN << "Current route segment is not connected with previous "
                     "route "
                     "segment";
        }
    ...
    if (remain_s > look_forward_required_distance)
    {
        ...
        *reference_line = *prev_ref;
        ADEBUG << "Reference line remain " << remain_s
               ...
               << look_forward_required_distance << " and no need to extend";
        return true;
    }

    double future_start_s = std::max(
            sl_point.s(),
            prev_segment_length - FLAGS_reference_line_stitch_overlap_distance);
    double future_end_s =
        ...
        const ReferenceLine &raw, const ReferenceLine &smoothed) const
{
         ...
         s += kReferenceLineDiffCheckStep)
    {
        ...
        {
            AERROR << "Fail to change xy point on smoothed reference line to "
                      "sl "
        ...
        std::vector<AnchorPoint> *anchor_points) const
{
    ...
    int num_of_anchors = std::max(
            2, static_cast<int>(reference_line.Length() / interval + 0.5));
                                               ...
                                               ReferenceLine *reference_line)
{
    ...
    bool is_prioity_ref_line = segments.is_reference_line_priority();
    reference_line->set_is_priority_ref_line(is_prioity_ref_line);
        ...
        ReferenceLine *reference_line)
{
    ...
    for (auto &point : anchor_points)
    {
        ...
        auto prefix_ref_point =
                prefix_ref.GetNearestReferencePoint(sl_point.s());
    ...
    AINFO << "print_ref_line_smooth_time:"
