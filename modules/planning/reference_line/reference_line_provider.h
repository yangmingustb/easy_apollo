...
public:
    ...
    const std::list<ReferenceLine>* GetConstReferenceLines()
    {
        return &reference_lines_;
    }

    void debug_route_segment(const hdmap::RouteSegments & lane_list) const
    {
        AINFO << "lane size" << lane_list.size();
        AINFO << lane_list.IsOnSegment();
        AINFO << lane_list.is_reference_line_priority();
        AINFO << lane_list.IsNeighborSegment();

        for (size_t i = 0; i < lane_list.size(); i++)
        {
             AINFO << lane_list.at(i).DebugString();
        }
