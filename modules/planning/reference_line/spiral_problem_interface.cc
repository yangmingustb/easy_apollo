    ...
    init_points_(std::move(points))
{
    ...
    {
        point_distances_.push_back(
                (init_points_[i + 1] - init_points_[i]).norm());
    }
    ...
    {
        double theta_diff = common::math::AngleDiff(relative_theta_.back(),
                                                    normalized_theta[i]);
                                                ...
                                                double* lambda)
{
    ...
    for (int i = 0; i + 1 < num_of_points_; ++i)
    {
        ...
        x[variable_offset + i] =
                point_distances_[i] / std::cos(0.5 * delta_theta);
    }
...
void SpiralProblemInterface::set_element_weight_kappa(const double weight_kappa)
{
