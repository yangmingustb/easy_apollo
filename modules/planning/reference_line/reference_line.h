...
public:
    ...
    bool Segment(const common::math::Vec2d& point,
                 const double distance_backward, const double distance_forward);
    ...
    bool has_curve_lane(const double s) const;
    ...
    double GetSpeedLimitFromS(const double s,
    ...
    void set_is_priority_ref_line(const bool value)
    {

        is_priority_ref_line_ = value;
    }

    bool is_priority_ref_line()
    {
...
private:
    ...
    // lane point, not ref line point
    hdmap::Path map_path_;

    // used by relative map
    uint32_t priority_ = 0;

    bool is_priority_ref_line_;
};
