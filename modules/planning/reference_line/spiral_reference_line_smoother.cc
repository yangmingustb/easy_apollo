        ...
        ReferenceLine* const smoothed_reference_line)
{
    ...
    else
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < anchor_points_.size(); ++i)
            {
                ...
                {
                    raw_point2d.emplace_back(p.path_point.x(),
                                             p.path_point.y());
                }
            }
            ...
            Smooth(raw_point2d, &opt_theta, &opt_kappa, &opt_dkappa, &opt_s,
                   &opt_x, &opt_y);
    ...
    for (const auto& p : smoothed_point2d)
    {
        ...
        ref_points.emplace_back(ReferencePoint(
                hdmap::MapPathPoint(common::math::Vec2d(p.x(), p.y()), heading,
                                    rlp.lane_waypoints()),
                kappa, dkappa));
    }
        ...
        std::vector<double>* ptr_y) const
{
    ...
    ptop->set_element_weight_curve_length(
            config_.spiral().weight_curve_length());
    ...
    ptop->get_optimization_results(ptr_theta, ptr_kappa, ptr_dkappa, ptr_s,
                                   ptr_x, ptr_y);
                                         ...
                                         std::vector<double>* ptr_y) const
{
    ...
    {
        ptop->set_start_point(fixed_start_x_, fixed_start_y_,
                              fixed_start_theta_, fixed_start_kappa_,
                              fixed_start_dkappa_);
    }
    ...
    app->Options()->SetIntegerValue(
            "acceptable_iter", config_.spiral().opt_acceptable_iteration());
    app->Options()->SetNumericValue("tol", config_.spiral().opt_tol());
    app->Options()->SetNumericValue("acceptable_tol",
                                    config_.spiral().opt_acceptable_tol());
        ...
        const double resolution) const
{
    ...
    common::PathPoint first_point =
            to_path_point(x.front(), y.front(), start_s, theta.front(),
                          kappa.front(), dkappa.front());
    ...
    for (size_t i = 0; i + 1 < theta.size(); ++i)
    {
        ...
        auto path_point_seg = Interpolate(
                start_x, start_y, start_s, theta[i], kappa[i], dkappa[i],
        ...
        const double delta_s, const double resolution) const
{
    ...
    for (size_t i = 1; i <= num_of_points; ++i)
    {
        ...
        auto path_point =
                to_path_point(start_x + dx, start_y + dy, start_s + inter_s,
                              theta, kappa, dkappa);
