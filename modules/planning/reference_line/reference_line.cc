...
ReferenceLine::ReferenceLine(const MapPath& hdmap_path) : map_path_(hdmap_path)
{
    for (const auto& point : hdmap_path.path_points())
    {
        ...
        reference_points_.emplace_back(
                hdmap::MapPathPoint(point, point.heading(), lane_waypoint), 0.0,
                0.0);
    }
    CHECK_EQ(static_cast<size_t>(map_path_.num_points()),
             reference_points_.size());
...
bool ReferenceLine::Stitch(const ReferenceLine& other)
{
    ...
    {
        AWARN << "Failed to project the first point to the other reference "
                 "line.";
        return false;
    }
    ...
    {
        AWARN << "Failed to project the last point to the other reference "
                 "line.";
        return false;
    }
    ...
    if (first_join)
    {
        ...
        {
            AERROR << "lateral stitching error on first join of reference line "
                      "too "
                      "big, stitching fails";
            return false;
        }
...
ReferenceLine::ToFrenetFrame(const common::TrajectoryPoint& traj_point) const
{
    ...
    // ADEBUG << "l " << l_condition[0];
    // ADEBUG << "dl " << l_condition[1];
    // ADEBUG << "ddl " << l_condition[2];

    // ADEBUG << traj_point.DebugString();

    // ADEBUG << "ref_point.DebugString()";
    // ADEBUG << ref_point.DebugString();
                                                ...
                                                const double y) const
{
    ...
    double s = ReferenceLine::FindMinDistancePoint(
            reference_points_[index_start], s0, reference_points_[index_end],
            s1, x, y);
        ...
        const double s1, const InterpolatedIndex& index) const
{
    ...
    const double dkappa =
            common::math::lerp(p0.dkappa(), s0, p1.dkappa(), s1, s);
...
bool ReferenceLine::GetApproximateSLBoundary(
        const common::math::Box2d& box, const double start_s,
        const double end_s, SLBoundary* const sl_boundary) const
{
    double s = 0.0;
    double l = 0.0;
    double distance = 0.0;
    if (!map_path_.GetProjectionWithHueristicParams(box.center(), start_s,
                                                    end_s, &s, &l, &distance))
    {
                                  ...
                                  SLBoundary* const sl_boundary) const
{
    ...
    for (const auto& point : corners)
    {
        ...
        if (!XYToSL(point, &sl_point))
        {
            AERROR << "Failed to get projection for point: "
                   << point.DebugString() << " on reference line.";
            return false;
        }
    ...
    for (size_t i = 0; i < corners.size(); ++i)
    {
        ...
        {
            AERROR << "Failed to get projection for point: "
                   << p_mid.DebugString() << " on reference line.";
            return false;
        }
...
double ReferenceLine::GetSpeedLimitFromS(
        const double s, const double speed_limit_by_scenario) const
{
    ...
    double speed_limit = speed_limit_by_scenario;
...
bool ReferenceLine::has_curve_lane(const double s) const
{
    ...
    for (const auto& lane_waypoint : map_path_point.lane_waypoints())
    {
        ...
        const auto& turn_type = lane_waypoint.lane->lane().turn();
        ...
        else
        {
            ...
            if (start_s >= limit.start_s)
            {
                ...
                if (end_s <= limit.end_s)
                {
                    ...
                    new_speed_limit.emplace_back(end_s, limit.end_s,
                                                 limit.speed_limit);
                }
                else
                {
                    new_speed_limit.emplace_back(start_s, limit.end_s,
                                                 min_speed);
                }
            }
            else
            {
                new_speed_limit.emplace_back(start_s, limit.start_s,
                                             speed_limit);
                if (end_s <= limit.end_s)
                {
                    new_speed_limit.emplace_back(limit.start_s, end_s,
                                                 min_speed);
                    new_speed_limit.emplace_back(end_s, limit.end_s,
                                                 limit.speed_limit);
                }
                ...
                {
                    new_speed_limit.emplace_back(limit.start_s, limit.end_s,
                                                 min_speed);
                }
            }
            start_s = limit.end_s;
