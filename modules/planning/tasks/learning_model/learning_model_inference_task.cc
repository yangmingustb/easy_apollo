...
Status LearningModelInferenceTask::Process(Frame* frame)
{
    ...
    if (learning_data_frame.adc_trajectory_point_size() <= 0)
    {
        const std::string msg = absl::StrCat(
                "learning_data adc_trajectory_point empty. frame_num[",
                learning_data_frame.frame_num(), "]");
        AERROR << msg;
        // hybrid model will use rule based planning when learning model output
        // is not ready
    ...
    if (!trajectory_imitation_inference_->DoInference(&learning_data_frame))
    {
        const std::string msg =
                absl::StrCat("TrajectoryImitationLibtorchInference Inference "
                             "failed. frame_num[",
                             learning_data_frame.frame_num(), "]");
        AERROR << msg;
        return Status(ErrorCode::PLANNING_ERROR, msg);
        ...
        std::vector<common::TrajectoryPoint>* adc_future_trajectory)
{
    ...
    for (size_t i = 1; i < adc_future_trajectory->size(); ++i)
    {
        ...
        const auto& pre_path_point =
                (*adc_future_trajectory)[i - 1].path_point();
        accumulated_s +=
    ...
    for (size_t i = 0; i + 1 < adc_future_trajectory->size(); ++i)
    {
        ...
        const auto& cur_relative_time =
                (*adc_future_trajectory)[i].relative_time();
