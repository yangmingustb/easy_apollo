...
#define debug_path_assessment_decider (0)

#define path_assessment_safe_buffer (0.05)

struct path_point_decision
{
    // PointDecision contains (s, PathPointType, distance to closest obstacle).

    double s;
    PathData::PathPointType type;
    double dist_to_obs;
};
...
private:
    ...
    // gjk collision check
    bool is_colliding_with_static_obstacles(
            const ReferenceLineInfo& reference_line_info,
            const PathData& path_data);
    ...
    void InitPathPointDecision(
            const PathData& path_data,
            std::vector<std::tuple<double, PathData::PathPointType,
                                   double>>* const path_point_decision);
    ...
    void check_path_cross_lane_line(
            const ReferenceLineInfo& reference_line_info,
            const PathData& path_data, const bool is_lane_change_path,
            std::vector<std::tuple<double, PathData::PathPointType,
                                   double>>* const path_point_state);
    ...
    int update_path_by_manual_lane_borrow(
            const LaneBorrowManual& lane_borrow_manual,
            ReferenceLineInfo* const reference_line_info);

    int get_sl_boundary_by_delta_theta(
    ...
    int update_path_by_turtle_car_decision_lane_borrow(
