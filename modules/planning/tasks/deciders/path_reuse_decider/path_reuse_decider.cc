                                 ...
                                 ReferenceLineInfo* const reference_line_info)
{
    ...
    */
    bool is_change_lane_path = reference_line_info->is_change_lane_ref_line();
    if (is_change_lane_path && !lane_change_status->is_current_opt_succeed())
    {
        ...
        ADEBUG << "Stop reusing path when optimization failed on change lane "
                  "path";
        return Status::OK();
    }
        ...
        double* blocking_obstacle_s)
{
    ...
    const auto* blocking_obstacle =
            indexed_obstacles.Find(blocking_obstacle_ID);
        ...
        for (const auto& corner_point : ABCDpoints)
        {
            ...
            for (const auto& obstacle_polygon : obstacle_polygons)
            {
                if (obstacle_polygon.IsPointIn(curr_point))
                {
                    ...
                    ADEBUG << "collision:" << curr_point.x() << ", "
                           << curr_point.y();
                    Vec2d xy_point;
                    reference_line.SLToXY(curr_point_sl, &xy_point);
                    ADEBUG << "collision:" << xy_point.x() << ", "
                           << xy_point.y();
        ...
        Frame* frame, ReferenceLineInfo* const reference_line_info)
{
    ...
    ADEBUG << "init_path_point x:[" << std::setprecision(9)
           << init_path_point.x() << "], y[" << init_path_point.y() << "], s: ["
           << init_path_point.s() << "]";
    ...
    ADEBUG << "previous path_data size: "
           << path_data->discretized_path().size();
