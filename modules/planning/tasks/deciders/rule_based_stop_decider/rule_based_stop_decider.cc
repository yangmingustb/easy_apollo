...
constexpr double kpath_end_stop_distance = 0.5;
}  // namespace
...
void RuleBasedStopDecider::CheckLaneChangeUrgency(Frame *const frame)
{
    for (auto &reference_line_info : *frame->mutable_reference_line_info())
    {
        ...
        {
            is_clear_to_change_lane_ = LaneChangeDecider::IsClearToChangeLane(
                    &reference_line_info);
        ...
        // When the target lane is blocked in change-lane case, check the
        // urgency Get the end point of current routing
            ...
            reference_line->IsOnLane(sl_point))
        {
            ...
            // If ADC is still far from the end of routing, no need to stop,
            // skip
            ...
            // In urgent case, set a temporary stop fence and wait to change
            // lane
            // TODO(Jiaxuan Xu): replace the stop fence to more intelligent
            // actions
        ...
        Frame *const frame, ReferenceLineInfo *const reference_line_info)
{
    ...
    start_s = reference_line_info->path_data().frenet_frame_path().front().s();
    end_s = reference_line_info->path_data().frenet_frame_path().back().s();

    double path_s = end_s - start_s;

    if (!reference_line_info->path_data().path_label().empty() &&
        path_s < FLAGS_short_path_length_threshold)
    {
        const std::string stop_wall_id =
                PATH_END_VO_ID_PREFIX +
                reference_line_info->path_data().path_label();

        std::vector<std::string> wait_for_obstacles;
        double stop_line_s = end_s;

        double stop_safe_dist = kpath_end_stop_distance;

        util::BuildStopDecision(stop_wall_id, stop_line_s, stop_safe_dist,
                                StopReasonCode::STOP_REASON_REFERENCE_END,
                                wait_for_obstacles, "RuleBasedStopDecider",
        ...
        ReferenceLineInfo *const reference_line_info)
{
    ...
    nearby_path.GetNearestPoint({stop_point->x(), stop_point->y()},
