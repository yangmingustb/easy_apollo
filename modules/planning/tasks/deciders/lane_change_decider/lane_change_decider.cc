        ...
        Frame* frame, ReferenceLineInfo* const current_reference_line_info)
{
    ...
    {
        PrioritizeChangeLane(true, reference_line_info_list);
        return Status::OK();
    }
    ...
    if (current_reference_line_info->is_change_lane_ref_line())
    {
        prev_status->set_is_clear_to_change_lane(
                IsClearToChangeLane(current_reference_line_info));
    }

    // 首次进入这个函数时，init state
    if (!prev_status->has_status())
    {
        UpdateStatus(now, ChangeLaneStatus::CHANGE_LANE_FINISHED,
                     GetCurrentPathId(*reference_line_info_list));

        prev_status->set_last_succeed_timestamp(now);

        AINFO << "lane change decider state init "
              << prev_status->DebugString();

        return Status::OK();
    }

    bool is_multi_lane = reference_line_info_list->size() > 1;

#if debug_lane_change_decider
    AINFO << "is_multi_lane: " << is_multi_lane;

    AINFO << prev_status->DebugString();
#endif

    if (!is_multi_lane)
    {
        const auto& path_id = reference_line_info_list->front().Lanes().Id();
        ...
        else
        {
            const std::string msg = absl::StrCat(
                    "Unknown state: ", prev_status->ShortDebugString());
            AERROR << msg;
            return Status(ErrorCode::PLANNING_ERROR, msg);
        }
    ...
    else
    {
        ...
        auto current_path_id = GetCurrentPathId(*reference_line_info_list);
        ...
        if (prev_status->status() == ChangeLaneStatus::IN_CHANGE_LANE)
        {
            ...
            else
            {
                ...
                PrioritizeChangeLane(false, reference_line_info_list);

                AINFO << " change lane finish";
...
#if debug_lane_change_decider
            AINFO << "lane change decider state " << prev_status->status();
            ...
            AINFO << "current lane id "<<current_reference_line_info->Lanes().Id();
            AINFO << " veh l to ref line: "
                  << current_reference_line_info
                             ->get_dist_from_plan_start_to_ref_line();
        ...
        else if (prev_status->status() == ChangeLaneStatus::CHANGE_LANE_FAILED)
        {
            ...
            {
                UpdateStatus(now, ChangeLaneStatus::IN_CHANGE_LANE,
                             current_path_id);
                 ...
                 ChangeLaneStatus::CHANGE_LANE_FINISHED)
        {
                ...
                lane_change_decider_config.change_lane_success_freeze_time())
            {
                ...
                AINFO << "freezed after completed lane change \n";
            }
            else
            {
                PrioritizeChangeLane(true, reference_line_info_list);
                ...
                AINFO << "change lane again after history lane change success";
            }
        }
        else
        {
        ...
        std::list<ReferenceLineInfo>* reference_line_info)
{
    ...
    {
        ADEBUG << "iter->is_change_lane_ref_line(): "
               << iter->is_change_lane_ref_line();

        /* is_prioritize_change_lane == true: prioritize
           change_lane_reference_line is_prioritize_change_lane == false:
           prioritize non_change_lane_reference_line */
        if ((is_prioritize_change_lane && iter->is_change_lane_ref_line()) ||
            (!is_prioritize_change_lane && !iter->is_change_lane_ref_line()))
        {
            ADEBUG << "is_prioritize_change_lane: "
    ...
    AINFO << "reference_line_info->is_change_lane_ref_line(): "
           << reference_line_info->begin()->is_change_lane_ref_line();

    AINFO << "ref line size " << reference_line_info->size();

    return;
}

void LaneChangeDecider::update_ref_line_sequence(
        const bool is_prioritize_change_lane,
        std::list<ReferenceLineInfo>* reference_line_info)
{
    if (reference_line_info->empty())
    ...
    while (iter != reference_line_info->end())
    {
        ADEBUG << "iter->is_change_lane_ref_line(): " << iter->is_change_lane_ref_line();
        ...
        std::list<ReferenceLineInfo>* reference_line_info) const
{
    ...
    while (iter != reference_line_info->end())
    {
        if (iter->is_change_lane_ref_line())
        {
        ...
        const std::list<ReferenceLineInfo>& reference_line_info) const
{
    ...
    {
        if (!info.is_change_lane_ref_line())
        {
        ...
        ReferenceLineInfo* reference_line_info)
{
         ...
         reference_line_info->path_decision()->obstacles().Items())
    {
        ...
        {
            double obstacle_moving_direction = obstacle->Trajectory()
                                                       .trajectory_point(0)
                                                       .path_point()
                                                       .theta();
            ...
            {
                vehicle_moving_direction = common::math::NormalizeAngle(
                        vehicle_moving_direction + M_PI);
            }
        ...
        static constexpr double kForwardMinSafeDistanceOnOppositeDirection =
                50.0;
        static constexpr double kBackwardMinSafeDistanceOnOppositeDirection =
                1.0;
...
bool LaneChangeDecider::IsPerceptionBlocked(
        ...
        const double search_beam_radius_intensity, const double search_range,
        const double is_block_angle_threshold)
{
         ...
         reference_line_info.path_decision().obstacles().Items())
    {
        ...
        if (std::fabs(common::math::NormalizeAngle(left_most_angle -
                                                   right_most_angle)) >
