...
#define debug_st_bounds_decider (0)
#define enable_st_bounds_decider (0)
                                ...
                                ReferenceLineInfo* const reference_line_info)
{
    ...
    // 申请了2个bound? st,vt? 这里会做overtake, yiled decision?
    // 生成了s bound, v bound, 但是dp没有使用？
                                                ...
                                                STBound* const vt_bound)
{
    ...
    for (size_t i = 0; i < st_bound->size(); ++i)
    {
        ...
        auto driving_limits_bound =
                st_driving_limits_.GetVehicleDynamicsLimits(t);
        ...
        std::vector<std::pair<double, double>>* const st_guide_line)
{
    ...
    for (size_t i = 0; i < st_bound->size(); ++i)
    {
...
#if debug_st_bounds_decider
        AINFO << "Processing st-boundary at t = " << t;
#endif

        // Get Boundary due to driving limits，运动学限制来初始化s boundary
        // when t is 0, lower and upper is current s
        auto driving_limits_bound =
                st_driving_limits_.GetVehicleDynamicsLimits(t);
        s_lower = std::fmax(s_lower, driving_limits_bound.first);
        s_upper = std::fmin(s_upper, driving_limits_bound.second);
...
#if debug_st_bounds_decider
        AINFO << "Bounds for s due to driving limits are "
               << "s_upper = " << s_upper << ", s_lower = " << s_lower;
#endif

        // Get Boundary due to obstacles
        // 当没有obs，available_s_bounds 就是默认值
        // available_s_bounds:可用s boundary
        // std::pair<double, double>： upper, lower
        // std::vector: 搜索所有的gap
        std::vector<std::pair<double, double>> available_s_bounds;

        // available_obs_decisions: 可用obs decision
        // 每一个gap对应一类障碍物决策
        std::vector<ObsDecSet> available_obs_decisions;
...
#if debug_st_bounds_decider
        AINFO << "Available choices are:";

        AINFO << "available_s_bounds " << available_s_bounds.size();
#endif

        for (int j = 0; j < static_cast<int>(available_s_bounds.size()); ++j)
        {
#if debug_st_bounds_decider
            AINFO << "  (" << available_s_bounds[j].first << ", "
                  << available_s_bounds[j].second << ")";

            for (size_t o = 0; o < available_obs_decisions[j].size(); o++)
            {
                AINFO << available_obs_decisions[j].at(o).first;
                AINFO << available_obs_decisions[j].at(o).second.DebugString();
            }
        ...
        if (!available_choices.empty())
        {
            ...
            RankDecisions(guide_line_s, driving_limits_bound,
                          &available_choices);
        ...
        // Update into st_bound
        // 看起来每一个t，只有一个s range。这里已经所有决策了，不需要DP搜索？
        st_bound->at(i) = std::make_tuple(t, s_lower, s_upper);
        vt_bound->at(i) = std::make_tuple(t, lower_obs_v, upper_obs_v);
    }
        ...
        std::vector<std::pair<STBoundPoint, ObsDecSet>>* available_choices)
{
    ...
    while (i < available_choices->size())
    {
        double s_lower = 0.0;
        double s_upper = 0.0;
        std::tie(std::ignore, s_lower, s_upper) =
        ...
        std::vector<std::pair<STBoundPoint, ObsDecSet>>* available_choices)
{
    ...
    {
        has_swaps = false;
        for (int i = 0; i < static_cast<int>(available_choices->size()) - 1;
             ++i)
        {
            ...
            if (A_room < kSTPassableThreshold || B_room < kSTPassableThreshold)
            {
                if (A_room < B_room)
                {
                    swap(available_choices->at(i + 1),
                         available_choices->at(i));
                    has_swaps = true;
        ...
        planning_internal::STGraphDebug* const st_graph_debug)
{
    ...
    {
        auto* speed_point = st_graph_debug->add_st_guide_line();
