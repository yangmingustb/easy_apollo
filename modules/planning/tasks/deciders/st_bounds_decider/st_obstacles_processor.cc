...
#define debug_st_obstacles_processor (0)
        ...
        PathDecision* const path_decision)
{
    ...
    // 只对交互障碍物，记录在non_ignore_obstacles。
    // 如果是非交互的危险障碍物，行人、自行车，没有处理。
    ...
    for (const auto* obs_item_ptr : path_decision->obstacles().Items())
    {
        ...
        else
        {
            ...
            if(obs_ptr->Id() == FLAGS_destination_obstacle_id)
            {
                boundary.SetBoundaryType(STBoundary::BoundaryType::STOP);
            }
...
#if debug_st_obstacles_processor
        AINFO << "Closest obstacle ID = " << closest_stop_obs_id;
        ...
        if (!closest_stop_obs_ptr->IsVirtual())
        {
            for (const auto& clear_zone : candidate_clear_zones_)
            {
                ...
                if (closest_stop_obs_boundary.min_s() >=
                            clear_zone_boundary.min_s() &&
                    closest_stop_obs_boundary.min_s() <=
                            clear_zone_boundary.max_s())
                {
                    std::tie(closest_stop_obs_id, closest_stop_obs_boundary,
                             closest_stop_obs_ptr) = clear_zone;
                    ADEBUG << "Clear zone " << closest_stop_obs_id
                           << " is closer.";
                    break;
                }
            }
...
#if debug_st_obstacles_processor
        AINFO << "Adding " << closest_stop_obs_ptr->Id()
              << " into the ST-graph.";
        AINFO << "min_s = " << closest_stop_obs_boundary.min_s();
    ...
    for (const auto* obs_item_ptr : path_decision->obstacles().Items())
    {
        ...
        if (non_ignore_obstacles.count(obs_ptr->Id()) == 0)
        {
            ...
            {
                obs_ptr->AddLateralDecision("st_obstacle_processor",
                                            ignore_decision);
            }
        }
    }
    ...
    // Fetch every obstacle's beginning and ending t-edges only.
    ...
    for (j = 0; j < obs_t_edges_.size(); j++)
    {
        obs_time_edge edge;
        edge.t = std::get<1>(obs_t_edges_[j]);
        edge.is_starting_time = std::get<0>(obs_t_edges_[j]);
        edge.s_min = std::get<2>(obs_t_edges_[j]);
        edge.s_max = std::get<3>(obs_t_edges_[j]);
        edge.obs_id = std::get<4>(obs_t_edges_[j]);

        AINFO << "Seeing a new t-edge at t = " << edge.t;

        AINFO << "is start edge: " << edge.is_starting_time;
        AINFO << "s min: " << edge.s_min;
        AINFO << "s max: " << edge.s_max;
        AINFO << "id: " << edge.obs_id;
    }
                ...
                available_obs_decisions)
{
...
#if debug_st_obstacles_processor
    AINFO << "There are " << obs_t_edges_.size() << " t-edges.";
#endif
...
#if debug_st_obstacles_processor
        obs_time_edge edge;
        edge.t = std::get<1>(obs_t_edges_[obs_t_edges_idx_ - 1]);
        edge.is_starting_time = std::get<0>(obs_t_edges_[obs_t_edges_idx_ - 1]);
        edge.s_min = std::get<2>(obs_t_edges_[obs_t_edges_idx_ - 1]);
        edge.s_max = std::get<3>(obs_t_edges_[obs_t_edges_idx_ - 1]);
        edge.obs_id = std::get<4>(obs_t_edges_[obs_t_edges_idx_ - 1]);

        AINFO << "Seeing a new t-edge at t = "
              << edge.t;
    ...
    for (const auto& obs_t_edge : new_t_edges)
    {
        ...
        {
#if debug_st_obstacles_processor
            AINFO << "Obstacle id: " << std::get<4>(obs_t_edge)
                  << " is leaving st-graph.";
#endif
    ...
    // 遍历obs，如果是overtake，删除做完overtake决策的obs
...
#if debug_st_obstacles_processor
        AINFO << "Obstacle id: " << obs_id_to_decision_pair.first;
        AINFO << "decision: " << obs_id_to_decision_pair.second.DebugString();
#endif
    }
...
#if debug_st_obstacles_processor
        AINFO << "remove Obstacle id: " << obs_id;
#endif
    }
...
#if debug_st_obstacles_processor
        AINFO << "Obstacle id: " << it.first;
        AINFO << "decision: " << it.second.DebugString();
#endif
    ...
    if (s_min > s_max)
    {
        AINFO << "stop and overtake conflict";
        return false;
    }

#if debug_st_obstacles_processor
    AINFO << "S-boundary based on existing decisions = (" << s_min << ", "
          << s_max << ")";
#endif
    ...
    for (auto obs_t_edge : new_t_edges)
    {
...
#if debug_st_obstacles_processor
        AINFO << "For obstacle id: " << std::get<4>(obs_t_edge)
		...
		// 需要停车的obs，直接决策
        auto obs_st_boundary = obs_id_to_st_boundary_[std::get<4>(obs_t_edge)];
        if (obs_st_boundary.boundary_type() == STBoundary::BoundaryType::STOP)
        {
#if debug_st_obstacles_processor
            AINFO << "  Apparently, it should be stop.";
#endif
        ...
        {
#if debug_st_obstacles_processor
            AINFO << "  Apparently, it should be yielded.";
#endif
        ...
        {
#if debug_st_obstacles_processor
            AINFO << "  Apparently, it should be overtaken.";
#endif
...
//         else if (std::get<4>(obs_t_edge) == FLAGS_destination_obstacle_id)
//         {
// #if debug_st_obstacles_processor
//             AINFO << "  Apparently, it should be stop.";
// #endif
...
//             decision.mutable_stop()->set_reason_code(
//                     StopReasonCode::STOP_REASON_DESTINATION);

//             obs_id_to_decision_[std::get<4>(obs_t_edge)] = decision;
//             obs_id_to_st_boundary_[std::get<4>(obs_t_edge)].SetBoundaryType(
...
//         }
        ...
        {
#if debug_st_obstacles_processor
            AINFO << "  It should be further analyzed.";
    ...
    for (auto s_gap : s_gaps)
    {
        ...
        for (auto obs_t_edge : ambiguous_t_edges)
        {
            ...
            obs_decisions.emplace_back(
                    obs_id, DetermineObstacleDecision(
                                    obs_s_min, obs_s_max,
    ...
    *is_caution_obstacle = false;
    ...
    else
    {
        ...
        for (const auto& obs_traj_pt : obs_trajectory.trajectory_point())
        {
            ...
            // 障碍物轨迹点是离散的,adc轨迹点也是离散的。在做碰撞检测的时候,会漏掉一些碰撞点.
            // 一种解决方案是，将障碍物轨迹点中心连接起来，再根据obs宽度，变成一个box，将box和adc
            // path继续进行碰撞检查.
        ...
        std::pair<double, double>* const overlapping_s)
{
    ...
    int pt_before_idx = GetSBoundingPathPointIndex(
            ...
            vehicle_param_.front_edge_to_center(), true, 0,
    ...
    int pt_after_idx = GetSBoundingPathPointIndex(
            adc_path_points, obstacle_instance,
            vehicle_param_.back_edge_to_center(), false, 0,
            static_cast<int>(adc_path_points.size()) - 2);
        ...
        const bool is_before, const int start_idx, const int end_idx)
{
    ...
    if (is_before)
    {
        ...
        if (IsPathPointAwayFromObstacle(adc_path_points[mid_idx],
                                        ...
                                        obstacle_instance, s_thresh, is_before))
        {
            return GetSBoundingPathPointIndex(adc_path_points,
                                              obstacle_instance, s_thresh,
                                              is_before, mid_idx, end_idx);
        }
        else
        {
            return GetSBoundingPathPointIndex(
                    adc_path_points, obstacle_instance, s_thresh, is_before,
                    start_idx, mid_idx - 1);
        }
    }
    else
    {
                                        ...
                                        obstacle_instance, s_thresh, is_before))
        {
            return GetSBoundingPathPointIndex(adc_path_points,
                                              obstacle_instance, s_thresh,
                                              is_before, start_idx, mid_idx);
        }
        else
        {
            return GetSBoundingPathPointIndex(adc_path_points,
                                              obstacle_instance, s_thresh,
                                              is_before, mid_idx + 1, end_idx);
        }
    }
}
        ...
        const Box2d& obs_box, const double s_thresh, const bool is_before)
{
    ...
    for (const auto& corner_pt : corner_points)
    {
        ...
        double corner_pt_s_dist =
                path_dir_unit_vec.InnerProd(perpendicular_vec);
...
/**
 * @brief  search gap 的原理：
 * 先搜索出下界，再搜索一个上界，形成一个gap. 一直循环下去
 * @note   
 * @param  obstacle_t_edges: 
 * @param  s_min: 
 * @param  s_max: 
 * @retval 
 */
std::vector<std::pair<double, double>> STObstaclesProcessor::FindSGaps(
        const std::vector<ObsTEdge>& obstacle_t_edges, double s_min,
        double s_max)
{
    // std::vector<std::pair<double, int>> obs_s_edges;
    std::vector<obs_st_corner_point> obs_s_edges;
	obs_st_corner_point point;
    for (auto obs_t_edge : obstacle_t_edges)
    {
        point.is_lower = 1;
        point.s = std::get<2>(obs_t_edge);

		//lower
        obs_s_edges.emplace_back(point);

		// upper
        point.is_lower = 0;
        point.s = std::get<3>(obs_t_edge);
        obs_s_edges.emplace_back(point);
    }
    ...
    // s， lower/upper排序,从小到大
    std::sort(obs_s_edges.begin(), obs_s_edges.end(),
              [](const obs_st_corner_point& lhs, const obs_st_corner_point& rhs)
              {
                  if (lhs.s != rhs.s)
                  {
                      return lhs.s < rhs.s;
    ...
    {
        if (obs_s_edge.is_lower == 1)
        {
            num_st_obs++;
            if (num_st_obs == 1)
            {
                s_gaps.emplace_back(prev_open_s, obs_s_edge.s);
            }
        }
        else
        {
            ...
            if (num_st_obs == 0)
            {
                prev_open_s = obs_s_edge.s;
            }
        }

#if debug_st_obstacles_processor
        AINFO << "  obs_s_edge: " << obs_s_edge.is_lower << ", "
              << obs_s_edge.s;
#endif
    }
