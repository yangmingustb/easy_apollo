...
struct obs_time_edge
{
    //  (is_starting_t, t, s_min, s_max, obs_id).
    int is_starting_time;
    double t;
    double s_min;
    double s_max;
    std::string obs_id;
...
public:
     ...
     *
     * 可能的决策结果，都记录到available_obs_decisions
     * 
     * available_s_bounds and available_obs_decisions是一一对应的关系
     * @param Time t
    ...
    bool GetSBoundsFromDecisions(
            ...
            std::vector<std::vector<
                    std::pair<std::string, ObjectDecisionType>>>* const
...
private:
    ...
    std::vector<std::pair<double, double>> FindSGaps(
            const std::vector<std::tuple<int, double, double, double,
                                         std::string>>& obstacle_t_edges,
...
private:
    ...
    // 存储时间边，根据st boundary，得到start boundary, end boundary.
    ...
    // 所有动态obs，最近的静态obs，停车区
    // 有多个静态障碍物时，只会对第一个障碍物生成boundary
    std::unordered_map<std::string, STBoundary> obs_id_to_st_boundary_;

    // 在范围内的obs决策？
    // 所有决策的obs，根据boundary生成的决策。
    // 如果对于红灯、终点，这种提前做好的决策，又根据boundary重新计算决策，有bug
    ...
    // adc 低路权： lane borrow, lane change
