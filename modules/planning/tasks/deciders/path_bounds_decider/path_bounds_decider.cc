...
// PathBound contains a vector of PathBoundPoints. bound的s相对于reference line
                                  ...
                                  ReferenceLineInfo* const reference_line_info)
{
    ...
    candidate_path_boundaries.back().type_ =
            path_boundary_type::PATH_BOUND_FALLBACK;
    ...
    if (plan_pull_over_path)
    {
        ...
        if (!ret.ok())
        {
            AWARN << "Cannot generate a pullover path bound, do regular "
                     "planning.";
        }
        else
        {
            ...
            candidate_path_boundaries.back().type_ =
                    path_boundary_type::PATH_BOUND_PULL_OVER;

            reference_line_info->SetCandidatePathBoundaries(
                    std::move(candidate_path_boundaries));
            ADEBUG << "Completed pullover and fallback path boundaries "
                      "generation.";
        ...
        reference_line_info->is_change_lane_ref_line())
    {
        ...
        candidate_path_boundaries.back().type_ =
                path_boundary_type::PATH_BOUND_LANE_CHANGE_LEFT;
        ...
        ADEBUG << "Completed lanechange and fallback path boundaries "
                  "generation.";
    ...
    bool is_lane_borrow_manual = false;
    bool is_lane_borrow_by_turtle_car = false;

    // 只要处于lane borrow 场景，且有lane borrow direction，那么就产生lane
    // borrow boundary. 至于后面是选择lane borrow path 还是lane keep
    // path，并不能确定
    if (reference_line_info->is_path_lane_borrow())
    {
        ...
        if (PathLaneBorrowDecider::has_manual_lane_borrow_decision(
                    path_decider_status))
        {
            is_lane_borrow_manual = true;
        }

        // low speed car trigger lane borrow
        if (PathLaneBorrowDecider::has_lane_borrow_decision_by_turtle_car(
                    path_decider_status))
        {
            is_lane_borrow_by_turtle_car = true;
        }

        bool has_left_lane_borrow_by_turtle_car = false;

        if (PathLaneBorrowDecider::has_left_lane_borrow_decision_by_turtle_car(
                    path_decider_status))
        {
            has_left_lane_borrow_by_turtle_car = true;
        }

        AINFO << "is_lane_borrow_by_turtle_car "
              << is_lane_borrow_by_turtle_car;

        for (const auto& lane_borrow_direction :
             path_decider_status.decided_side_pass_direction())
        {
            if (lane_borrow_direction == LANE_BORROW_DIRECTION_LEFT ||
                has_left_lane_borrow_by_turtle_car)
            {
                lane_borrow_info_list.push_back(LaneBorrowInfo::LEFT_BORROW);
            }
            else if (lane_borrow_direction == LANE_BORROW_DIRECTION_RIGHT)
            {
    ...
    for (const auto& lane_borrow_info : lane_borrow_info_list)
    {
        ...
        switch (lane_borrow_info)
        {
            case LaneBorrowInfo::LEFT_BORROW:
                path_label = "left";
                candidate_path_boundaries.back().type_ =
                        path_boundary_type::PATH_BOUND_LANE_BORROW_LEFT;

                // if manual lane borrow, then update target offset in lane
                if (is_lane_borrow_manual)
                {
                    check_s = reference_line_info->AdcSlBoundary().end_s();

                    neighbour_width =
                            reference_line_info->get_neighbour_lane_width(
                                    adc_direction::LEFT, check_s);

                    reference_line_info->reference_line().GetLaneWidth(
                            check_s, &left_width, &right_width);

                    candidate_path_boundaries.back().set_target_offset(
                            neighbour_width * 0.5 + left_width);
                }

                if (is_lane_borrow_by_turtle_car)
                {
                    check_s = reference_line_info->AdcSlBoundary().end_s();
            ...
            case LaneBorrowInfo::RIGHT_BORROW:
                ...
                candidate_path_boundaries.back().type_ =
                        path_boundary_type::PATH_BOUND_LANE_BORROW_RIGHT;
                ...
                if (is_lane_borrow_manual)
                {
                    ...
                    candidate_path_boundaries.back().set_target_offset(
                            -neighbour_width * 0.5 - right_width);
                }
            ...
            default:
                ...
                candidate_path_boundaries.back().type_ =
                        path_boundary_type::PATH_BOUND_LANE_KEEP;
        ...
        AINFO << "label " << candidate_path_boundaries.back().label();
        AINFO << "blocking obs id "
              << candidate_path_boundaries.back().blocking_obstacle_id();
    }
        ...
        const Frame& frame, ReferenceLineInfo& reference_line_info)
{
    ...
    sl_point.set_s(adc_frenet_s_);
    sl_point.set_l(adc_frenet_l_);
    reference_line_info.set_start_point_sl(sl_point);
    ...
    adc_l_upper_ = sl_bound.end_l();
    adc_l_lower_ = sl_bound.start_l();

    // update adc  center
    common::math::Vec2d adc_point;
    common::SLPoint adc_sl_point;

    const common::VehicleState& adc_state = reference_line_info.vehicle_state();

    adc_point.set_x(adc_state.x());
    adc_point.set_y(adc_state.y());

    reference_line.XYToSL(adc_point, &adc_sl_point);

    reference_line_info.set_adc_point_sl(adc_sl_point);

    AINFO << "print_adc_lateral_lane_offet:"
          << "(" << adc_sl_point.l() << ","
          << ")";

#if debug_path_bound_decider
    AINFO << "adc lower, upper: " << adc_l_lower_ << ", " << adc_l_upper_;
#endif
        ...
        std::string* const borrow_lane_type)
{
    ...
    // 2. Decide a rough boundary based on lane info and ADC's
    // position，已经减去车宽.
    // 如果是lane borrow 决策，bound 初始化为2个车道
        ...
        PathBound* const path_bound)
{
    ...
    {
        const std::string msg = "ADC is outside road boundary already. Cannot "
                                "generate pull-over path";
        AERROR << msg;
        return Status(ErrorCode::PLANNING_ERROR, msg);
    }
        ...
        std::tuple<double, double, double, int>* const pull_over_configuration)
{
                    ...
                    pull_over_space_length))
    {
                        ...
                        pull_over_space_length))
        {
            ...
            {
                AERROR << "Not close enough to road-edge. Not feasible for "
                          "pull-over.";
            ...
            ADEBUG << "left_bound[" << left_bound << "] right_bound["
                   << right_bound << "]";
            if (left_bound - right_bound < pull_over_space_width)
            {
                AERROR << "Not wide enough to fit ADC. Not feasible for "
                          "pull-over.";
                is_feasible_window = false;
                break;
            }
        ...
        if (is_feasible_window)
        {
            ...
            const double back_clear_to_total_length_ratio =
                    (0.5 * (kPulloverLonSearchCoeff - 1.0) *
            ...
            auto pull_over_idx = static_cast<size_t>(
                    back_clear_to_total_length_ratio *
            ...
            // reference line theta in case of reference line theta not aligned
            // with the lane
            ...
            double s = 0.0;
            double l = 0.0;
            auto point = common::util::PointFactory::ToPointENU(pull_over_x,
                                                                pull_over_y);
        ...
        PathBound* const path_bound)
{
    ...
    max_planning_dist = std::fmax(kPathBoundsDeciderHorizon,
                                  reference_line_info.GetCruiseSpeed() *
                                          FLAGS_trajectory_time_length);

    double max_planning_ref_s = adc_frenet_s_ + max_planning_dist;

    max_planning_ref_s = std::fmin(max_planning_ref_s, reference_line.Length());

    // Starting from ADC's current position, increment until the horizon, and
    // set lateral bounds to be infinite at every spot.
    for (double curr_s = adc_frenet_s_; curr_s < max_planning_ref_s;
        ...
        std::string* const borrow_lane_type)
{
    ...
    for (size_t i = 0; i < path_bound->size(); ++i)
    {
        ...
        else
        {
            ...
            // The left-width and right-width are w.r.t. lane-center, not
            // ref-line.
        ...
        bool is_fallback_lanechange)
{
    ...
    // 速度buffer, 因为车辆有初始速度，所以需要给boundary拓展
    // v^2 = 2*acc_*x
    double ADC_speed_buffer = (adc_frenet_ld_ > 0 ? 1.0 : -1.0) *
                              adc_frenet_ld_ * adc_frenet_ld_ / lat_acc_bound /
                              2.0;

    double ADC_speed_buffer_left;
    double ADC_speed_buffer_right;

    ADC_speed_buffer_left = 0.0;
    ADC_speed_buffer_right = 0.0;
    // 向左运动
    if (adc_frenet_ld_ > 0.0)
    {
        ADC_speed_buffer_left = ADC_speed_buffer;
    }
    else
    {
        ADC_speed_buffer_right = ADC_speed_buffer;
    }
    ...
    for (size_t i = 0; i < path_bound->size(); ++i)
    {
        ...
        else
        {
                                            ...
                                            &curr_road_right_width))
            {
                ...
                is_right_lane_boundary =
                        (std::abs(curr_road_right_width -
                                  curr_lane_right_width) > boundary_buffer);
            }
            reference_line.GetOffsetToMap(curr_s, &offset_to_lane_center);
            curr_lane_left_width += offset_to_lane_center;
            curr_lane_right_width -= offset_to_lane_center;
        ...
        // bounds. 
        // in lane keep or lane borrow, 也需要包含adc boundary
        // 因为车辆朝向不同，需要包含最大l。
        // 应该只拓展车辆的附近位置，而不是所有位置
        if (config_.path_bounds_decider_config()
                    .is_extend_lane_bounds_to_include_adc() ||
            is_fallback_lanechange)
        {
            ...
            // 相对于ref line的l
            double curr_left_bound_adc =
                    adc_l_upper_ + ADC_speed_buffer_left + ADC_buffer;

            curr_left_bound = std::fmax(curr_left_bound_lane - offset_to_map,
                                        curr_left_bound_adc);
            ...
            double curr_right_bound_adc =
                    adc_l_lower_ + ADC_speed_buffer_right - ADC_buffer;

            curr_right_bound = std::fmin(curr_right_bound_lane - offset_to_map,
...
// 如果不安全，处于换道观察阶段，生成换道轨迹，首先需要限制boundary
// 如果安全，不需要限制path boudary？
        ...
        std::string* const blocking_obstacle_id)
{
    ...
    for (size_t i = 1; i < path_boundaries->size(); ++i)
    {
            ...
            std::get<1>(sorted_obstacles[obs_idx]) < curr_s)
        {
                   ...
                   std::get<1>(sorted_obstacles[obs_idx]) < curr_s)
            {
                ...
                //   - If boundaries blocked, then decide whether can
                //   side-pass.
                ...
                if (std::get<0>(curr_obstacle) == 1)
                {
                    ...
                    if (curr_obstacle_l_min + curr_obstacle_l_max <
                        center_line * 2)
                    {
                        // Obstacle is to the right of center-line, should pass
                        // from left.
                    ...
                    {
                        // Obstacle is to the left of center-line, should pass
                        // from right.
                ...
                else
                {
                    ...
                    {
                        right_bounds.erase(
                                right_bounds.find(curr_obstacle_l_max));
                    }
                    else
                    {
                        left_bounds.erase(
                                left_bounds.find(curr_obstacle_l_min));
                    }
                ...
                if (std::get<1>((*path_boundaries)[i]) >
                    std::get<2>((*path_boundaries)[i]))
                {
                    AINFO << "Path is blocked at s = " << curr_s;
                    ...
                    {
                        *blocking_obstacle_id =
                                obs_id_to_direction.begin()->first;
                    }
                    break;
                }
                else
    ...
    // AINFO << "blocked obs " << *blocking_obstacle_id;
        ...
        const IndexedList<std::string, Obstacle>& indexed_obstacles)
{
    ...
    for (const auto* obstacle : indexed_obstacles.Items())
    {
        ...
        // AINFO << "id[" << obstacle->PerceptionId() << "] s["
        //       << obstacle_sl.start_s() << "] curr_obstacle_l_min["
        //       << obstacle_sl.start_l() << "] curr_obstacle_l_max["
        //       << obstacle_sl.end_l() << "] end s[" << obstacle_sl.end_s()
        //       << "]";
    }
        ...
        PathBound* const curr_path_bounds)
{
    ...
    {
        ADEBUG << "Completed path bounds search ending at path_idx = "
               << path_idx;
        ...
        std::get<1>(sorted_obstacles[obs_idx]) > curr_s)
    {
             ...
             it != obs_id_to_details->end(); ++it)
        {
            ...
            {
                // Pass from left.
                right_bounds_from_obstacles = std::max(
                        right_bounds_from_obstacles, std::get<1>(it->second));
            }
            else
            {
                // Pass from right.
                left_bounds_from_obstacles = std::min(
                        left_bounds_from_obstacles, std::get<1>(it->second));
            }
        }
        // 2. Update the path boundary
        bool is_able_to_update = UpdatePathBoundaryWithBuffer(
                path_idx, left_bounds_from_obstacles,
                right_bounds_from_obstacles, curr_path_bounds);
        // 3. Return proper values.
        std::vector<PathBound> ret;
        if (is_able_to_update)
        {
            ret = ConstructSubsequentPathBounds(sorted_obstacles, path_idx + 1,
                                                obs_idx, obs_id_to_details,
                                                curr_path_bounds);
        }
        else
        {
           ...
           std::get<1>(sorted_obstacles[new_obs_idx]) <= curr_s)
    {
        if (!std::get<0>(sorted_obstacles[new_obs_idx]))
        {
            ...
            obs_id_to_details->erase(
                    std::get<4>(sorted_obstacles[new_obs_idx]));
        }
        else
        {
    ...
    for (size_t i = 0; i < pass_direction_decisions.size(); ++i)
    {
        ...
        for (size_t j = 0; j < pass_direction_decisions[i].size(); ++j)
        {
            if (pass_direction_decisions[i][j])
            {
                ...
                (*obs_id_to_details)[std::get<4>(new_entering_obstacles[j])] =
                        std::make_tuple(true,
                                        std::get<3>(new_entering_obstacles[j]));
            }
            else
            {
                // Pass from right.
                (*obs_id_to_details)[std::get<4>(new_entering_obstacles[j])] =
                        std::make_tuple(false,
                                        std::get<2>(new_entering_obstacles[j]));
            }
        }
        ...
        // c. Update for this path_idx, and construct the subsequent path
        // bounds.
        ...
        const std::vector<ObstacleEdge>& new_entering_obstacles)
{
    ...
    lateral_edges.emplace_back(1, std::numeric_limits<double>::lowest(), 0.0,
                               0.0, "l_min");
    ...
    for (size_t i = 0; i < new_entering_obstacles.size(); ++i)
    {
        ...
        lateral_edges.emplace_back(1, std::get<2>(new_entering_obstacles[i]),
                                   0.0, 0.0,
                                   std::get<4>(new_entering_obstacles[i]));
        lateral_edges.emplace_back(0, std::get<3>(new_entering_obstacles[i]),
                                   0.0, 0.0,
                                   std::get<4>(new_entering_obstacles[i]));
    }
    ...
    for (size_t i = 0; i < lateral_edges.size(); ++i)
    {
        ...
        {
            empty_slot.push_back((std::get<1>(lateral_edges[i]) +
                                  std::get<1>(lateral_edges[i + 1])) /
                                 2.0);
        }
    }
        ...
        bool is_right_lane_bound)
{
    ...
    double new_l_max =
            std::fmin(std::get<2>((*path_boundaries)[idx]),
                      left_bound - left_adc_buffer_coeff *
                                           GetBufferBetweenADCCenterAndEdge());
    ...
    {
        // AINFO << "Path is blocked at idx = " << idx;
        return false;
    }
                                       ...
                                       PathBound* const path_boundaries)
{
    if (path_blocked_idx != -1)
    {
        ...
        int range =
                static_cast<int>(path_boundaries->size()) - path_blocked_idx;
...
void PathBoundsDecider::PathBoundsDebugString(const PathBound& path_boundaries)
{
