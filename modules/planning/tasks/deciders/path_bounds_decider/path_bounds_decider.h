...
#define debug_path_bound_decider (0)
...
// boundary要减去车辆半宽
// 如果被障碍物阻挡，则不继续生成bound，所以bound长度比较短
// 如果车道线是实线，bound怎么生成？
...
private:
    ...
    bool SearchPullOverPosition(
            const Frame& frame, const ReferenceLineInfo& reference_line_info,
            const std::vector<std::tuple<double, double, double>>& path_bound,
            std::tuple<double, double, double, int>* const
    ...
    bool GetBoundaryFromStaticObstacles(
            ...
            std::vector<std::tuple<double, double, double>>* const
                    path_boundaries,
            std::string* const blocking_obstacle_id);
    ...
    std::vector<std::vector<bool>> DecidePassDirections(
            ...
            const std::vector<std::tuple<int, double, double, double,
                                         std::string>>& new_entering_obstacles);
    ...
    /** @brief Update the path_boundary at "idx", as well as the new
     * center-line. It also checks if ADC is blocked (lmax < lmin).
     *  @param The current index of the path_bounds
     *  @param The minimum left boundary (l_max)
     *  @param The maximum right boundary (l_min)
     *  @param The path_boundaries (its content at idx will be updated)
    ...
    void RecordDebugInfo(const std::vector<std::tuple<double, double, double>>&
                                 path_boundaries,
                         const std::string& debug_name,
                         ReferenceLineInfo* const reference_line_info);

private:
    //  planning start point, not adc position
