...
// 这个decicer是干啥的：
// 速度点在下方，决策包括：follow, stop, yield
//速度点在上方，决策包括：overtake
// 通过DP决策之后，生成纵向决策结果
...
public:
    ...
    int generate_speed_profile(const SpeedData& speed_profile,
                                 PathDecision* const path_decision,
                                 ReferenceLineInfo* reference_line_info);

    int record_constraints(ReferenceLineInfo* reference_line_info);
...
private:
    ...
    bool create_stop_decision_for_reverse_obs(
    ...
    /**
     * @brief "too close" is determined by whether ego vehicle will hit the
     *front obstacle if the obstacle drive at current speed and ego vehicle use
     *some reasonable deceleration
     **/
    bool is_adc_need_stop(const Obstacle& obstacle) const;

    bool is_reverse_driving_obstacle(const Obstacle& obstacle,
                                     const STBoundary& boundary) const;
...
private:
    ...
    SpeedData* hard_brake_speed_data_;
    SpeedData soft_brake_speed_data_;
    SpeedData speed_up_data_;
