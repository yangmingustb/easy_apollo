...
#if debug_path_decider
    AINFO << "blocking_obstacle_id: " << blocking_obstacle_id;
    AINFO << path_decision->stop_reference_line_s();
    AINFO << path_decision->main_stop().DebugString();
#endif

    return Status::OK();
}

// path只对静态障碍物做decision,dynamic obstacle 不做decision
        ...
        PathDecision *const path_decision)
{
    ...
    for (const auto *obstacle : path_decision->obstacles().Items())
    {
        ...
        // - skip decision making for obstacles with IGNORE/STOP decisions
        // already.
                     ...
                     .is_in_path_lane_borrow_scenario())
        {
            ...
            *object_decision.mutable_stop() =
            ...
            path_decision->AddLongitudinalDecision(
                    "PathDecider/blocking_obstacle", obstacle->Id(),
                    object_decision);
            continue;
        }
        ...
        {
            path_decision->AddLongitudinalDecision(
                    "PathDecider/not-in-s", obstacle->Id(), object_decision);
                 ...
                 sl_boundary.start_l() <= curr_l + min_nudge_l)
        {
            *object_decision.mutable_stop() =
            ...
            {
                path_decision->AddLongitudinalDecision(
                        "PathDecider/nearest-stop", obstacle->Id(),
                        object_decision);
            }
            else
            {
                ...
                object_decision.mutable_ignore();
                path_decision->AddLongitudinalDecision(
                        "PathDecider/not-nearest-stop", obstacle->Id(),
                        object_decision);
            }
        }
        else
        ...
        const Obstacle &obstacle) const
{
    ...
    const double ego_front_stop_ref_s =
