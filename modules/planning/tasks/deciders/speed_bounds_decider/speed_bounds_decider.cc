...
#include "modules/planning/tasks/deciders/speed_bounds_decider/prior_decision.h"
...
#define debug_speed_bounds_decider (0)
#define speed_bounds_decider_time_profile (1)
    ...
    Decider(config, injector)
{
    ...
    if (Name().find("SPEED_BOUNDS_PRIORI_DECIDER") != std::string::npos)
    {
        decision_stage_ =
                speed_bounds_decider_stage::SPEED_BOUNDS_PRIORI_DECISION;
    }
    else
    {
        decision_stage_ =
                speed_bounds_decider_stage::SPEED_BOUNDS_POSTERIOR_DECISION;
    }
}
                                   ...
                                   ReferenceLineInfo *const reference_line_info)
{
    ...
    STBoundaryMapper boundary_mapper(
            ...
            speed_bounds_config_.total_time(), injector_);

    // 使用基于规则的方法，得到st boundary。默认0, 会删除st boundary
    // 不使用st bounds decider生成的boundary
    ...
    // 先验决策
    make_prior_object_decision(reference_line_info, path_decision);
...
#if speed_bounds_decider_time_profile
    ...
    for (auto *obstacle : path_decision->obstacles().Items())
    {
        ...
        if (!st_boundary.IsEmpty())
        {
            ...
            {
                path_decision->Find(id)->set_keep_clear(true);
            }
            else
            {
                path_decision->Find(id)->set_keep_clear(false);
            }
            boundaries.push_back(&st_boundary);
        }
    }

    // 如果只有同向obs， min s是obs的;
    // 如果有逆向obs，而逆向obs交汇点更近，return 0。这样会不会brake too much?
    const double min_s_on_st_boundaries =
            SetSpeedFallbackDistance(path_decision);
    ...
    if (!speed_limit_decider
                 .GetSpeedLimits(path_decision->obstacles(), &speed_limit,
                                 reference_line_info->get_max_speed())
                 .ok())
    {
    ...
    // 平滑限速
    speed_limit_decider.smooth_speed_limit(&speed_limit, init_point.v());
        ...
        PathDecision *const path_decision)
{
    ...
    for (auto *obstacle : path_decision->obstacles().Items())
    {
        ...
        const auto lowest_s =
                std::min(left_bottom_point_s, right_bottom_point_s);
...
#if debug_speed_bounds_decider
    AINFO << Name();
    if (decision_stage_ ==
        speed_bounds_decider_stage::SPEED_BOUNDS_PRIORI_DECISION)
    {
        AINFO << "decision stage: prior ";
    }
    else
    {
        AINFO << "decision stage: posterior ";
    }
    ...
    for (const auto &boundary : st_graph_data.st_boundaries())
    {
        ...
        switch (boundary->boundary_type())
        {
            case STBoundary::BoundaryType::FOLLOW:
                ...
                boundary_debug->set_distance_s(
                        -boundary->characteristic_length());
                break;
