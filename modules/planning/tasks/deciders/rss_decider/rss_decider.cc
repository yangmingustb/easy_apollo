...
Status RssDecider::Process(Frame *frame, ReferenceLineInfo *reference_line_info)
{
    ...
    rss_result = RssResponse.provideProperResponse(responseStateVector,
                                                   properResponse);
    ...
    rss_result =
            ::ad_rss::situation::calculateSafeLongitudinalDistanceSameDirection(
                    leadingVehicleState, followingVehicleState,
                    safeLonDistance);
    if (!rss_result)
    {
        rss_world_info.err_code =
                ("ad_rss::calculateSafeLongitudinalDistanceSameDirection "
                 "failed");
        rss_dump_world_info(rss_world_info);
        return Status(ErrorCode::PLANNING_ERROR, rss_world_info.err_code);
    }
