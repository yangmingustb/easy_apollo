...
Status OpenSpaceFallbackDecider::Process(Frame* frame)
{
                ...
                &first_collision_index))
    {
        ...
        *(frame_->mutable_open_space_info()->mutable_future_collision_point()) =
        ...
        const double vehicle_max_acc =
                4.0;  // vehicle_config.max_acceleration();
        ...
        {
            stop_deceleration = std::min(fallback_start_point.v() *
        ...
        {
            stop_deceleration = std::max(-fallback_start_point.v() *
                                                 fallback_start_point.v() /
                                                 (2.0 * (stop_distance + 1e-6)),
                                         vehicle_max_dec);
        ...
        {
            if (std::abs(fallback_trajectory_pair_candidate.first[i]
                                 .path_point()
                                 .s()) >=
        ...
        for (size_t i = 0; i < fallback_start_index; ++i)
        {
            fallback_trajectory_pair_candidate.first[i].set_v(
                    fallback_start_point.v());
            fallback_trajectory_pair_candidate.first[i].set_a(
                    stop_deceleration);
        }
        ...
        {
            // 1. Set fallback start speed to 0, acceleration to max
            // acceleration.
            ...
            fallback_trajectory_pair_candidate.first.erase(
                    fallback_trajectory_pair_candidate.first.begin() +
                            stop_index + 1,
        ...
        for (size_t i = fallback_start_index; i <= stop_index; ++i)
        {
            ...
            double temp_v = 0.0;
            double c = -2.0 * fallback_trajectory_pair_candidate.first[i]
                                      .path_point()
                                      .s();

            if (QuardraticFormulaLowerSolution(stop_deceleration,
                                               2.0 * fallback_start_point.v(),
                                               c, &new_relative_time) &&
                std::abs(fallback_trajectory_pair_candidate.first[i]
                                 .path_point()
                                 .s()) <= std::abs(stop_distance))
            {
                ADEBUG << "new_relative_time" << new_relative_time;
                temp_v = fallback_start_point.v() +
                         stop_deceleration * new_relative_time;
                // speed limit
            ...
            else
            {
                ...
                if (i != 0)
                {
                    fallback_trajectory_pair_candidate.first[i]
                            .mutable_path_point()
                            ->CopyFrom(fallback_trajectory_pair_candidate
                                               .first[i - 1]
                                               .path_point());
        ...
        // 2. Erase afterwards
        fallback_trajectory_pair_candidate.first.erase(
                fallback_trajectory_pair_candidate.first.begin() + stop_index +
                        1,
                fallback_trajectory_pair_candidate.first.end());
