...
Status OpenSpaceRoiDecider::Process(Frame *frame)
{
    ...
    if (roi_type == OpenSpaceRoiDeciderConfig::PARKING)
    {
        ...
        if (!GetParkingBoundary(frame, spot_vertices, nearby_path,
                                &roi_boundary))
        {
            const std::string msg = "Fail to get parking boundary from map";
            AERROR << msg;
    ...
    else if (roi_type == OpenSpaceRoiDeciderConfig::DEAD_END)
    {
        ...
        size_t waypoint_num = frame->local_view()
                                      .routing->routing_request()
                                      .waypoint()
                                      .size();
        ...
        if (!SelectTargetDeadEndJunction(&junctions, dead_end_point_,
                                         &junction))
        {
        ...
        if (!GetDeadEndBoundary(frame, dead_end_vertices, nearby_path,
                                &roi_boundary))
        {
    ...
    else if (roi_type == OpenSpaceRoiDeciderConfig::PARK_AND_GO)
    {
        ...
        nearby_path = frame->reference_line_info()
                              .front()
                              .reference_line()
                              .GetMapPath();
        ...
        Frame *const frame, const std::array<common::math::Vec2d, 4> &vertices)
{
    ...
    // TODO(Jinyun): adjust end pose setting for more parking spot
    // configurations
...
{
    double start_s = center_line_s - config_.open_space_roi_decider_config()
                                             .roi_longitudinal_range_start();
    double end_s = center_line_s + config_.open_space_roi_decider_config()
    ...
    while (check_point_s <= end_s)
    {
        ...
        bool is_center_lane_heading_change =
                std::abs(common::math::NormalizeAngle(
                        check_point_heading - last_check_point_heading)) >
                config_.open_space_roi_decider_config()
        ...
        // point.
        bool is_anchor_point = check_point_s == start_s ||
                               check_point_s == end_s ||
                               is_center_lane_heading_change;
        ...
        index += 1.0;
        check_point_s =
                start_s + index * config_.open_space_roi_decider_config()
        ...
        std::vector<double> *right_lane_road_width)
{
    ...
    while (check_point_s <= end_s)
    {
        ...
        {
            for (size_t i = 0;
                 i < (*road_boundaries.at(0)).left_boundary.line_points.size();
                 i++)
            {
                right_lane_boundary->emplace_back(
                        Vec2d((*road_boundaries.at(0))
                                      .left_boundary.line_points[i]
                                      .x(),
                              (*road_boundaries.at(0))
                                      .left_boundary.line_points[i]
                                      .y()));
            }
            for (size_t i = 0;
        ...
        check_point_s =
                check_point_s + config_.open_space_roi_decider_config()
    ...
    if (!left_lane_boundary->empty())
    {
        ...
        auto unique_end = std::unique(left_lane_boundary->begin(),
    ...
    if (!right_lane_boundary->empty())
    {
        ...
        auto unique_end = std::unique(right_lane_boundary->begin(),
        ...
        std::vector<double> *center_lane_s, std::vector<double> *road_width)
{
    ...
    double next_road_width = is_left_curb
                                     ? nearby_path.GetRoadLeftWidth(
                                               check_point_s + next_distance_s)
                                     : nearby_path.GetRoadRightWidth(
                                               check_point_s + next_distance_s);
        ...
        std::vector<PointENU> *In_right_boundary_points)
{
    ...
    {
        In_right_boundary_points->push_back(
                right_points[temp_record_right - 1]);
    }
        ...
        std::vector<PointENU> *Out_right_boundary_points)
{
    ...
    {
        Out_right_boundary_points->push_back(
                right_points[temp_record_right + 1]);
    }
                ...
                *const roi_deadend_boundary)
{
    ...
    routing_target_point_ = frame->local_view()
                                    .routing->routing_request()
                                    .dead_end_info()
                                    .target_point();
    if (routing_in_flag_)
    {
        routing_end_point_ = frame->local_view()
                                     .routing->routing_request()
                                     .waypoint()
                                     .at(waypoint_num - 1)
                                     .pose();
    ...
    hdmap_->GetNearestLane(routing_end_point_, &start_nearest_lane, &nearest_s,
                           &nearest_l);
    ...
    hdmap_->GetNearestLane(routing_target_point_, &end_nearest_lane, &nearest_s,
                           &nearest_l);
    GetOutLaneStartPoint(end_nearest_lane, &left_start_point,
                         &right_start_point);
    ...
    std::vector<PointENU> In_left_boundary_points;
    std::vector<PointENU> In_right_boundary_points;
    GetInLaneBoundaryPoints(car_lane, nearby_path, &In_left_boundary_points,
    ...
    hdmap_->GetNearestLane(routing_target_point_, &target_lane, &nearest_s,
                           &nearest_l);
    std::vector<PointENU> Out_left_boundary_points;
    std::vector<PointENU> Out_right_boundary_points;
    GetOutLaneBoundaryPoints(target_lane, nearby_path,
    ...
    point_boundary.push_back(
            {In_right_boundary_points[0].x(), In_right_boundary_points[0].y()});
    const auto &origin_point = frame->open_space_info().origin_point();
    const auto &origin_heading = frame->open_space_info().origin_heading();
                ...
                *const roi_parking_boundary)
{
    ...
    {
        AERROR << "fail to get parking spot points' projections on reference "
                  "line";
        return false;
    }
    ...
    if (average_l < 0)
    {
        ...
        auto point_left_to_left_top_connor_s =
                std::lower_bound(center_lane_s_right.begin(),
                                 center_lane_s_right.end(), left_top_s);
        ...
        auto point_left_to_left_top_connor_itr =
                right_lane_boundary.begin() +
                point_left_to_left_top_connor_index;
        auto point_right_to_right_top_connor_s =
                std::upper_bound(center_lane_s_right.begin(),
                                 center_lane_s_right.end(), right_top_s);
        ...
        std::copy(right_lane_boundary.begin(),
                  point_left_to_left_top_connor_itr,
                  std::back_inserter(boundary_points));

        std::vector<Vec2d> parking_spot_boundary{left_top, left_down,
                                                 right_down, right_top};

        std::copy(parking_spot_boundary.begin(), parking_spot_boundary.end(),
                  std::back_inserter(boundary_points));

        std::copy(point_right_to_right_top_connor_itr,
                  right_lane_boundary.end(),
                  std::back_inserter(boundary_points));
        ...
        std::vector<Vec2d> left_stitching_segment{
                right_lane_boundary[point_left_to_left_top_connor_index],
                left_top};
        ...
        std::vector<Vec2d> right_stitching_segment{
                right_top,
                right_lane_boundary[point_right_to_right_top_connor_index]};
    ...
    else
    {
        ...
        for (size_t i = 0; i < point_size; i++)
        {
            ...
            left_lane_boundary[i] -= origin_point;
            left_lane_boundary[i].SelfRotate(-origin_heading);
            ADEBUG << "left_lane_boundary[" << i
                   << "]: " << left_lane_boundary[i].x() << ", "
                   << left_lane_boundary[i].y();
        }

        auto point_right_to_right_top_connor_s =
                std::lower_bound(center_lane_s_left.begin(),
                                 center_lane_s_left.end(), right_top_s);
        size_t point_right_to_right_top_connor_index = std::distance(
        ...
        auto point_right_to_right_top_connor_itr =
                left_lane_boundary.begin() +
                point_right_to_right_top_connor_index;

        auto point_left_to_left_top_connor_s =
                std::upper_bound(center_lane_s_left.begin(),
                                 center_lane_s_left.end(), left_top_s);
        size_t point_left_to_left_top_connor_index = std::distance(
        ...
        std::vector<Vec2d> left_stitching_segment{
                left_lane_boundary[point_left_to_left_top_connor_index],
                left_top};
        roi_parking_boundary->push_back(left_stitching_segment);
        ...
        std::vector<Vec2d> right_stitching_segment{
                right_top,
                left_lane_boundary[point_right_to_right_top_connor_index]};
        roi_parking_boundary->push_back(right_stitching_segment);
                ...
                *const roi_parking_boundary)
{
    ...
    {
        GetRoadBoundaryFromMap(nearby_path, center_line_s, origin_point,
                               origin_heading, &left_lane_boundary,
                               &right_lane_boundary, &center_lane_boundary_left,
...
bool OpenSpaceRoiDecider::GetDeadEndSpot(Frame *const frame,
                                         JunctionInfoConstPtr *junction,
                                         std::vector<Vec2d> *dead_end_vertices)
{
    if (frame == nullptr)
                                         ...
                                         Path *nearby_path)
{
            ...
            if (succeed_lanes_num != 0)
            {
                for (size_t j = 0; j < succeed_lanes_num; j++)
                {
                    ...
                    LaneSegment succeed_lanesegment = LaneSegment(
                            succeed_lane, succeed_lane->accumulate_s().front(),
    ...
    {
        AERROR << "target parking spot found, but too far, distance larger "
                  "than "
        ...
        const hdmap::ParkingSpaceInfoConstPtr &target_parking_spot)
{
    ...
    nearby_path.GetNearestPoint(vehicle_vec, &vehicle_point_s,
                                &vehicle_point_l);
        ...
        std::vector<std::vector<common::math::Vec2d>> *line_segments_vec)
{
    ...
    while (cur_segment != line_segments_vec->end() - 1)
    {
        ...
        if (CrossProd(cur_second_to_last_point, cur_last_point,
                      next_second_point) < 0.0)
        {
            cur_segment->push_back(next_second_point);
            next_segment->erase(next_segment->begin(),
                                next_segment->begin() + 2);
        ...
        Frame *const frame)
{
    ...
    if (config_.open_space_roi_decider_config().enable_perception_obstacles())
    {
        ...
        for (const auto &obstacle : obstacles_by_frame_->Items())
        {
            ...
            // vertice is repeated at the end of the vector to help transform
            // all four edges to inequality constraint
