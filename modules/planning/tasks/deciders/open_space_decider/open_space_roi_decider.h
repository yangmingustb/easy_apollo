...
private:
    ...
    // @brief generate the path by vehicle location and return the target
    // parking spot on that path
    ...
    void AddBoundaryKeyPoint(
            ...
            const bool is_anchor_point, const bool is_left_curb,
    ...
    bool GetParkAndGoBoundary(Frame *const frame,
                              const hdmap::Path &nearby_path,
                              std::vector<std::vector<common::math::Vec2d>>
                                      *const roi_parking_boundary);
