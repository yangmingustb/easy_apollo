                                     ...
                                     ReferenceLineInfo *reference_line_info)
{
        ...
        frame->reference_line_info().size() > 1)
    {
        ...
        ADEBUG << "valid_path_reference_counter["
               << valid_path_reference_counter_ << "] total_path_counter["
               << total_path_counter_ << "]";
    ...
    ADEBUG << "There are " << learning_model_trajectory.size()
           << " path points.";
    ...
    ConvertTrajectoryToPath(stitched_learning_model_trajectory,
                            &path_reference);
    ...
    reference_line_info->mutable_debug()
            ...
            ->mutable_path_point()
            ->CopyFrom({evaluated_path_reference.begin(),
        ...
        const std::vector<PathPoint> &path_reference)
{
    for (auto path_referece_point : path_reference)
    {
        ...
        if (-1 == IsPointWithinPathBounds(reference_line_info,
                                          regular_path_bound, cur_x, cur_y))
        {
            ...
            for (auto vehicle_box : vehicle_boxes)
            {
                if (vehicle_box.HasOverlap(line_segment))
                {
                    ADEBUG << std::setprecision(9) << "Vehicle box:["
                           << vehicle_box.center_x() << ","
                           << vehicle_box.center_y() << "]"
                           << "Violate path bound at ["
                           << line_segment.start().x() << ","
        ...
        ReferenceLineInfo *const reference_line_info)
{
    ...
    auto *ptr_display_path = reference_line_info->mutable_debug()
                                     ->mutable_planning_data()
                                     ->add_path();
