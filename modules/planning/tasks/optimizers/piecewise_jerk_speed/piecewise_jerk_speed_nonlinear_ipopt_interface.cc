                                                            ...
                                                            double *grad_f)
{
    ...
    {
        grad_f[a_offset_ + i] +=
                c * (2.0 * x[a_offset_ + i] - x[a_offset_ + i + 1] -
                     x[a_offset_ + i - 1]);
    }
    grad_f[a_offset_ + num_of_points_ - 1] +=
            c * (x[a_offset_ + num_of_points_ - 1] -
                 x[a_offset_ + num_of_points_ - 2]);
        ...
        int *jCol, double *values)
{
    if (values == nullptr)
    {
        ...
        // jerk bound constraint, |s_ddot_i+1 - s_ddot_i| / delta_t <=
        // s_dddot_max
        for (int i = 0; i + 1 < num_of_points_; ++i)
        {
            // a_i
        ...
        int *jCol, double *values)
{
    ...
    else
    {
        ...
        for (int i = 0; i < num_of_points_; ++i)
        {
            ...
            auto h_s_s_obj = 2.0 * kappa_dot * kappa_dot * v4 +
                             2.0 * kappa * kappa_ddot * v4;
        ...
        if (use_v_bound_)
        {
            ...
            for (int i = 0; i < num_of_points_; ++i)
            {
                ...
                auto h_s_s_index =
                        hessian_mapper_[to_hash_key(s_index, s_index)];
        ...
        for (int i = 0; i + 1 < num_of_points_; ++i)
        {
            ...
            auto h_a0_a0_index =
                    hessian_mapper_[to_hash_key(a0_index, a0_index)];
            values[h_a0_a0_index] += c;

            auto h_a0_a1_index =
                    hessian_mapper_[to_hash_key(a0_index, a1_index)];
            values[h_a0_a1_index] += -c;

            auto h_a1_a1_index =
                    hessian_mapper_[to_hash_key(a1_index, a1_index)];
        ...
        Ipopt::IpoptCalculatedQuantities *ip_cq)
{
    ...
    if (use_soft_safety_bound_)
    {
        // statistic analysis on soft bound intrusion by inspecting slack
        // variable
        double lower_s_mean_intrusion = 0.0;
        double lower_s_highest_intrusion =
                -std::numeric_limits<double>::infinity();
        int lower_s_highest_intrustion_index = 0.0;
        double upper_s_mean_intrusion = 0.0;
        double upper_s_highest_intrusion =
                -std::numeric_limits<double>::infinity();
        ...
        ADEBUG << "lower soft s boundary average intrustion is ["
               ...
               << delta_t_ *
                          static_cast<double>(lower_s_highest_intrustion_index)
               << "].";
        ADEBUG << "upper soft s boundary average intrustion is ["
               ...
               << delta_t_ *
                          static_cast<double>(upper_s_highest_intrustion_index)
               << "].";
    }
}
...
void PiecewiseJerkSpeedNonlinearIpoptInterface::set_w_target_state(
        const double w_target_s, const double w_target_v,
