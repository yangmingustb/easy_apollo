    ...
    TrajectoryOptimizer(config, injector)
{
    ...
    heading_offset_to_midpoint_ = open_space_trajectory_partition_config_
                                          .heading_offset_to_midpoint();
    lateral_offset_to_midpoint_ = open_space_trajectory_partition_config_
                                          .lateral_offset_to_midpoint();
    longitudinal_offset_to_midpoint_ =
            open_space_trajectory_partition_config_
                    .longitudinal_offset_to_midpoint();
...
Status OpenSpaceTrajectoryPartition::Process()
{
    ...
    for (size_t i = 0; i < trajectories_size; ++i)
    {
        ...
        flag_change_to_next = CheckReachTrajectoryEnd(
                trajectory, gear, trajectories_size, i,
                &current_trajectory_index, &current_trajectory_point_index);
        ...
        for (size_t j = 0; j < trajectory_size; ++j)
        {
            ...
            const Vec2d tracking_vector(path_point_x - ego_x_,
                                        path_point_y - ego_y_);
            ...
            const double traj_point_moving_direction =
                    gear == canbus::Chassis::GEAR_REVERSE
                            ...
                            : path_point_theta;
            const double head_track_difference = std::abs(NormalizeAngle(
                    tracking_direction - vehicle_moving_direction_));
                ...
                head_track_difference < heading_track_range_)
            {
                ...
                Box2d path_point_box({path_point_x, path_point_y},
                                     path_point_theta, ego_length_, ego_width_);
                ...
                double iou_ratio = Polygon2d(ego_box_).ComputeIoU(
                        Polygon2d(path_point_box));
    ...
    if (!flag_change_to_next)
    {
        ...
        {
            if (!UseFailSafeSearch(*partitioned_trajectories,
    ...
    {
        if (InsertGearShiftTrajectory(
                    flag_change_to_next, current_trajectory_index,
        ...
        for (size_t j = 0; j < interpolated_points_num; ++j)
        {
            ...
            interpolated_trajectory->emplace_back(
                    common::math::InterpolateUsingLinearApproximation(
                            ...
                            stitched_trajectory_result.at(i + 1),
                            relative_time));
        }
    }
        ...
        const DiscretizedTrajectory& trajectory, std::string* const encoding)
{
    ...
    const int init_point_x = static_cast<int>(
            (init_path_point.x() - encoding_origin_x) * 1000.0);
    const int init_point_y = static_cast<int>(
            (init_path_point.y() - encoding_origin_y) * 1000.0);
    const int init_point_heading =
            static_cast<int>(init_path_point.theta() * 10000.0);
    const int last_point_x = static_cast<int>(
            (last_path_point.x() - encoding_origin_x) * 1000.0);
    const int last_point_y = static_cast<int>(
            (last_path_point.y() - encoding_origin_y) * 1000.0);
        ...
        std::vector<TrajGearPair>* partitioned_trajectories)
{
    ...
    *gear = std::abs(common::math::NormalizeAngle(tracking_angle -
    ...
    for (size_t i = 0; i < horizon - 1; ++i)
    {
        ...
        if (cur_gear != *gear)
        {
            ...
            LoadTrajectoryPoint(trajectory_point, is_trajectory_last_point,
                                *gear, &last_pos_vec, &distance_s, trajectory);
            partitioned_trajectories->emplace_back();
...
bool OpenSpaceTrajectoryPartition::CheckReachTrajectoryEnd(
        ...
        const size_t trajectories_size, const size_t trajectories_index,
...
{
    const size_t partitioned_trajectories_size =
            partitioned_trajectories.size();
    ...
    *(current_partitioned_trajectory) =
    ...
    s_shift = unpartitioned_trajectory_result->at(index_estimate)
                      .path_point()
                      .s();
