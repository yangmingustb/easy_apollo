        ...
        PathData* const final_path_data)
{
...
#if debug_pwj_path_optimizer
    AINFO << "s state: " << init_frenet_state.first[0] << ", "
          << init_frenet_state.first[1] << ", " << init_frenet_state.first[2];
    AINFO << "l state: " << init_frenet_state.second[0] << ", "
    ...
    const auto& config =
            reference_line_info_->is_change_lane_ref_line()
    ...
    std::array<double, 5> w = {
            config.l_weight(),
            config.dl_weight() * std::fmax(init_frenet_state.first[1] *
...
#if debug_pwj_path_optimizer
    AINFO << "There are " << path_boundaries.size() << " path boundaries.";

    for (const auto& path_boundary : path_boundaries)
    {
        AINFO << "bound name: " << path_boundary.label();
    }
    ...
    AINFO << reference_path_data.DebugString();
#endif
    ...
    for (const auto& path_boundary : path_boundaries)
    {
        ...
        // if the path_boundary is normal, it is possible to have less than 2
        // points skip path boundary of this kind
        if (path_boundary.label().find("regular") != std::string::npos &&
            path_boundary_size < 2)
        {
            AINFO << "bound point size is small";
            continue;
        }
        ...
        {
            AERROR << "path boundary size is less 2";
...
#if debug_pwj_path_optimizer
        AINFO << "end state: " << end_state[0] << ", " << end_state[1] << ", "
              << end_state[2];
#endif
        ...
        if (!FLAGS_enable_force_pull_over_open_space_parking_test)
        {
            ...
            const auto& pull_over_status = injector_->planning_context()
                                                   ->planning_status()
                                                   .pull_over();
                ...
                path_boundary.label().find("pullover") != std::string::npos)
            {
                ...
                reference_line.XYToSL(pull_over_status.position(),
                                      &pull_over_sl);
        ...
        size_t path_reference_size;

        // 学习类算法，才使用
        {
            path_reference_size = reference_path_data.path_reference().size();

            if (path_boundary.label().find("regular") != std::string::npos &&
                reference_path_data.is_valid_path_reference())
            {
                update_path_reference_by_learning_base(
                        path_boundary, path_reference_size, reference_line,
                        reference_path_data, path_reference_l);
        ...
        const LaneBorrowStatus& lane_borrow_state =
        ...
        //  manual lane borrow
        {
            ...
            {
                if (path_boundary.type_ == PATH_BOUND_LANE_BORROW_LEFT ||
                    path_boundary.type_ == PATH_BOUND_LANE_BORROW_RIGHT)
                {
                    // when path reference is ready
                    double target_offset = path_boundary.get_target_offset();

                    for (size_t i = 0; i < path_boundary_size; ++i)
                    {
                        path_reference_l[i] = target_offset;
                    }

                    end_state[0] = path_reference_l.back();

                    is_valid_path_reference = true;
                    path_reference_size = path_boundary_size;
                }
            }
        }
        ...
        // ref: https://zhuanlan.zhihu.com/p/514864431?utm_id=0
        // 假设: delta_theta =0, k_r=0， 那么可以计算ddl近似bound
        // 这种近似计算引入了bug
        ...
        for (size_t i = 0; i < path_boundary_size; ++i)
        {
            ...
            ddl_bounds.emplace_back(-lat_acc_bound - kappa,
                                    lat_acc_bound - kappa);
        }
        ...
        bool res_opt = OptimizePath(
                ...
                is_valid_path_reference, path_boundary.boundary(), ddl_bounds,
                w, max_iter, &opt_l, &opt_dl, &opt_ddl);
        ...
        if (res_opt)
        {
            ...
            {
                AINFO << "for s["
                       << static_cast<double>(i) * path_boundary.delta_s()
            ...
            path_data.type_ = path_boundary.type_;

            AINFO << "blocking obs id is: "
                  << path_boundary.blocking_obstacle_id()
                  << ", path opt success, label name is: "
                  << path_boundary.label();
        ...
        {
            AINFO << "path optimizer fail, label is: " << path_boundary.label();
        }
    }
        ...
        std::vector<double>* ddx)
{
    ...
    if (is_valid_path_reference)
    {
        ...
        std::vector<double> weight_x_ref_vec(kNumKnots, 10.0);
        ...
        //             GaussianWeighting(x, peak_value, peak_value_x);
    ...
    // https://zhuanlan.zhihu.com/p/598926237?utm_id=0
    ...
    double vehicle_speed = std::fmax(init_state.first[1], 1.0);

    double jerk_bound =
            EstimateJerkBoundary(vehicle_speed, axis_distance, max_yaw_rate);

    // 这种近似计算出来的jerk boundary非常小，一般是lower, upper: -0.00328933,
    // 0.00328933。导致非常容易出现无解
    double k_jerk_bound_min = 0.05;

    jerk_bound = std::max(k_jerk_bound_min, jerk_bound);
        ...
        const double peak_weighting_x) const
{
    ...
    ADEBUG << peak_weighting * exp(-0.5 * (x - peak_weighting_x) *
                                   (x - peak_weighting_x));
    ADEBUG << Gaussian(u, std, x_updated);
    return Gaussian(u, std, x_updated);
}

int PiecewiseJerkPathOptimizer::update_path_reference_by_learning_base(
        const PathBoundary& path_boundary, size_t path_reference_size,
        const ReferenceLine& reference_line,
        const PathData& reference_path_data,
        std::vector<double>& path_reference_l)
{
    AINFO << "path label is: " << path_boundary.label();
