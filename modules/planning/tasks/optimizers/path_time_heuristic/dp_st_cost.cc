...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point)
{
    ...
    for (const auto* obstacle : obstacles_)
    {
        ...
        // Stop obstacles are assumed to have a safety margin when mapping them
        // out, so repelling force in dp st is not needed as it is designed to
        // have adc stop right at the stop distance we design in prior mapping
        // process
        // 停车决策的障碍物，没有cost
        ...
        // 后方障碍物，如果自车有路权，ignore
        // ego没有路权，要避让。暂时只考虑lane following scenario, so ego 有路权
        //
        if (obstacle->PerceptionSLBoundary().end_s() < adc_sl_bound_.start_s())
        {
            continue;
        }
        ...
        // overtake, follow有cost?
        // 碰撞，cost是inf
        // dp之前，decision不一定完成，可以借助最后的cost来决策
        ...
        {
            s_upper = boundary_cost_[boundary_index][st_graph_point.index_t()]
                              .first;
            s_lower = boundary_cost_[boundary_index][st_graph_point.index_t()]
                              .second;
        }
...
double DpStCost::get_speed_cost(const double cur_v, const STPoint& first,
                                const STPoint& second, const double speed_limit,
                                const double cruise_speed) const
{
    double cost = 0.0;
    const double speed = cur_v;
    if (speed < 0)
    {
        return kInf;
    }
...
double DpStCost::GetAccelByTwoPoint(const double left_point_speed,
                                    const common::SpeedPoint& left_point,
                                    const common::SpeedPoint& right_point)
{
    double current_speed = (right_point.s() - left_point.s()) / unit_t_;
    double acc = (current_speed - left_point_speed) / unit_t_;
    return acc;
}
...
double DpStCost::GetJerkByTwoPoint(const double left_point_speed,
                                   const double left_point_acc,
                                   const common::SpeedPoint& left_point,
                                   const common::SpeedPoint& right_point)
{
    const double curr_speed = (right_point.s() - left_point.s()) / unit_t_;
    const double curr_acc = (curr_speed - left_point_speed) / unit_t_;
    const double jerk = (curr_acc - left_point_acc) / unit_t_;
    return jerk;
}
