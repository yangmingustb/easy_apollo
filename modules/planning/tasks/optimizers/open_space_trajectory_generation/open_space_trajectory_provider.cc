...
Status OpenSpaceTrajectoryProvider::Process()
{
    ...
    auto trajectory_data = frame_->mutable_open_space_info()
    ...
    if (FLAGS_enable_open_space_planner_thread)
    {
        ...
        if (trajectory_error_)
        {
            ...
            // smoothing result has out of bound pathpoint which is not allowed
            // for next iteration hybrid astar algorithm which requires start
            // position to be strictly in bound
        ...
        else
        {
            GenerateStopTrajectory(trajectory_data);
            return Status(ErrorCode::OK,
                          "Stop due to computation not finished");
        }
    }
    else
    {
    ...
    while (!is_generation_thread_stop_)
    {
        if (!trajectory_updated_ && data_ready_)
        {
            ...
            Status status = open_space_trajectory_optimizer_->Plan(
                    ...
                    thread_data.translate_origin,
                    thread_data.obstacles_edges_num, thread_data.obstacles_A,
                    thread_data.obstacles_b, thread_data.obstacles_vertices_vec,
                    &time_latency);
            frame_->mutable_open_space_info()->set_time_latency(time_latency);
            if (status == Status::OK())
            {
        ...
        const Vec2d& translate_origin)
{
    ...
    double distance_to_vehicle2 = std::sqrt(
    ...
    double distance_to_vehicle1 =
            std::sqrt((vehicle_state.x() - end_pose[0]) *
                      ...
                      (vehicle_state.y() - end_pose[1]) *
                              (vehicle_state.y() - end_pose[1]));
    double distance_to_vehicle = std::sqrt(
            (vehicle_state.x() - end_pose_to_world_frame.x()) *
                    (vehicle_state.x() - end_pose_to_world_frame.x()) +
            (vehicle_state.y() - end_pose_to_world_frame.y()) *
                    (vehicle_state.y() - end_pose_to_world_frame.y()));
    ...
    AERROR << "theta_to_vehicle" << theta_to_vehicle
