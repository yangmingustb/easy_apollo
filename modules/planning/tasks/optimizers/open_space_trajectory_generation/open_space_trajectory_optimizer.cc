    ...
    config_(config)
{
    ...
    dual_variable_warm_start_.reset(new DualVariableWarmStartProblem(
            config.planner_open_space_config()));
        ...
        double* time_latency)
{
    ...
    if (FLAGS_enable_parallel_trajectory_smoothing)
    {
        ...
        {
            LoadHybridAstarResultInEigen(&partition_trajectories[i],
                                         &xWS_vec[i], &uWS_vec[i]);
            ...
            // trajectories. In the next ones, control and velocity are assumed
            // to be zero as the next trajectories always start from vehicle
            // static state
            if (i == 0)
            {
            ...
            const auto smoother_start_timestamp =
            ...
            if (FLAGS_use_iterative_anchoring_smoother)
            {
                            ...
                            &control_result_ds_vec[i], &time_result_ds_vec[i]))
                {
                    ...
                    ADEBUG << i << "th trajectory size is "
                           << xWS_vec[i].cols();
                    return Status(ErrorCode::PLANNING_ERROR,
                                  "iterative anchoring smoothing problem "
                                  "failed to solve");
                }
            }
            else
            {
                ...
                if (!GenerateDistanceApproachTraj(
                            xWS_vec[i], uWS_vec[i], XYbounds,
                            obstacles_edges_num, obstacles_A, obstacles_b,
                            obstacles_vertices_vec, last_time_u, init_v,
                            &state_result_ds_vec[i], &control_result_ds_vec[i],
                            &time_result_ds_vec[i], &l_warm_up_vec[i],
                            &n_warm_up_vec[i], &dual_l_result_ds_vec[i],
                            &dual_n_result_ds_vec[i]))
                {
                    ...
                    return Status(ErrorCode::PLANNING_ERROR,
                                  "distance approach smoothing problem failed "
                                  "to solve");
                }
        ...
        ADEBUG << i << "th smoothed trajectory size is "
            ...
            const auto smoother_end_timestamp =
                    std::chrono::system_clock::now();
    ...
    else
    {
        ...
        if (!GenerateDistanceApproachTraj(
                    xWS, uWS, XYbounds, obstacles_edges_num, obstacles_A,
                    obstacles_b, obstacles_vertices_vec, last_time_u, init_v,
                    &state_result_ds, &control_result_ds, &time_result_ds,
                    &l_warm_up, &n_warm_up, &dual_l_result_ds,
                    &dual_n_result_ds))
        {
            return Status(
    ...
    if (FLAGS_enable_record_debug)
    {
        ...
        RecordDebugInfo(trajectory_stitching_point, translate_origin,
                        rotate_angle, end_pose, xWS, uWS, l_warm_up, n_warm_up,
    ...
    {
        PathPointDeNormalizing(
                rotate_angle, translate_origin, &(state_result_ds(0, i)),
                &(state_result_ds(1, i)), &(state_result_ds(2, i)));
    }
        ...
        const std::vector<std::vector<Vec2d>>& obstacles_vertices_vec)
{
    ...
    {
        auto* warm_start_point =
                warm_start_trajectory->add_vehicle_motion_point();
    ...
    for (size_t i = 0; i < horizon; ++i)
    {
        ...
        smoothed_point->mutable_trajectory_point()->mutable_path_point()->set_y(
                state_result_ds(1, i));
        smoothed_point->mutable_trajectory_point()
                ->mutable_path_point()
                ->set_theta(state_result_ds(2, i));
...
void OpenSpaceTrajectoryOptimizer::PathPointNormalizing(
        double rotate_angle, const Vec2d& translate_origin, double* x,
        double* y, double* phi)
{
...
void OpenSpaceTrajectoryOptimizer::LoadTrajectory(
        const Eigen::MatrixXd& state_result,
        ...
        Eigen::MatrixXd* dual_n_result_ds)
{
    ...
    *time_result_ds = Eigen::MatrixXd::Constant(
            1, time_result_horizon,
            config_.planner_open_space_config().delta_t());
}
        ...
        Eigen::MatrixXd* dual_n_result_ds)
{
    ...
    {
        if (dual_variable_warm_start_->Solve(horizon, ts, ego, obstacles_num,
                                             obstacles_edges_num, obstacles_A,
                                             obstacles_b, xWS, l_warm_up,
                                             n_warm_up, &s_warm_up))
        {
    ...
    {
        *l_warm_up = 0.5 * Eigen::MatrixXd::Ones(obstacles_edges_num.sum(),
                                                 horizon + 1);
        *n_warm_up =
                0.5 * Eigen::MatrixXd::Ones(4 * obstacles_num, horizon + 1);
    }

    // Distance approach trajectory smoothing
    if (distance_approach_->Solve(x0, xF, last_time_u, horizon, ts, ego, xWS,
                                  uWS, *l_warm_up, *n_warm_up, s_warm_up,
                                  XYbounds, obstacles_num, obstacles_edges_num,
                                  obstacles_A, obstacles_b, state_result_ds,
    ...
    else
    {
        ...
        {
            UseWarmStartAsResult(xWS, uWS, *l_warm_up, *n_warm_up,
        ...
        Eigen::MatrixXd* dual_n_result_ds)
{
    ...
    Eigen::MatrixXd state_result_ds_ = Eigen::MatrixXd::Zero(
            state_result_ds_vec[0].rows(), smoothed_state_size);
    ...
    Eigen::MatrixXd dual_l_result_ds_ = Eigen::MatrixXd::Zero(
            dual_l_result_ds_vec[0].rows(), l_smoothed_size);
    Eigen::MatrixXd dual_n_result_ds_ = Eigen::MatrixXd::Zero(
            dual_n_result_ds_vec[0].rows(), n_smoothed_size);
    ...
    state_result_ds_.col(counter) = state_result_ds_vec.back().col(
        ...
        Eigen::MatrixXd* time_result_dc)
{
    ...
    if (!iterative_anchoring_smoother_->Smooth(xWS, init_a, init_v,
                                               obstacles_vertices_vec,
                                               &smoothed_trajectory))
    {
        return false;
        ...
        Eigen::MatrixXd* time_result_dc)
{
    ...
    *time_result_dc = Eigen::MatrixXd::Zero(1, points_size - 1);
    ...
    {
        control_result(0, i) = std::atan(
                discretized_trajectory[i].path_point().kappa() * wheel_base);
