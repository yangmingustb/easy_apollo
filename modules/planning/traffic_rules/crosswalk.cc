...
using CrosswalkToStop = std::vector<
        std::pair<const hdmap::PathOverlap*, std::vector<std::string>>>;
                              ...
                              ReferenceLineInfo* const reference_line_info)
{
    ...
    for (auto crosswalk_overlap : crosswalk_overlaps_)
    {
        ...
        for (const auto* obstacle : path_decision->obstacles().Items())
        {
                        ...
                        kStartWatchTimerDistance)
            {
                ...
                {
                    if (crosswalk_stop_timer[crosswalk_id].count(obstacle_id) <
                        1)
                    {
                        // add timestamp
    ...
    for (auto crosswalk_to_stop : crosswalks_to_stop)
    {
        ...
        ADEBUG << "BuildStopDecision: crosswalk["
               << crosswalk_overlap->object_id << "] start_s["
               << crosswalk_overlap->start_s << "]";
        ...
        util::BuildStopDecision(
                ...
                StopReasonCode::STOP_REASON_CROSSWALK, crosswalk_to_stop.second,
        ...
        const double stop_deceleration)
{
    ...
    ADEBUG << "obstacle_id[" << obstacle_id << "] type[" << obstacle_type_name
           ...
           << obstacle_l_distance << "] on_lane[" << is_on_lane
             ...
             config_.crosswalk().stop_strict_l_distance())
    {
        ...
        else
        {
            ...
            else
            {
                ...
                const auto obstacle_v =
                        Vec2d(perception_obstacle.velocity().x(),
                              perception_obstacle.velocity().y());
                const auto adc_path_point = Vec2d(
                        injector_->ego_info()->start_point().path_point().x(),
