                                 ...
                                 ReferenceLineInfo* const reference_line_info)
{
    ...
    AINFO << "refline light size: " << traffic_light_overlaps.size();

    for (const auto& traffic_light_overlap : traffic_light_overlaps)
    {
        AINFO << "traffic_light[" << traffic_light_overlap.object_id
        ...
        if (traffic_light_ptr != nullptr)
        {
            const auto& signal = traffic_light_ptr->signal();

            AINFO << signal.DebugString();
        }
    }
#endif

    // 前方的traffic light need to do stop decision
    // 如果都是绿灯，不需要生成obstacle
    // 如果有多个red light, 生成多个obstacle?
    for (const auto& traffic_light_overlap : traffic_light_overlaps)
    {
        ...
        AINFO << "traffic_light[" << traffic_light_overlap.object_id
               << "] start_s[" << traffic_light_overlap.start_s
               << "] s_distance[" << s_distance << "] actual_distance["
               << distance << "]";

#endif

        // light距离太远，不使用这个light。
        // 这里有bug：实际上，light和car相距30米也可能。未来需要修复
        if (s_distance >= 0 &&
            fabs(s_distance - distance) > kSDiscrepanceTolerance)
        {
#if debug_traffic_light
            AINFO << "SKIP traffic_light[" << traffic_light_overlap.object_id
...
#if debug_traffic_light
        AINFO << "traffic_light_id[" << traffic_light_overlap.object_id
        ...
        // build stop decision
#if debug_traffic_light
        AINFO << "BuildStopDecision: traffic_light["
