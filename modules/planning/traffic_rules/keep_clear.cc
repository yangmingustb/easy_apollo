    ...
    if (config_.keep_clear().enable_junction())
    {
        ...
        for (const auto& overlap : first_encountered_overlaps)
        {
            ...
            switch (overlap.first)
            {
                case ReferenceLineInfo::CROSSWALK:
                    ...
                    crosswalk_overlap =
                            const_cast<PathOverlap*>(&overlap.second);
                    break;
                ...
                case ReferenceLineInfo::PNC_JUNCTION:
                    ADEBUG << "PNC_JUNCTION[" << overlap.second.object_id
                           << "] s[" << overlap.second.start_s << ", "
                           << overlap.second.end_s << "]";
        ...
        if (pnc_junction_overlap != nullptr)
        {
            ...
            if (!IsCreeping(pnc_junction_overlap->start_s, adc_front_edge_s))
            {
                ...
                {
                    ADEBUG << "adjust pnc_junction_start_s["
                ...
                {
                    ADEBUG << "adjust pnc_junction_start_s["
                           << pnc_junction_start_s << "] to stop_sign_start_s"
                           << stop_sign_overlap->start_s << "]";
                ...
                {
                    ADEBUG << "adjust pnc_junction_start_s["
                           << pnc_junction_start_s << "] to cross_walk_start_s"
                           << crosswalk_overlap->start_s << "]";
                    pnc_junction_start_s = crosswalk_overlap->start_s;
                }

                const auto obstacle_id = KEEP_CLEAR_JUNCTION_VO_ID_PREFIX +
                                         pnc_junction_overlap->object_id;

                if (BuildKeepClearObstacle(frame, reference_line_info,
                                           obstacle_id, pnc_junction_start_s,
                           ...
                           const double adc_front_edge_s) const
{
    ...
    const auto& stage_type = injector_->planning_context()
                                     ->planning_status()
                                     .scenario()
                                     .stage_type();
        ...
        const double keep_clear_end_s)
{
    ...
    auto* obstacle = frame->CreateStaticObstacle(
            reference_line_info, virtual_obstacle_id, keep_clear_start_s,
            keep_clear_end_s);
    if (!obstacle)
    {
