...
void TrafficDecider::RegisterRules()
{
    s_rule_factory.Register(
            ...
            [](const TrafficRuleConfig &config,
               const std::shared_ptr<DependencyInjector> &injector)
                    -> TrafficRule * { return new BacksideVehicle(config); });
    s_rule_factory.Register(
...
void TrafficDecider::debug_string(ReferenceLineInfo *reference_line_info)
{
        ...
        const std::shared_ptr<DependencyInjector> &injector)
{
    ...
    for (const auto &rule_config : rule_configs_.config())
    {
        ...
        {
            // std::cout << "\nRule " << rule_config.rule_id() << " not
            // enabled";
            continue;
        }
        auto rule = s_rule_factory.CreateObject(rule_config.rule_id(),
                                                rule_config, injector);
        if (!rule)
        {
            std::cout << "Could not find rule " << rule_config.DebugString();
            continue;
        }
...
#if debug_traffic_decider
    debug_string(reference_line_info);
#endif
