                                   ...
                                   ReferenceLineInfo* const reference_line_info)
{
...
#if debug_ref_line_end
    AINFO << "ReferenceLineEnd length[" << reference_line.Length() << "]";

    for (const auto& segment : reference_line_info->Lanes())
    {
        AINFO << "   lane[" << segment.lane->lane().id().id() << "]";
    }

#endif

    // check
    double remain_s = reference_line.Length() -
    ...
    {
        return Status(common::PLANNING_ERROR,
                      "Failed to create path obstacle for reference line end "
                      "obstacle");
    }
    ...
    AINFO << "BuildStopDecision: ref line end";

    ObjectDecisionType stop;
    auto stop_decision = stop.mutable_stop();
    stop_decision->set_reason_code(StopReasonCode::STOP_REASON_REFERENCE_END);
    stop_decision->set_distance_s(
    ...
    path_decision->AddLongitudinalDecision(
            TrafficRuleConfig::RuleId_Name(config_.rule_id()), stop_wall->Id(),
            stop);
