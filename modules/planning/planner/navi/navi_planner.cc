...
void NaviPlanner::RegisterTasks()
{
    ...
    task_factory_.Register(TaskConfig::NAVI_SPEED_DECIDER, []() -> NaviTask* {
        return new NaviSpeedDecider();
    });
}

Status NaviPlanner::Init(const PlanningConfig& config)
{
    // NaviPlanner is only used in navigation mode based on the real-time
    // relative map.
    ...
    {
        tasks_.emplace_back(task_factory_.CreateObject(
                static_cast<TaskConfig::TaskType>(task)));
                         ...
                         Frame* frame, ADCTrajectory* ptr_computed_trajectory)
{
    ...
    for (auto& reference_line_info : *frame->mutable_reference_line_info())
    {
        ...
        auto status = PlanOnReferenceLine(planning_init_point, frame,
                                          &reference_line_info);
        ...
        ReferenceLineInfo* reference_line_info)
{
    ...
    *heuristic_speed_data = SpeedData(speed_profile);
    ...
    if (FLAGS_enable_trajectory_check)
    {
        ...
        if (ConstraintChecker::ValidTrajectory(
                    trajectory, adc_speed,
        ...
        const ReferenceLineInfo* reference_line_info, PathData* path_data)
{
    ...
    const auto& adc_ref_point =
            reference_line_info->reference_line().GetReferencePoint(0.5 *
                                                                    adc_s);
    ...
    for (double s = adc_s; s < max_s; s += unit_s)
    {
        ...
        path_points.push_back(PointFactory::ToPathPoint(
                ref_point.x() + dx, ref_point.y() + dy, 0.0, s,
                                           ...
                                           const double init_acc) const
{
    ...
    double pre_s = 0.0;
    const double t_mid =
            (FLAGS_slowdown_profile_deceleration - first_point_acc) /
            kFixedJerk;
    const double s_mid = init_speed * t_mid +
                         0.5 * first_point_acc * t_mid * t_mid +
                         1.0 / 6.0 * kFixedJerk * t_mid * t_mid * t_mid;
    const double v_mid = init_speed + first_point_acc * t_mid +
                         0.5 * kFixedJerk * t_mid * t_mid;
    ...
    for (double t = 0.0; t < max_t; t += unit_t)
    {
        ...
        if (t <= t_mid)
        {
            s = std::fmax(pre_s, init_speed * t +
                                         0.5 * first_point_acc * t * t +
                                         1.0 / 6.0 * kFixedJerk * t * t * t);
            v = std::fmax(0.0, init_speed + first_point_acc * t +
                                       0.5 * kFixedJerk * t * t);
        ...
        else
        {
            ...
            speed_data.AppendSpeedPoint(
                    s, t, v, FLAGS_slowdown_profile_deceleration, 0.0);
        }
        pre_s = s;
    }
    return speed_data;
        ...
        const double init_speed, const double init_acc) const
{
    ...
    for (double t = 2.0; t <= kMaxT; t += 0.5)
    {
        ...
        {
            QuinticPolynomialCurve1d curve(0.0, init_speed, init_acc, s, 0.0,
                                           0.0, t);
