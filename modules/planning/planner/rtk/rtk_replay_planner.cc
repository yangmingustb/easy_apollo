                              ...
                              ADCTrajectory* ptr_computed_trajectory)
{
    ...
    bool has_plan = false;
    auto it = std::find_if(frame->mutable_reference_line_info()->begin(),
                           frame->mutable_reference_line_info()->end(),
                           [](const ReferenceLineInfo& ref) {
                               return ref.is_change_lane_ref_line();
                           });
    if (it != frame->mutable_reference_line_info()->end())
    {
        status = PlanOnReferenceLine(planning_start_point, frame, &(*it));
        has_plan = (it->IsDrivable() && it->is_change_lane_ref_line() &&
                    it->trajectory().GetSpatialLength() >
        ...
        ReferenceLineInfo* reference_line_info)
{
    ...
    std::uint32_t matched_index = QueryPositionMatchedPoint(
            planning_init_point, complete_rtk_trajectory_);
...
void RTKReplayPlanner::ReadTrajectoryFile(const std::string& filename)
{
    ...
    while (true)
    {
        ...
        if (tokens.size() < 11)
        {
            AERROR << "RTKReplayPlanner parse line failed; the data dimension "
                      "does "
                      "not match.";
            AERROR << line;
            continue;
        ...
        const std::vector<TrajectoryPoint>& trajectory) const
{
    ...
    double d_min = func_distance_square(trajectory.front(),
                                        start_point.path_point().x(),
                                        start_point.path_point().y());
    std::uint32_t index_min = 0;
    for (std::uint32_t i = 1; i < trajectory.size(); ++i)
    {
        double d_temp = func_distance_square(trajectory[i],
                                             start_point.path_point().x(),
                                             start_point.path_point().y());
        if (d_temp < d_min)
        {
