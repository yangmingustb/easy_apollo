                            ...
                            std::array<double, 3>* ptr_d)
{
    CartesianFrenetConverter::cartesian_to_frenet(
            ...
            matched_point.theta(), matched_point.kappa(),
                            ...
                            ADCTrajectory* ptr_computed_trajectory)
{
    ...
    for (auto& reference_line_info : *frame->mutable_reference_line_info())
    {
        ...
        auto status = PlanOnReferenceLine(planning_start_point, frame,
                                          &reference_line_info);
        ...
        ReferenceLineInfo* reference_line_info)
{
    ...
    // 3. according to the matched point, compute the init state in Frenet
    // frame.
    ...
    // 5. generate 1d trajectory bundle for longitudinal and lateral
    // respectively.
    ...
    TrajectoryEvaluator trajectory_evaluator(
            init_s, planning_target, lon_trajectory1d_bundle,
            lat_trajectory1d_bundle, ptr_path_time_graph, ptr_reference_line);
    ...
    while (trajectory_evaluator.has_more_trajectory_pairs())
    {
        ...
        ADEBUG << "Starting Lon. State: s = " << init_s[0]
