        ...
        const double ref_cruise_speed) const
{
    ...
    for (const auto& time : time_samples)
    {
        ...
        size_t num_of_mid_points = std::min(
                static_cast<size_t>(FLAGS_num_velocity_sample - 2),
        ...
        if (num_of_mid_points > 0)
        {
            ...
            {
                State end_s = {0.0,
                               v_lower + velocity_seg * static_cast<double>(i),
                               0.0};
...
EndConditionSampler::QueryPathTimeObstacleSamplePoints() const
{
         ...
         ptr_path_time_graph_->GetPathTimeObstacles())
    {
        ...
        QueryOvertakePathTimePoints(vehicle_config, obstacle_id,
                                    &sample_points);
    }
