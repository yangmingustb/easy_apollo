                           ...
                           ADCTrajectory* const trajectory_pb)
{
    ...
    // This estimate is only valid if the current time and vehicle state
    // timestamp differs only a small amount (20ms). When the different is too
    // large, the estimation is invalid.
    ...
    if (!status.ok())
    {
        ...
        {
            // Because the function "Control::ProduceControlCommand()" checks
            // the "estop" signal with the following line (Line 170 in
            // control.cc): estop_ = estop_ || trajectory_.estop().is_estop();
            // we should add more information to ensure the estop being
            // triggered.
        ...
        std::vector<std::pair<std::string, double>>* const lane_info_group)
{
    ...
    for (auto& ref_line_info : ref_line_info_group)
    {
        ...
        auto lane_id = ref_line_info.Lanes().Id();
        auto ref_point = ref_line.GetReferencePoint(vehicle_state.x(),
                                                    vehicle_state.y());
        ...
        ADCTrajectory* const trajectory_pb)
{
    ...
    // trajectory.Otherwise, it will cause the Dreamview planning track to
    // display flashing or bouncing
