...
Status NaviSpeedTsGraph::Solve(std::vector<NaviSpeedTsPoint>* output)
{
    ...
    for (ssize_t i = constraints_.size() - 2; i >= 0; i--)
    {
        ...
        cur.v_max = std::min(
                std::sqrt(next.v_max * next.v_max + 2 * next.b_max * s_step_),
                cur.v_max);
    ...
    for (size_t i = 1; i < constraints_.size(); i++)
    {
        ...
        auto& cur = constraints_[i];
        cur.v_max = std::min(
                std::sqrt(prev.v_max * prev.v_max + 2 * cur.a_max * s_step_),
                cur.v_max);
        cur.v_preffered = std::min(cur.v_max, cur.v_preffered);
    }
    ...
    for (ssize_t i = constraints_.size() - 2; i >= 0; i--)
    {
        ...
        auto& cur = constraints_[i];
        cur.v_preffered =
                std::min(std::sqrt(next.v_preffered * next.v_preffered +
    ...
    for (size_t i = 1; i < constraints_.size(); i++)
    {
        ...
        auto& cur = constraints_[i];
        cur.v_preffered =
                std::min(std::sqrt(prev.v_preffered * prev.v_preffered +
