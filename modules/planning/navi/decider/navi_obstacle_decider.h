...
private:
     ...
     * the sign is added to the distance away from the path data.Take positive
     * on the left and negative on the right
     */
    void AddObstacleOffsetDirection(
            const common::PathPoint &projection_point,
            const std::vector<common::PathPoint> &path_data_points,
            const Obstacle *current_obstacle, const double proj_len,
            double *dist);
    ...
    double GetObstacleActualOffsetDistance(
            ...
            const double right_nedge_lane, const double left_nudge_lane,
