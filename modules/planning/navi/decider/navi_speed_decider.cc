...
bool NaviSpeedDecider::Init(const PlanningConfig& planning_config)
{
    ...
    preferred_jerk_ = std::abs(config.planner_navi_config()
                                       .navi_speed_decider_config()
                                       .preferred_jerk());
    max_accel_ = std::abs(config.planner_navi_config()
                                  .navi_speed_decider_config()
                                  .max_accel());
    max_decel_ = std::abs(config.planner_navi_config()
                                  .navi_speed_decider_config()
                                  .max_decel());
    ...
    kappa_preview_ = config.planner_navi_config()
                             .navi_speed_decider_config()
                             .kappa_preview();
    ...
    if (enable_accel_auto_compensation_)
    {
        if (prev_v_ > 0.0 && current_v > 0.0 && prev_v_ > expected_v)
        {
            ...
            decel_compensation_ratio_ =
                    (decel_compensation_ratio_ + raw_ratio) / 2.0;
            decel_compensation_ratio_ = Clamp(decel_compensation_ratio_, 1.0,
                                              kDecelCompensationLimit);
            ADEBUG << "change decel_compensation_ratio: "
                   << decel_compensation_ratio_ << " raw: " << raw_ratio;
        }
        prev_v_ = current_v;
    }
        ...
        SpeedData* const speed_data)
{
    ...
    for (auto& ts_point : ts_points)
    {
        ...
        {
            AERROR << "The v: " << ts_point.v
                   << " of point with s: " << ts_point.s
                   << " and t: " << ts_point.t
                   << "is greater than hard_speed_limit " << hard_speed_limit_;
        ...
        {
            AERROR << "The a: " << ts_point.a
                   << " of point with s: " << ts_point.s
                   << " and t: " << ts_point.t
                   << "is greater than hard_accel_limit " << hard_accel_limit_;
        ...
        speed_data->AppendSpeedPoint(ts_point.s + start_s, ts_point.t,
                                     ts_point.v, ts_point.a, ts_point.da);
    }
        ...
        const std::function<const Obstacle*(const std::string&)>& find_obstacle)
{
    ...
    for (const auto& info : obstacle_decider_.UnsafeObstacles())
    {
        ...
        if (obstacle != nullptr)
        {
            ...
            ts_graph_.UpdateObstacleConstraints(
                    obstacle_distance, safe_distance, following_accel_ratio_,
        ...
        const std::vector<PathPoint>& path_points)
{
    ...
    for (size_t i = 1; i < path_points.size(); i++)
    {
        ...
        auto v_preffered =
                std::min(std::sqrt(soft_centric_accel_limit_ / kappa),
    ...
    AINFO << "add speed limit for centric acceleration with kappa: "
          << max_kappa << " v_max: " << max_kappa_v
          << " v_preffered: " << preffered_kappa_v << " s: " << max_kappa_s;
