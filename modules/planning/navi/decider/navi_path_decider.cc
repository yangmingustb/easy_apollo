...
bool NaviPathDecider::Init(const PlanningConfig& config)
{
    ...
    for (const auto& item : move_dest_lane_config_talbe.lateral_shift())
    {
        ...
        {
            move_dest_lane_config_talbe_.emplace(std::make_pair(
                    max_speed_level, max_move_dest_lane_shift_y));
        ...
        std::vector<common::PathPoint>* const path_points)
{
    ...
    ADEBUG << "in current plan cycle move to dest lane, adc shift to dest "
              "reference line : "
           << lateral_shift_value;
    std::transform(path_points->begin(), path_points->end(),
                   path_points->begin(),
        ...
        std::vector<common::PathPoint>* const path_points)
{
    ...
    if (dest_lateral_distance <= max_keep_lane_distance_)
    {
        ...
        double actual_dest_point_y =
                NudgeProcess(reference_line, *path_points, obstacles,
                             *path_decision, vehicle_state_);
        ...
        if (actual_dest_lateral_distance > min_keep_lane_offset_)
        {
            ...
            lateral_shift_value =
                    (actual_dest_lateral_distance <
                     max_keep_lane_shift_y_ + min_keep_lane_offset_ -
            ...
            actual_shift_y =
                    std::copysign(lateral_shift_value, actual_dest_point_y);
        }

        ADEBUG << "in current plan cycle keep lane, actual dest : "
               << actual_dest_point_y
               << " adc shift to dest : " << actual_shift_y;
        std::transform(path_points->begin(), path_points->end(),
                       path_points->begin(),
                       [actual_shift_y](common::PathPoint& old_path_point) {
                           common::PathPoint new_path_point = old_path_point;
                           double new_path_point_y =
                                   old_path_point.y() + actual_shift_y;
                           new_path_point.set_y(new_path_point_y);
                           return new_path_point;
                       });
    }
        ...
        std::vector<common::PathPoint>* const path_points)
{
    ...
    // get the start plan point project s on reference line and get the length
    // of reference line
                                       ...
                                       const PathDecision& path_decision)
{
    ...
    for (const auto* obstacle : path_decision.obstacles().Items())
    {
        ...
        const double kForwardSafeDistance = std::max(
                ...
                ((vehicle_state_.linear_velocity() - obstacle->speed()) *
                 kSafeTime));
        const double kBackwardSafeDistance = std::max(
                kBackwardMinSafeDistance,
                ((obstacle->speed() - vehicle_state_.linear_velocity()) *
                 kSafeTime));
        ...
        const common::VehicleState& vehicle_state)
{
    ...
    double nudge_distance = obstacle_decider_.GetNudgeDistance(
            ...
            vehicle_state, &lane_obstacles_num);
