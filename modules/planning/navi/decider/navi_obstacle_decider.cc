        ...
        PathPoint* projection_point_ptr)
{
    ...
    *projection_point_ptr = PathMatcher::MatchToPath(
    ...
    double obstacle_start_position =
            projection_point_ptr->s() -
            current_obstacle->Perception().length() / 2.0;
        ...
        const common::VehicleState& vehicle_state)
{
    ...
    for (const auto& current_obstacle : obstacles)
    {
        ...
        auto dist = func_distance(
                projection_point, current_obstacle->Perception().position().x(),
                current_obstacle->Perception().position().y());

        if (dist < (config_.max_nudge_distance() +
                    current_obstacle->Perception().width() +
        ...
        const double left_nudge_lane, int* lane_obstacles_num)
{
    ...
    if (last_nudge_dist_ > 0)
    {
        ...
        {
            if (actual_dist <
                std::fabs(right_nudge_lane) + config_.safe_distance())
            {
    ...
    else if (last_nudge_dist_ < 0)
    {
        ...
        {
            if (actual_dist <
                std::fabs(left_nudge_lane) + config_.safe_distance())
            {
                *lane_obstacles_num = *lane_obstacles_num + 1;
            }
        }
        ...
        const common::VehicleState& vehicle_state, int* lane_obstacles_num)
{
    ...
    ProcessObstacle(obstacles, path_data_points, path_decision, min_lane_width,
                    vehicle_state);
    for (auto iter = obstacle_lat_dist_.begin();
         iter != obstacle_lat_dist_.end(); ++iter)
    {
            ...
            actual_dist < config_.max_nudge_distance())
        {
            ...
            if (lat_dist >= 0.0)
            {
                right_nudge_obstacle =
                        -1.0 * std::max(std::fabs(right_nudge_obstacle),
                                        need_nudge_dist);
            }
            else
            {
                left_nudge_obstacle = std::max(std::fabs(left_nudge_obstacle),
                                               need_nudge_dist);
            }
        }
    }
