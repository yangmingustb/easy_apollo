...
bool FemPosDeviationSqpOsqpInterface::Solve()
{
    ...
    {
        AERROR << "ref_points and bounds size not equal, solver early "
                  "terminates";
        return false;
    }
        ...
        while (sub_itr < sqp_sub_max_iter_)
        {
            ...
            if (!iterative_solve_res)
            {
                AERROR << "iteration at " << sub_itr
                       << ", solving fails with max sub iter "
                       << sqp_sub_max_iter_;
            ...
            if (ftol < sqp_ftol_)
            {
                ADEBUG << "merit function value converges at sub itr num "
                       << sub_itr;
                ADEBUG << "merit function value converges to " << cur_fvalue
                       << ", with ftol " << ftol << ", under max_ftol "
                       << sqp_ftol_;
                fconverged = true;
                break;
            }
        ...
        std::vector<c_float>* upper_bounds)
{
    ...
    {
        columns[i].emplace_back(i + num_of_slack_variables_,
                                -1.0 * scale_factor);
    }
    ...
    for (int i = 0; i < num_of_points_; ++i)
    {
        ...
        (*upper_bounds)[i * 2 + 1] =
                ref_point_xy.second + bounds_around_refs_[i];
        ...
        std::vector<c_float>* primal_warm_start)
{
    ...
    for (int i = 1; i < num_of_points_; ++i)
    {
        ...
        total_length +=
                std::sqrt((pre_point.first - cur_point.first) *
        ...
        const std::vector<std::pair<double, double>>& points)
{
    ...
    for (size_t i = 1; i < points.size() - 1; ++i)
    {
        ...
        double cviolation =
                curvature_constraint_sqr -
                (-2.0 * x_m + x_f + x_l) * (-2.0 * x_m + x_f + x_l) +
                (-2.0 * y_m + y_f + y_l) * (-2.0 * y_m + y_f + y_l);
        max_cviolation =
                max_cviolation < cviolation ? cviolation : max_cviolation;
    }
