...
int PiecewiseJerkProblem::piece_wise_jerk_problem_init(
...
bool PiecewiseJerkProblem::Optimize(const int max_iter)
{
    ...
    AINFO << "speed qp optimization takes "
    ...
    {
        AERROR << "failed optimization status: " << status << ", "
        ...
        std::vector<c_float>* upper_bounds)
{
    ...
    for (int i = 0; i + 1 < n; ++i)
    {
        ...
        variables[2 * n + i + 1].emplace_back(
                constraint_index, -0.5 * delta_s_ * scale_factor_[1]);
