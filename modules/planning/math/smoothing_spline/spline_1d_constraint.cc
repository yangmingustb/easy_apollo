                                                   ...
                                                   const double range)
{
    return AddConstraintInRange(
            std::bind(&Spline1dConstraint::AddBoundary, this, _1, _2, _3), x,
            fx, range);
}
...
{
    return AddConstraintInRange(
            std::bind(&Spline1dConstraint::AddDerivativeBoundary, this, _1, _2,
                      _3),
            x, dfx, range);
}
...
{
    return AddConstraintInRange(
            std::bind(&Spline1dConstraint::AddSecondDerivativeBoundary, this,
                      _1, _2, _3),
            x, ddfx, range);
}
...
bool Spline1dConstraint::AddDerivativeSmoothConstraint()
{
    ...
    const uint32_t num_params = spline_order_ + 1;
    Eigen::MatrixXd equality_constraint = Eigen::MatrixXd::Zero(
            n_constraint, (x_knots_.size() - 1) * num_params);
    ...
    for (uint32_t i = 0; i < n_constraint; i += 2)
    {
        ...
        for (uint32_t j = 0; j < num_params; ++j)
        {
            ...
            if (j >= 1)
            {
                equality_constraint(i + 1, num_params * (i / 2) + j) =
                        left_dcoef * j;
...
bool Spline1dConstraint::AddSecondDerivativeSmoothConstraint()
{
    ...
    for (uint32_t i = 0; i < n_constraint; i += 3)
    {
        ...
        for (uint32_t j = 0; j < num_params; ++j)
        {
            ...
            if (j >= 1)
            {
                equality_constraint(i + 1, num_params * (i / 3) + j) =
                        left_dcoef * j;
...
bool Spline1dConstraint::AddThirdDerivativeSmoothConstraint()
{
    ...
    for (uint32_t i = 0; i < n_constraint; i += 4)
    {
        ...
        for (uint32_t j = 0; j < num_params; ++j)
        {
            ...
            if (j >= 1)
            {
                equality_constraint(i + 1, num_params * i / 4 + j) =
                        left_dcoef * j;
        ...
        const std::vector<double>& x_coord)
{
    ...
    for (uint32_t i = 1; i < x_coord.size(); ++i)
    {
        ...
        if (cur_spline_index == prev_spline_index)
        {
            ...
            {
                inequality_constraint(i - 1,
                                      cur_spline_index * num_params + j) =
        ...
        else
        {
            ...
            for (uint32_t j = 0; j < cur_coef.size(); ++j)
            {
                inequality_constraint(i - 1, prev_spline_index * num_params +
                                                     j) = -prev_coef[j];
                inequality_constraint(
                        i - 1, cur_spline_index * num_params + j) = cur_coef[j];
            }
        }
        prev_coef = cur_coef;
...
{
    auto upper_bound =
            std::upper_bound(x_knots_.begin() + 1, x_knots_.end(), x);
