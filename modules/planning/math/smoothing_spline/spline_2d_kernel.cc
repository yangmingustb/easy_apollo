...
{
    total_params_ = (t_knots_.size() > 1
                             ? 2 * (t_knots_.size() - 1) * (1 + spline_order_)
                             : 0);
...
{
    Eigen::MatrixXd id_matrix = Eigen::MatrixXd::Identity(
...
bool Spline2dKernel::AddReferenceLineKernelMatrix(
        ...
        const std::vector<Vec2d>& ref_points, const double weight)
{
