...
Eigen::MatrixXd SplineSegKernel::NthDerivativeKernel(const uint32_t n,
        ...
        const uint32_t num_params, const double accumulated_x)
{
    ...
    Eigen::MatrixXd term_matrix;
    IntegratedTermMatrix(num_params, accumulated_x, "second_order",
                         &term_matrix);
        ...
        const uint32_t num_params, const double accumulated_x)
{
    ...
    Eigen::MatrixXd term_matrix;
    IntegratedTermMatrix(num_params, accumulated_x, "third_order",
                         &term_matrix);
...
void SplineSegKernel::CalculateThirdOrderDerivative(const uint32_t num_params)
{
    ...
    for (int r = 3; r < kernel_third_order_derivative_.rows(); ++r)
    {
        ...
        {
            kernel_third_order_derivative_(r, c) = (r * r - r) * (r - 2) *
                                                   (c * c - c) * (c - 2) /
                                                   (r + c - 5.0);
        }
    }
