    ...
    t_knots_(t_knots), spline_order_(order)
{
    ...
    total_param_ = 2 * (spline_order_ + 1) *
                   (static_cast<uint32_t>(t_knots.size()) - 1);
}
        ...
        const std::vector<double>& lateral_bound)
{
    ...
    for (uint32_t i = 0; i < t_coord.size(); ++i)
    {
        ...
        for (uint32_t j = 0; j < 2 * (spline_order_ + 1); ++j)
        {
            ...
            // upper longitudinal
            affine_inequality(4 * i + 2, index_offset + j) =
                    longitudinal_coef[j];
            // lower longitudinal
            affine_inequality(4 * i + 3, index_offset + j) =
                    -longitudinal_coef[j];
        }
        ...
        const std::vector<double>& lateral_bound)
{
    ...
    for (uint32_t i = 0; i < t_coord.size(); ++i)
    {
        ...
        const uint32_t index_offset = 2 * index * (spline_order_ + 1);
        std::vector<double> longi_coef =
                AffineThirdDerivativeCoef(angle[i], rel_t);
        std::vector<double> longitudinal_coef =
                                                 ...
                                                 const double angle)
{
    ...
    std::vector<double> line_derivative_coef =
            AffineDerivativeCoef(angle, rel_t);
    ...
    for (uint32_t i = 0; i < t_coef.size(); ++i)
    {
        ...
        affine_inequality(1, i + index_offset + num_params) =
                t_coef[i] * y_sign;
    }
...
bool Spline2dConstraint::AddSmoothConstraint()
{
    ...
    for (uint32_t i = 0; i + 2 < t_knots_.size(); ++i)
    {
        ...
        for (uint32_t j = 0; j < num_params; ++j)
        {
            ...
            affine_equality(2 * i + 1, j + index_offset + num_params) =
                    power_t[j];
        }
...
bool Spline2dConstraint::AddDerivativeSmoothConstraint()
{
    ...
    for (uint32_t i = 0; i + 2 < t_knots_.size(); ++i)
    {
        ...
        for (uint32_t j = 0; j < num_params; ++j)
        {
            ...
            affine_equality(4 * i + 2, j + index_offset + num_params) =
                    power_t[j];
...
bool Spline2dConstraint::AddThirdDerivativeSmoothConstraint()
{
    ...
    for (uint32_t i = 0; i + 2 < t_knots_.size(); ++i)
    {
        ...
        for (uint32_t j = 0; j < num_params; ++j)
        {
            ...
            affine_equality(8 * i + 2, j + index_offset) =
                    second_derivative_t[j];
            affine_equality(8 * i + 3, j + index_offset) =
                    third_derivative_t[j];
            affine_equality(8 * i + 4, j + index_offset + num_params) =
                    power_t[j];
...
{
    auto upper_bound =
            std::upper_bound(t_knots_.begin() + 1, t_knots_.end(), t);
