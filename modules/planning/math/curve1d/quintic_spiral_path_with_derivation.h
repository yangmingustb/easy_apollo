...
private:
    ...
    double DeriveThetaDerivative(const size_t param_index,
                                                    ...
                                                    const size_t n)
{
    ...
    auto res = Evaluate(
            order, static_cast<double>(i) / static_cast<double>(n) * param_);
        ...
        const size_t param_index, const int i, const int n)
{
    ...
    if (param_index == DELTA_S)
    {
        derivative += 60.0 * coef_[5] * s2 * r + 24.0 * coef_[4] * s * r +
                      6.0 * coef_[3] * r;
    }
        ...
        const size_t param_index, const double r) const
{
    ...
    double derivative = coef_deriv_[5][param_index] * s5 +
                        coef_deriv_[4][param_index] * s4 +
                        coef_deriv_[3][param_index] * s3 +
                        coef_deriv_[2][param_index] * s2 +
                        coef_deriv_[1][param_index] * s +
                        coef_deriv_[0][param_index];
