    ...
    if (lane_.has_type())
    {
        if (lane_.type() == Lane::CITY_DRIVING)
        {
            for (const auto &p : sampled_left_width_)
            {
                ...
                {
                    AERROR << "lane[id = " << lane_.id().DebugString()
                            ...
                            double *s_offset, int *s_offset_index) const
{
    ...
    *s_offset = accumulated_s_[index] +
                segments_[index].start().DistanceTo(*map_point);
    return distance;
}
...
void LaneInfo::CreateKDTree()
{
    ...
    lane_segment_kdtree_.reset(
            new LaneSegmentKDTree(segment_box_list_, params));
}
