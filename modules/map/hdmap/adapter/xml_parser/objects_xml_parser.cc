                                         ...
                                         std::vector<PbCrosswalk>* crosswalks)
{
    ...
    while (sub_node)
    {
        ...
        int checker = UtilXmlParser::QueryStringAttribute(*sub_node, "type",
                                                          &object_type);
        ...
        if (object_type == "crosswalk")
        {
            ...
            RETURN_IF_ERROR(
                    UtilXmlParser::ParseOutline(*outline_node, polygon));
...
Status ObjectsXmlParser::ParseSpeedBumps(const tinyxml2::XMLElement& xml_node,
                                         std::vector<PbSpeedBump>* speed_bumps)
{
    ...
    while (object_node)
    {
        std::string object_type;
        std::string object_id;
        int checker = UtilXmlParser::QueryStringAttribute(*object_node, "id",
                                                          &object_id);
        checker += UtilXmlParser::QueryStringAttribute(*object_node, "type",
                                                       &object_type);
        if (checker != tinyxml2::XML_SUCCESS)
        {
            std::string err_msg = "Error parse object type.";
        ...
        if (object_type == "speedBump")
        {
            ...
            while (sub_node)
            {
                ...
                RETURN_IF_ERROR(
                        UtilXmlParser::ParseGeometry(*sub_node, curve_segment));
                                      ...
                                      PbPassageGroup* passage_group)
{
    ...
    while (sub_node)
    {
        ...
        RETURN_IF_ERROR(
                ParsePassageIds(*sub_node, "laneID", &passage_node_ids));
        ...
        RETURN_IF_ERROR(
                ParsePassageIds(*sub_node, "signalID", &passage_node_ids));
        for (auto id : passage_node_ids)
        {
        ...
        RETURN_IF_ERROR(
                ParsePassageIds(*sub_node, "yieldlID", &passage_node_ids));
        for (auto id : passage_node_ids)
        {
...
Status ObjectsXmlParser::ParseRSUs(const tinyxml2::XMLElement& xml_node,
                                   std::vector<RSUInternal>* rsus)
{
    ...
    while (rsu_node)
    {
        std::string object_type;
        std::string object_id;
        int checker = UtilXmlParser::QueryStringAttribute(*rsu_node, "type",
                                                          &object_type);
        checker += UtilXmlParser::QueryStringAttribute(*rsu_node, "id",
                                                       &object_id);
        if (checker != tinyxml2::XML_SUCCESS)
        {
        ...
        if (object_type == "rsu")
        {
            ...
            checker = UtilXmlParser::QueryStringAttribute(
                    *rsu_node, "junctionID", &junction_id);
            if (checker != tinyxml2::XML_SUCCESS)
            {
