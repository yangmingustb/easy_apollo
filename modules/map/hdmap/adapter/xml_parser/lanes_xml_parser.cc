                             ...
                             std::vector<RoadSectionInternal>* sections)
{
    ...
    while (sub_node)
    {
        ...
        RETURN_IF_ERROR(ParseSectionBoundary(
                *sub_node, section_internal.section.mutable_boundary()
        ...
        const tinyxml2::XMLElement& xml_node, PbBoundaryPolygon* boundary)
{
    ...
    while (sub_node)
    {
        ...
        RETURN_IF_ERROR(UtilXmlParser::ParseCurve(
                *sub_node, boundary_edge->mutable_curve()));
        std::string type;
        int checker =
                UtilXmlParser::QueryStringAttribute(*sub_node, "type", &type);
        ...
        const tinyxml2::XMLElement& xml_node, bool bleft, PbLane* lane)
{
    ...
    auto sub_node =
            xml_node.FirstChildElement(bleft ? "leftRoadSampleAssociations"
    ...
    while (sub_node)
    {
        ...
        {
            std::string err_msg =
                    "Error parse road sample association attribute";
            return Status(apollo::common::ErrorCode::HDMAP_DATA_ERROR, err_msg);
        }

        auto road_sample = bleft ? lane->add_left_road_sample()
                                 : lane->add_right_road_sample();
        road_sample->set_s(s);
        ...
        const tinyxml2::XMLElement& xml_node, PbLane* lane)
{
    ...
    else
    {
        ...
        while (sub_node)
        {
            ...
            checker += sub_node->QueryDoubleAttribute("leftWidth", &left_width);
            checker +=
                    sub_node->QueryDoubleAttribute("rightWidth", &right_width);
            if (checker != tinyxml2::XML_SUCCESS)
            {
                std::string err_msg =
                        "Error parse road sample association attribute";
        ...
        std::vector<OverlapWithLane>* object_overlaps)
{
    ...
    if (object_overlap)
    {
        ...
        while (sub_node)
        {
            ...
            RETURN_IF_ERROR(ParseRegionOverlap(
                    *sub_node, &overlap_with_lane.region_overlaps));

            if (overlap_with_lane.region_overlaps.size() > 0)
            {
                UtilXmlParser::QueryStringAttribute(
                        *sub_node, "regionOverlapId",
                                   ...
                                   PbLane* lane)
{
    ...
    while (sub_node)
    {
        ...
        int checker =
                UtilXmlParser::QueryStringAttribute(*sub_node, "id", &lane_id);
        checker +=
                UtilXmlParser::QueryStringAttribute(*sub_node, "side", &side);
        checker += UtilXmlParser::QueryStringAttribute(*sub_node, "direction",
                                                       &direction);
        if (checker == tinyxml2::XML_SUCCESS)
        {
            if (side == "left")
        ...
        std::vector<PbRegionOverlap>* region_overlaps)
{
    ...
    while (region_overlap_node != nullptr)
    {
        ...
        int checker = UtilXmlParser::QueryStringAttribute(
                *region_overlap_node, "id", &region_overlap_id);
        if (checker != tinyxml2::XML_SUCCESS)
        {
