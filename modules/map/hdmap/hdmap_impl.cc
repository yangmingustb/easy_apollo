...
int HDMapImpl::LoadMapFromProto(const Map& map_proto)
{
    ...
    {
        crosswalk_table_[crosswalk.id().id()].reset(
                new CrosswalkInfo(crosswalk));
    }
    for (const auto& stop_sign : map_.stop_sign())
    {
        stop_sign_table_[stop_sign.id().id()].reset(
...
int HDMapImpl::GetJunctions(const PointENU& point, double distance,
                            std::vector<JunctionInfoConstPtr>* junctions) const
{
    return GetJunctions({point.x(), point.y()}, distance, junctions);
}

int HDMapImpl::GetJunctions(const Vec2d& point, double distance,
                            std::vector<JunctionInfoConstPtr>* junctions) const
{
...
int HDMapImpl::GetStopSigns(const PointENU& point, double distance,
                            std::vector<StopSignInfoConstPtr>* stop_signs) const
{
    return GetStopSigns({point.x(), point.y()}, distance, stop_signs);
}

int HDMapImpl::GetStopSigns(const Vec2d& point, double distance,
                            std::vector<StopSignInfoConstPtr>* stop_signs) const
{
        ...
        std::vector<ParkingSpaceInfoConstPtr>* parking_spaces) const
{
    ...
    const int status = SearchObjects(point, distance,
                                     *parking_space_polygon_kdtree_, &ids);
    if (status < 0)
    {
        return status;
    }
                                   ...
                                   std::vector<LaneInfoConstPtr>* lanes) const
{
    return GetLanesWithHeading({point.x(), point.y()}, distance,
                               central_heading, max_heading_difference, lanes);
}
        ...
        std::vector<JunctionBoundaryPtr>* junctions) const
{
    ...
    for (const auto& lane : lanes)
    {
        ...
        if (road_ptr->has_junction_id())
        {
            ...
            {
                AERROR << "junction id [" << junction_id.id()
                       << "] is not found.";
                continue;
            }
        ...
        std::vector<JunctionInfoConstPtr>* junctions) const
{
    ...
    for (const auto& road_ptr : roads)
    {
        ...
        else
        {
            ...
            const std::vector<apollo::hdmap::RoadBoundary>&
                    temp_road_boundaries = road_ptr->GetBoundaries();
            ...
            for (const auto& temp_road_boundary : temp_road_boundaries)
            {
                ...
                for (const auto& edge : boundary_polygon.edge())
                {
                        ...
                        apollo::hdmap::BoundaryEdge::LEFT_BOUNDARY)
                    {
                        for (const auto& s : edge.curve().segment())
                        {
                            ...
                            {
                                road_boundary_ptr->left_boundary.line_points
                                        .push_back(p);
                            }
                        }
                    }
                        ...
                        apollo::hdmap::BoundaryEdge::RIGHT_BOUNDARY)
                    {
                        for (const auto& s : edge.curve().segment())
                        {
                            for (const auto& p : s.line_segment().point())
                            {
                                road_boundary_ptr->right_boundary.line_points
                                        .push_back(p);
                            }
                        }
                    }
                ...
                if (temp_road_boundary.hole_size() != 0)
                {
                    for (const auto& hole : temp_road_boundary.hole())
                    {
                        ...
                        road_boundary_ptr->holes_boundary.push_back(
                                hole_boundary);
                    }
                }
            }
                      ...
                      std::vector<PolygonRoiPtr>* polygons_roi)
{
                    ...
                    for (const auto& edge : boundary_polygon.edge())
                    {
                            ...
                            apollo::hdmap::BoundaryEdge::RIGHT_BOUNDARY)
                        {
                            for (const auto& s : edge.curve().segment())
                            {
                                for (const auto& p : s.line_segment().point())
                                {
                                    road_boundary_ptr->right_boundary
    ...
    for (const auto& lane_ptr : lanes)
    {
        ...
        for (const auto& overlap_id : lane_ptr->lane().overlap_id())
        {
            ...
            {
                if (overlap_ptr->overlap().object(i).id().id() ==
                    lane_ptr->id().id())
                {
                    continue;
                }
                else
                {
                    ParkingSpaceInfoConstPtr parkingspace_ptr =
                            GetParkingSpaceById(
                                    overlap_ptr->overlap().object(i).id());
                    if (parkingspace_ptr != nullptr)
                    {
                        ...
                        {
                            PolygonRoiPtr polygon_roi_ptr(new PolygonRoi());
                            polygon_roi_ptr->polygon =
                                    parkingspace_ptr->polygon();
                            polygon_roi_ptr->attribute.type =
                                    PolygonType::PARKINGSPACE_POLYGON;
                            polygon_roi_ptr->attribute.id =
                                    parkingspace_ptr->id();
                            polygons_roi->push_back(polygon_roi_ptr);
        ...
        std::vector<SignalInfoConstPtr>* signals) const
{
    ...
    for (const auto& lane : surrounding_lanes)
    {
        ...
        {
            lane_ptr = lane;
            nearest_l = lane_ptr->DistanceTo(car_point, &map_point, &nearest_s,
                                             &s_index);
            break;
        }
    }
    ...
    while (lane_ptr != nullptr)
    {
        ...
        for (const auto& overlap_id : lane_ptr->lane().overlap_id())
        {
            ...
            for (int i = 0; i < overlap_ptr->overlap().object_size(); ++i)
            {
                if (overlap_ptr->overlap().object(i).id().id() ==
                    lane_ptr->id().id())
                {
                    lane_overlap_offset_s = overlap_ptr->overlap()
                                                    .object(i)
                                                    .lane_overlap_info()
                                                    .start_s() -
                                            s_start;
        ...
        unused_distance =
                unused_distance - (lane_ptr->total_length() - s_start);
    ...
    while (s < real_distance)
    {
        ...
        for (size_t x = 0; x < lane_ptr->junctions().size(); ++x)
        {
            ...
            for (int i = 0; i < overlap_ptr->overlap().object_size(); ++i)
            {
                ...
                overlap_junctions.push_back(
                        std::make_pair(start_s, junction_ptr));
            }
        }
        ...
        for (const auto& overlap_junction : overlap_junctions)
        {
            ...
            for (const auto& overlap_id : junction->junction().overlap_id())
            {
                ...
                for (int i = 0; i < overlap_ptr->overlap().object_size(); ++i)
                {
                    const auto& overlap_object =
                            overlap_ptr->overlap().object(i);
                                   ...
                                   std::unique_ptr<KDTree>* const kdtree)
{
    ...
    for (const auto& info_with_id : table)
    {
        ...
        for (size_t id = 0; id < info->segments().size(); ++id)
        {
            ...
            box_table->emplace_back(apollo::common::math::AABox2d(
