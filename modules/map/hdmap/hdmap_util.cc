...
std::string FindFirstExist(const std::string& dir, const std::string& files)
{
    ...
    for (const auto& filename : candidates)
    {
        const std::string file_path =
                absl::StrCat(FLAGS_map_dir, "/", filename);
...
std::string BaseMapFile()
{
    ...
    {
        AWARN << "base_map file is not used when FLAGS_use_navigation_mode is "
                 "true";
    }

    std::cout << FLAGS_test_base_map_filename << "\n"
              << FLAGS_map_dir << "\n"
...
std::string SimMapFile()
{
    if (FLAGS_use_navigation_mode)
    {
        AWARN << "sim_map file is not used when FLAGS_use_navigation_mode is "
                 "true";
    }
...
std::string RoutingMapFile()
{
    if (FLAGS_use_navigation_mode)
    {
        AWARN << "routing_map file is not used when FLAGS_use_navigation_mode "
                 "is "
                 "true";
    }
