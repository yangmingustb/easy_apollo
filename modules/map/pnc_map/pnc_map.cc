...
LaneWaypoint PncMap::ToLaneWaypoint(const routing::LaneWaypoint &waypoint) const
{
...
void PncMap::UpdateNextRoutingWaypointIndex(int cur_index)
{
    ...
    while (next_routing_waypoint_index_ != 0 &&
           ...
           routing_waypoint_index_[next_routing_waypoint_index_].index ==
                   cur_index &&
           adc_waypoint_.s <
                   routing_waypoint_index_[next_routing_waypoint_index_]
                           .waypoint.s)
    {
        --next_routing_waypoint_index_;
    }
    ...
    while (next_routing_waypoint_index_ < routing_waypoint_index_.size() &&
           cur_index == routing_waypoint_index_[next_routing_waypoint_index_]
                                .index &&
           adc_waypoint_.s >=
                   routing_waypoint_index_[next_routing_waypoint_index_]
                           .waypoint.s)
...
void PncMap::UpdateRoutingRange(int adc_index)
{
    ...
    {
        const auto &lane_id =
                route_indices_[range_end_].segment.lane->id().id();
...
bool PncMap::UpdateVehicleState(const VehicleState &vehicle_state)
{
    ...
    // adc_waypoint_是route优先车道的点，还是任意route任意车道的点？任意车道的点
...
// route是单车道组成的，还是多车道组成？
// 一般情况下是单车道组成。如果存在多次route换道，那么部分路段是由多车道组成。
    ...
    for (int road_index = 0; road_index < routing.road_size(); ++road_index)
    {
             ...
             ++passage_index)
        {
                 ...
                 ++lane_index)
            {
                ...
                route_indices_.back().index = {road_index, passage_index,
                                               lane_index};
            }
        }
    }
                                        ...
                                        const LaneWaypoint &waypoint) const
{
    ...
    while (i >= 0 && !RouteSegments::WithinLaneSegment(
                               ...
                               RouteSegments *segments) const
{
    ...
    for (const auto &lane : passage.segment())
    {
        ...
        segments->emplace_back(
                lane_ptr, std::max(0.0, lane.start_s()),
...
// 返回几条passage?
// apollo
// 参考线的数量和车道数量无关，而是和route有关系，如果route一直直行，即使是多车道，ref
// line也一直是一条。只有在需要换道时，ref line才会有多条
std::vector<int> PncMap::GetNeighborPassages(
        ...
        priority_lane_direction *priority_lane) const
{
    ...
    *priority_lane = priority_lane_direction::current_lane;
    ...
    if (source_passage.change_lane_type() == routing::LEFT)
    {
        ...
        *priority_lane = priority_lane_direction::left;
    }
    else if (source_passage.change_lane_type() == routing::RIGHT)
    {
        ...
        *priority_lane = priority_lane_direction::right;
    }
    ...
    for (int i = 0; i < road.passage_size(); ++i)
    {
        ...
        const auto &neighbour_passage = road.passage(i);
        for (const auto &segment : neighbour_passage.segment())
        {
                              ...
                              std::list<RouteSegments> *const route_segments)
{
    ...
    {
        AERROR << "Invalid vehicle state in pnc_map, update vehicle state "
                     "first.";
        return false;
    }
    ...
    // vehicle passage
    const int adc_passage_index = route_index[1];
    const auto &road = routing_.road(road_index);

    // Raw filter to find all neighboring passages
    // passage id
    priority_lane_direction priority_lane_direction_;
    auto drive_passages = GetNeighborPassages(road, adc_passage_index,
                                              &priority_lane_direction_);

    bool has_multi_lane = false;

    if (drive_passages.size() > 1)
    {
        has_multi_lane = true;
    }
    ...
    for (const int index : drive_passages)
    {
        ...
        // 不是车辆所在的passage,此时有route 换道需求
        if (index != adc_passage_index)
        {
            if (!segments.CanDriveFrom(adc_waypoint_))
            {
                AERROR << "You cannot drive from current waypoint to passage: "
                       << index;
        ...
        if (index == adc_passage_index)
        {
            ...
            route_segments->back().SetIsNeighborSegment(false);
        }
        else if (sl.l() > 0)
        {
            route_segments->back().SetPreviousAction(routing::RIGHT);
            route_segments->back().SetIsNeighborSegment(true);
        }
        else
        {
        ...
        // 判定优先级
        if (passage.change_lane_type() == routing::FORWARD)
        {
            route_segments->back().set_reference_line_priority(true);

        }
        else
        {
            // 车辆所在车道不是优先车道，那么另一个车道就是优先车道
            if (index != adc_passage_index)
            {
                route_segments->back().set_reference_line_priority(true);
            }
            ...
            {
                route_segments->back().set_reference_line_priority(false);
            }
        }
    }
                                        ...
                                        LaneWaypoint *waypoint) const
{
...
#if debug_pnc_map
    AINFO << "lanes size: " << lanes.size();

    for (auto &i : lanes)
    {
        AINFO << "lane id: " << i->lane().id().id();
    }
#endif
    ...
    {
        std::copy_if(lanes.begin(), lanes.end(),
                     std::back_inserter(valid_lanes),
                     [&](LaneInfoConstPtr ptr) {
...
#if debug_pnc_map
    AINFO << "valid lanes size: " << valid_lanes.size();

    for (auto &i : valid_lanes)
    {
        AINFO << "valid lane id: " << i->lane().id().id();
    }
#endif
    ...
    for (const auto &lane : valid_lanes)
    {
        ...
        {
#if debug_pnc_map
            AINFO << "no such lane in route" << lane->lane().id().id();
                            ...
                            RouteSegments *extended_segments)
{
    ...
    return ExtendSegments(segments, sl.s() - look_backward,
                          sl.s() + look_forward, extended_segments);
}
                            ...
                            RouteSegments *const truncated_segments) const
{
    ...
    for (const auto &lane_segment : segments)
    {
        const double adjusted_start_s =
                std::max(start_s - router_s + lane_segment.start_s,
        ...
        if (adjusted_start_s < adjusted_end_s)
        {
                     ...
                     unique_lanes.end())
            {
                truncated_segments->emplace_back(
                        lane_segment.lane, adjusted_start_s, adjusted_end_s);
    ...
    if (router_s < end_s && !truncated_segments->empty())
    {
        ...
        if (back.lane->total_length() > back.end_s)
        {
            ...
            back.end_s = std::min(back.end_s + end_s - router_s,
                                  back.lane->total_length());
    ...
    while (router_s < end_s - kRouteEpsilon)
    {
        ...
        const double length =
                std::min(end_s - router_s, last_lane->total_length());
