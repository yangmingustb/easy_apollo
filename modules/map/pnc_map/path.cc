                                                          ...
                                                          const double end_s)
{
    ...
    {
        // AINFO << "lane pt size " << lane->points().size() << " id: " << i;
        // AINFO << accumulate_s << " " << start_s << " " << end_s;
...
Path::Path(const std::vector<LaneSegment>& segments) : lane_segments_(segments)
{
...
void Path::InitPoints()
{
    ...
    for (int i = 0; i < num_points_; ++i)
    {
        ...
        else
        {
            ...
            // TODO(All): use heading.length when all adjacent lanes are
            // guarantee to be connected.
            s += heading.Length();
        }
        heading.Normalize();
...
void Path::InitWidth()
{
    ...
    for (int i = 0; i < num_sample_points_; ++i)
    {
        ...
        if (point.lane_waypoints().empty())
        {
            ...
            AWARN << "path point:" << point.DebugString()
                  << " has invalid width.";
        }
        else
        {
            ...
            waypoint.lane->GetWidth(waypoint.s, &lane_left_width,
                                    &lane_right_width);
        ...
        for (const auto& overlap : GetOverlaps_from_lane(*(lane_segment.lane)))
        {
                ...
                lane_overlap_info.end_s() >= lane_segment.start_s)
            {
                ...
                const double adjusted_start_s =
                        std::max(lane_overlap_info.start_s(),
                                 lane_segment.start_s) +
                        ref_s;
                const double adjusted_end_s =
                        std::min(lane_overlap_info.end_s(),
                                 lane_segment.end_s) +
                        ref_s;
                for (const auto& object : overlap->overlap().object())
                {
                    ...
                    {
                        overlaps_by_id[object.id().id()].emplace_back(
    ...
    for (auto& overlaps_one_object : overlaps_by_id)
    {
        ...
        for (const auto& segment : segments)
        {
            if (!overlaps->empty() && overlaps->back().object_id == object_id &&
                segment.first - overlaps->back().end_s <=
                        kMinOverlapDistanceGap)
            {
                overlaps->back().end_s =
            ...
            {
                overlaps->emplace_back(object_id, segment.first,
                                       segment.second);
            }
        }
    }
...
void Path::InitOverlaps()
{
    ...
    GetAllOverlaps(std::bind(&LaneInfo::yield_signs, _1),
    ...
    GetAllOverlaps(std::bind(&LaneInfo::clear_areas, _1),
...
InterpolatedIndex Path::GetIndexFromS(double s) const
{
    ...
    int high = (next_sample_id < num_sample_points_
                        ? std::min(num_points_,
                                   last_point_index_[next_sample_id] + 1)
                        : num_points_);
    while (low + 1 < high)
    {
...
std::vector<hdmap::LaneSegment> Path::GetLaneSegments(const double start_s,
                                                      const double end_s) const
{
    ...
    {
        lanes.emplace_back(lane_segments_[end_index.id].lane, 0,
                           end_index.offset);
    }
    return lanes;
}
...
void PathApproximation::InitDilute(const Path& path)
{
    ...
    for (int i = 0; i < num_points_ - 1; ++i)
    {
        max_error_per_segment_.push_back(compute_max_error(
                path, original_ids_[i], original_ids_[i + 1]));
    }
}
...
void PathApproximation::InitProjections(const Path& path)
{
    ...
    for (size_t i = 0; i < projections_.size(); ++i)
    {
        ...
        if (i + 1 < projections_.size())
        {
            ...
            for (int idx = original_ids_[i] + 1; idx < original_ids_[i + 1];
                 ++idx)
            {
                const double proj =
                        segment.ProjectOntoUnit(original_points[idx]);
                original_projections_.push_back(
                        projections_[i] +
                        std::max(0.0, std::min(proj, segment.length())));
            }
        }
    }
                                      ...
                                      double* min_distance) const
{
    ...
    *min_distance = std::numeric_limits<double>::infinity();
    ...
    for (size_t i = 0; i < segments_.size(); ++i)
    {
        ...
        double max_original_projection =
        ...
        if (first_segment_idx < last_segment_idx)
        {
            ...
            const double scan_distance =
                    sqrt(min_distance_sqr_with_error - prod_sqr);
            const double min_projection = projection - scan_distance;
            max_original_projection =
        ...
        {
            if (min_original_projections_to_right_[idx] >
                max_original_projection)
            {
                break;
            }
