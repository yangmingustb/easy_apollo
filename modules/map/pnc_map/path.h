...
struct LaneWaypoint
{
    ...
    // start s is lane start point, not route start point
    double s = 0.0;

    // lane offset to ref line:
    // ref line offset to lane point: this
    double l = 0.0;
...
// 对于一个普通的车道LaneInfoConstPtr，可能会分成多个lane segement
// 和routing.proto中的LaneSegment类似，但包含了更多的信息。
struct LaneSegment
{
    LaneSegment() = default;
    LaneSegment(LaneInfoConstPtr lane, const double start_s,
                const double end_s) :
...
{
    PathOverlap() = default;
    PathOverlap(std::string object_id, const double start_s,
                const double end_s) :
...
// path point是一个点,但是内部怎么会有std::vector<LaneWaypoint>，是一个数组
...
protected:
    // [-pi, +pi]
    double heading_ = 0.0;
    // 这些点都是位置相同的点。还是不清楚这里相同点从哪里来的，为什么会相同。
...
protected:
    ...
    using GetOverlapFromLaneFunc =
            std::function<const std::vector<OverlapInfoConstPtr>&(
                    const LaneInfo&)>;
...
protected:
    ...
    // lane point, ref line point? lane point
    ...
    // lane point 的左宽度，还是ref line point的左宽度：
