...
public:
    ...
    /**
     * Get the next change lane action need to take by the vehicle, if the
     ...
     * --- If the vehicle need to change to right lane according to routing,
     * then change_lane_type_ == routing::RIGHT;
     */
     ...
     * If the vehicle is already on this segment, or does not need to change
     * lane to reach this segment, then change_lane_type = routing::FORWARD; If
     * the vehicle need to change to left to reach this segment, then
     * change_lane_type_ =  routing::LEFT;
     ...
     * route segment.
     * @return false if error happened or projected outside of the lane
     * segments.
     */
    ...
    void set_reference_line_priority(bool is_priority_ref_line)
    {
        is_priority_ref_line_ = is_priority_ref_line;
    }

    bool is_reference_line_priority() const
    {
       return  is_priority_ref_line_;
    }
...
private:
    ...
    // 这是什么的id? route中的索引id， road segment index + passage index
    std::string id_;

    /**
     * Whether the vehicle should stop for destination. In a routing that has
     * loops, the adc may pass by destination many times, but it only need to
     * stop for destination  in the last loop.
     */
    bool stop_for_destination_ = false;

    // 根据route，是否是优先车道
    bool is_priority_ref_line_ = false;
};
