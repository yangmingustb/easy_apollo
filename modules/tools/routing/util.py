...
def get_map_dir(argv):
    ...
    mapdir = os.path.normpath(
        os.path.join(os.path.dirname(__file__), '/data/apollo_virutal_map/', FLAGS.map_dir))
...
def get_routingdata():
    ...
    route_data_path = os.path.join(log_dir, 'routing_res.bin')
    print("File: %s" % route_data_path)
    # return proto_utils.get_pb_from_text_file(route_data_path, routing_pb2.RoutingResponse())
    return proto_utils.get_pb_from_bin_file(route_data_path, routing_pb2.RoutingResponse())
