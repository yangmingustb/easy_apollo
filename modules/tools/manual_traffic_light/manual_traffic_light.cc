...
public:
    ...
    bool Proc()
    {
        ...
        if (!result)
        {
            ADEBUG << "Failed to get traffic signals from current location on "
                      "map";
        }
        else
        {
        ...
        {
            prev_traffic_lights_ = std::unordered_set<std::string>(
                    signal_ids.begin(), signal_ids.end());
            updated_ = true;
        }
        ...
        if (updated_)
        {
            ...
            if (signal_ids.empty())
            {
                ...
                {
                    std::cout << " No lights in the next "
                              << FLAGS_traffic_light_distance << " meters";
                }
            }
            else
            {
