...
# 带有安全buffer 的s
speed_decider_final_obs_buffer_t=[]
speed_decider_final_obs_buffer_s=[]
...
minimum_jerk_speed_profile_t=[]
minimum_jerk_speed_profile_s=[]

soft_brake_speed_profile_t=[]
soft_brake_speed_profile_s=[]
...
def callback(planning_pb):
    ...
    global speed_decider_final_obs_buffer_t   
    global speed_decider_final_obs_buffer_s   
    ...
    global minimum_jerk_speed_profile_t
    global minimum_jerk_speed_profile_s

    global soft_brake_speed_profile_t
    global soft_brake_speed_profile_s
    ...
    for st_graph in planning_pb.debug.planning_data.st_graph:
        ...
        print(st_graph.name)
        print("traj id %d" %planning_pb.header.sequence_num)
        ...
        if 'SPEED_BOUNDS_FINAL_DECIDER' == st_graph.name:
            for boundary in st_graph.boundary:
                ...
                bound_point_size = len(boundary.point)
                print("bound_point_size %d" %bound_point_size)

                lower_bound_point_size = bound_point_size / 2
                ...
                for point in boundary.point:
                    ...
                    # stop bound不需要拓展，因为已经拓展了
                    if(boundary.type != 2):
                        if(index < lower_bound_point_size):
                            speed_decider_final_obs_buffer_t.append(point.t)
                            speed_decider_final_obs_buffer_s.append(point.s + boundary.distance_s)
                    # print(i)
                    index = index+1
    ...
    for st in planning_pb.debug.planning_data.speed_plan:
        ...
        if(st.name == 'minimum_jerk_speed_profile'):
            for point in st.speed_point:
                minimum_jerk_speed_profile_t.append(point.t)
                minimum_jerk_speed_profile_s.append(point.s)

        if(st.name == 'soft_brake_speed_profile'):
            for point in st.speed_point:
                soft_brake_speed_profile_t.append(point.t)
                soft_brake_speed_profile_s.append(point.s)
...
def update(frame_number):
    ...
    speed_decider_final_obs_safe_buffer_curve.set_xdata(speed_decider_final_obs_buffer_t)
    speed_decider_final_obs_safe_buffer_curve.set_ydata(speed_decider_final_obs_buffer_s)
    ...
    minimum_jerk_speed_profile.set_xdata(minimum_jerk_speed_profile_t)
    minimum_jerk_speed_profile.set_ydata(minimum_jerk_speed_profile_s)

    soft_brake_speed_profile.set_xdata(soft_brake_speed_profile_t)
    soft_brake_speed_profile.set_ydata(soft_brake_speed_profile_s)
...
if __name__ == '__main__':
    ...
    speed_decider_obs_curve,= ax_st.plot(speed_decider_obs_t,
        speed_decider_obs_s,
        c='orange', alpha=0.8, label='speed_bound_prior_decider_obs',linestyle='--')
    ...
    speed_decider_final_obs_curve,= ax_st.plot(speed_decider_final_obs_t,
        speed_decider_final_obs_s,
        c='pink', alpha=0.8, label='speed_bound_decider_final_obs',marker ='o',linestyle='-')

    speed_decider_final_obs_safe_buffer_curve,= ax_st.plot(speed_decider_final_obs_buffer_t,
        speed_decider_final_obs_buffer_s,
        c='m', alpha=0.9, label='speed_bound_obs_safe_buffer',marker ='*',linestyle='-')
    ...
    pwj_speed_drive_bound,= ax_st.plot(pwj_speed_drive_region_t,
        pwj_speed_drive_region_s, c='black', alpha=0.99,
    ...
    # minimum_jerk_speed_profile
    minimum_jerk_speed_profile,= ax_st.plot(minimum_jerk_speed_profile_t,
        minimum_jerk_speed_profile_s, c='red', alpha=0.99,
        label='minimum_jerk_speed_profile', linestyle='-')

    soft_brake_speed_profile,= ax_st.plot(soft_brake_speed_profile_t,
        soft_brake_speed_profile_s, c='red', alpha=0.99,
        label='soft_brake_speed_profile', linestyle='-.')
