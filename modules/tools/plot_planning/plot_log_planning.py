...
def plot_lon_acc(lines, ax):
    ...
    for key in elem_map.keys():
        if elem_map[key]:
            ax["lon_acc"].plot(x,y, '-', label= key)
    
    ax["lon_acc"].set_ylim(-10, 10)
    ax["lon_acc"].set_xlabel("id")
    ax["lon_acc"].set_ylabel("lon_acc")

def plot_wheel(lines, ax):
    elem_map = {'start_angle': []
                }
    start_angle_list =[]
    for line in lines:
        for key in elem_map.keys():
            ...
            if name in line:
                ...
                if(key=='start_angle'):
                    start_angle_list.append( elem_map[key][0][0])
                else:
                    chassis_wheel.append( elem_map[key][0][0])
    
    print(start_angle_list)

    x = [x+1 for x in range(len(start_angle_list))]

    for key in elem_map.keys():
        if elem_map[key]:
            if(key =='start_angle'):
                y = start_angle_list
                ax["start_angle_list"].plot(x,y, '-', label= key)
            else:
                y = chassis_wheel
                ax["start_angle_list"].plot(x2,y, '-', label= key)
            

    
    ax["start_angle_list"].set_ylim(-600, 600)
    ax["start_angle_list"].set_xlabel("id")
    ax["start_angle_list"].set_ylabel("planning start point degree")


def plot_adc_lateral_lane_offset(lines, ax):
    elem_map = {'adc_lateral_lane_offet': []
                }


    adc_lateral_lane_offet_list =[]
    adc_lateral_lane_offet_time =[]
    ...
    for line in lines:
        for key in elem_map.keys():
            ...
            if name in line:
                ...
                adc_lateral_lane_offet_list.append( elem_map[key][0][0])
                time =  get_timestamp_from_line(line)
                adc_lateral_lane_offet_time.append(time)
    

    for key in elem_map.keys():
        if elem_map[key]:
            if(key =='adc_lateral_lane_offet'):
                y = adc_lateral_lane_offet_list
                x = adc_lateral_lane_offet_time
                # x = [x+1 for x in range(len(adc_lateral_lane_offet_list))]

                ax["adc_lateral_lane_offset"].plot(x,y, '-', label= "adc lateral lane offset")
            

    
    # ax["adc_lateral_lane_offset"].set_ylim(-6, 6)
    ax["adc_lateral_lane_offset"].set_xlabel("time")
    ax["adc_lateral_lane_offset"].set_ylabel("adc lateral lane offset")
    ax["adc_lateral_lane_offset"].ticklabel_format(useOffset=False, style='plain')

def plot_path_end_lateral_lane_offset(lines, ax):
    elem_map = {'path_end_lateral_lane_offet': []
                }


    path_end_lateral_lane_offset_list =[]
    path_end_lateral_lane_offset_time =[]
    ...
    for line in lines:
        for key in elem_map.keys():
            ...
            if name in line:
                ...
                path_end_lateral_lane_offset_list.append( elem_map[key][0][0])
                time =  get_timestamp_from_line(line)
                path_end_lateral_lane_offset_time.append(time)
    

    for key in elem_map.keys():
        if elem_map[key]:
            if(key =='path_end_lateral_lane_offet'):
                y = path_end_lateral_lane_offset_list
                x = path_end_lateral_lane_offset_time
                # x = [x+1 for x in range(len(path_end_lateral_lane_offset_list))]

                ax["path_end_lateral_lane_offset"].plot(x,y, '-', label= "path_end_lateral_lane_offset")
            

    
    # ax["path_end_lateral_lane_offset"].set_ylim(-10, 10)
    ax["path_end_lateral_lane_offset"].set_xlabel("time")
    ax["path_end_lateral_lane_offset"].set_ylabel("path_end_lateral_lane_offset")
    ax["path_end_lateral_lane_offset"].ticklabel_format(useOffset=False, style='plain')

def plot_adc_time_headway(lines, ax):
    elem_map = {'adc_time_headway': []
                }


    adc_time_headway_list =[]
    adc_time_headway_time =[]
    ...
    for line in lines:
        for key in elem_map.keys():
            ...
            if name in line:
                ...
                adc_time_headway_list.append( elem_map[key][0][0])
                time =  get_timestamp_from_line(line)
                adc_time_headway_time.append(time)
    

    for key in elem_map.keys():
        if elem_map[key]:
            if(key =='adc_time_headway'):
                y = adc_time_headway_list
                x = adc_time_headway_time
                # x = [x+1 for x in range(len(adc_time_headway_list))]

                ax["adc_time_headway"].plot(x,y, '-', label= key)
            

    
    # ax["adc_time_headway"].set_ylim(-1, 10)
    ax["adc_time_headway"].set_xlabel("time")
    ax["adc_time_headway"].set_ylabel("adc_time_headway")
    ax["adc_time_headway"].ticklabel_format(useOffset=False, style='plain') 
...
if __name__ == '__main__':
    ...
    ax = {}
    ax['lon_acc'] = fig.add_subplot(gs[0,0])
    ax['start_angle_list'] = fig.add_subplot(gs[1,0])

    ax['adc_lateral_lane_offset'] = fig.add_subplot(gs[2,0])

    ax['adc_time_headway'] = fig.add_subplot(gs[3,0])
    ax['path_end_lateral_lane_offset'] = fig.add_subplot(gs[4,0])

    plot_lon_acc( lines, ax)
    plot_wheel( lines, ax)
    plot_adc_lateral_lane_offset( lines, ax)
    plot_path_end_lateral_lane_offset( lines, ax)
    plot_adc_time_headway(lines, ax)
