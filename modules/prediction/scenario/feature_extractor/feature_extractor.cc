        ...
        const common::math::Vec2d& ego_position)
{
    ...
    if (ptr_left_neighbor_lane == nullptr &&
        ...
        ptr_ego_lane->lane().left_boundary().boundary_type(0).types_size() !=
                0 &&
                ...
                hdmap::LaneBoundaryType::CURB)
    {
        ...
        std::for_each(
                ...
                [&ptr_environment_features](
                    ...
                    ptr_environment_features->AddNonneglectableReverseLanes(
                            t.id());
                });
    }
}
        ...
        const LaneInfoPtr& ptr_ego_lane, const Vec2d& ego_position)
{
    ...
    if (ptr_left_neighbor_lane != nullptr)
    {
        ...
        ptr_left_neighbor_lane->GetProjection(
                ego_position, &left_neighbor_lane_s, &left_neighbor_lane_l);
    ...
    if (ptr_right_neighbor_lane != nullptr)
    {
        ...
        ptr_right_neighbor_lane->GetProjection(
                ego_position, &right_neighbor_lane_s, &right_neighbor_lane_l);
    ...
    for (const auto& overlap_id : junction->junction().overlap_id())
    {
        if (PredictionMap::OverlapById(overlap_id.id()) != nullptr)
        {
            for (const auto& object :
                 PredictionMap::OverlapById(overlap_id.id())
                         ->overlap()
                         .object())
            {
