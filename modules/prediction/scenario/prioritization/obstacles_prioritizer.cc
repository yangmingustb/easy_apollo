        ...
        const std::string& junction_id)
{
    ...
    for (const int obstacle_id : obstacle_ids)
    {
        ...
        {
            SetCautionIfCloseToEgo(ego_vehicle,
                                   FLAGS_caution_distance_threshold,
                                   obstacle_ptr);
        }
    }
}
        ...
        const Obstacle& ego_vehicle, ObstaclesContainer* obstacles_container)
{
         ...
         ego_latest_feature.lane().lane_graph().lane_sequence())
    {
        ...
        if (obstacle_ptr == nullptr)
        {
            AERROR << "Obstacle [" << nearest_front_obstacle_id
                   << "] Not found";
            continue;
        }
        ...
        const Obstacle& ego_vehicle, ObstaclesContainer* obstacles_container)
{
         ...
         ego_latest_feature.lane().lane_graph().lane_sequence())
    {
                                               ...
                                               ego_trajectory_container))
        {
            ...
            if (nearest_front_obstacle_id >= 0)
            {
                Obstacle* front_obstacle_ptr = obstacles_container->GetObstacle(
            ...
            if (nearest_backward_obstacle_id >= 0)
            {
                Obstacle* backward_obstacle_ptr =
                        obstacles_container->GetObstacle(
        ...
        const Obstacle& ego_vehicle, ObstaclesContainer* obstacles_container)
{
         ...
         obstacles_container->curr_frame_movable_obstacle_ids())
    {
        ...
        {
            if (!adc_trajectory_container->IsLaneIdInTargetReferenceLine(
                        lane_id))
            {
                continue;
            }
            ...
            if (PredictionMap::GetProjection({start_x, start_y}, lane_info_ptr,
                                             &start_s, &start_l) &&
                PredictionMap::GetProjection({end_x, end_y}, lane_info_ptr,
                                             &end_s, &end_l))
            {
                if (std::fabs(start_l) <
                            FLAGS_pedestrian_nearby_lane_search_radius ||
        ...
        const Obstacle& ego_vehicle, ObstaclesContainer* obstacles_container)
{
           ...
           FLAGS_caution_obs_max_nums)
    {
        ...
        obstacle_ptr->mutable_latest_feature()
                ->mutable_priority()
                ->set_priority(ObstaclePriority::NORMAL);
        ...
        ObstaclesContainer* obstacles_container)
{
    ...
    while (!lane_info_queue.empty())
    {
            ...
            !lane_obstacles[lane_id].empty())
        {
            ...
            Obstacle* obstacle_ptr =
                    obstacles_container->GetObstacle(obstacle_id);
            if (obstacle_ptr == nullptr)
            {
                AERROR << "Obstacle [" << obstacle_id << "] Not found";
                continue;
        ...
        for (const auto& pre_lane_id : curr_lane->lane().predecessor_id())
        {
            ...
            ConstLaneInfoPtr pre_lane_ptr =
                    PredictionMap::LaneById(pre_lane_id.id());
            lane_info_queue.emplace(
                    pre_lane_ptr, cumu_distance + pre_lane_ptr->total_length());
        }
    }
}
