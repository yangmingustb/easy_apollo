        ...
        Obstacle* obstacle, ObstaclesContainer* obstacles_container)
{
    ...
    for (int i = 0; i < feature_ptr->predicted_trajectory_size(); ++i)
    {
        Trajectory* trajectory_ptr =
                feature_ptr->mutable_predicted_trajectory(i);
                                         ...
                                         ObstacleClusters* clusters_ptr)
{
    ...
    {
        ExtrapolateByLane(lane_search_result, extraplation_speed,
                          trajectory_ptr, clusters_ptr);
    }
    else
    {
        ExtrapolateByFreeMove(kNumTailPoint, extraplation_speed,
                              trajectory_ptr);
    }
}
...
void ExtrapolationPredictor::ExtrapolateByLane(
        ...
        const double extraplation_speed, Trajectory* trajectory_ptr,
        ObstacleClusters* clusters_ptr)
{
    ...
    double lane_s = 0.0;
    double lane_l = 0.0;
    bool projected = PredictionMap::GetProjection(position, lane_info_ptr,
                                                  &lane_s, &lane_l);
    if (!projected)
    {
    ...
    for (int i = 1; i <= num_point_remained; ++i)
    {
        ...
        TrajectoryPoint* trajectory_point =
                trajectory_ptr->add_trajectory_point();
