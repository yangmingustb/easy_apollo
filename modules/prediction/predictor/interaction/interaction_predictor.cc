        ...
        Obstacle* obstacle, ObstaclesContainer* obstacles_container)
{
    ...
    for (int i = 0; i < num_lane_sequence; ++i)
    {
        ...
        lane_graph->mutable_lane_sequence(i)->set_probability(
        ...
        for (int i = 0; i < num_lane_sequence; ++i)
        {
            ...
            if (lane_sequence.has_stop_sign())
            {
                ...
                double stop_distance =
                        lane_sequence.stop_sign().lane_sequence_s() -
...
bool InteractionPredictor::DrawTrajectory(
        ...
        const double period, std::vector<TrajectoryPoint>* trajectory_points)
{
    // Sanity check.
        ...
        const ADCTrajectoryContainer* adc_trajectory_container)
{
        ...
        PredictionConstants::kDumpDataForTuning)
    {
        std::vector<double> cost_values = {lon_acc_cost, centri_acc_cost,
                                           collision_cost};
        FeatureOutput::InsertDataForTuning(obstacle.latest_feature(),
                                           cost_values, "interaction",
                                           lane_sequence, adc_trajectory_);
    }
