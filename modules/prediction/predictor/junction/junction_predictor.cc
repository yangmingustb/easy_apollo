        ...
        std::vector<TrajectoryPoint>* trajectory_points)
{
    ...
    while (t <= exit_time)
    {
        ...
        path_point.set_theta(
                std::atan2(EvaluateCubicPolynomial(y_coeffs, t, 1),
                           EvaluateCubicPolynomial(x_coeffs, t, 1)));
        TrajectoryPoint trajectory_point;
        trajectory_point.mutable_path_point()->CopyFrom(path_point);
        trajectory_point.set_v(
                std::hypot(EvaluateCubicPolynomial(x_coeffs, t, 1),
    ...
    while (t <= total_time)
    {
        ...
        while (lane_s > PredictionMap::LaneById(lane_id)->total_length())
        {
            ...
            if (PredictionMap::LaneById(lane_id)->lane().successor_id_size() <
                1)
            {
                return;
            }
            // TODO(all) consider the logic to choose successor_id
            lane_id = PredictionMap::LaneById(lane_id)
                              ->lane()
                              .successor_id(0)
                              .id();
        }
        ...
        const Feature& feature)
{
         ...
         feature.junction_feature().junction_exit())
    {
        ...
        double angle =
                std::atan2(y, x) - std::atan2(feature.raw_velocity().y(),
                                              feature.raw_velocity().x());
