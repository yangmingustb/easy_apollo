        ...
        std::vector<bool>* enable_lane_sequence)
{
    ...
    for (int i = 0; i < num_lane_sequence; ++i)
    {
        ...
        double distance = GetLaneChangeDistanceWithADC(
                sequence, ego_vehicle_ptr, adc_trajectory_container);
        ...
        if (distance > 0.0 && distance < FLAGS_lane_change_dist)
        {
            ...
            for (int j = 0; j < feature.polygon_point_size(); j++)
            {
                ...
                double lane_s = 0.0;
                double lane_l = 0.0;
                PredictionMap::GetProjection(position, lane_info, &lane_s,
                                             &lane_l);
    ...
    for (int i = 0; i < num_lane_sequence; ++i)
    {
        ...
        if (LaneSequenceWithMaxProb(lane_change_type[i], probability,
                                    all.second))
        {
            all.first = i;
        ...
        const std::string& lane_id, const LaneSequence& lane_sequence)
{
    ...
    else
    {
        ...
        if (PredictionMap::IsLeftNeighborLane(ptr_change_lane,
                                              ptr_current_lane))
        {
        ...
        const ADCTrajectoryContainer* adc_trajectory_container)
{
    ...
    if (ego_vehicle_ptr != nullptr && ego_vehicle_ptr->history_size() > 0)
    {
        ...
        double lane_s = 0.0;
        double lane_l = 0.0;
        if (PredictionMap::GetProjection(
                    adc_position, PredictionMap::LaneById(obstacle_lane_id),
                    &lane_s, &lane_l))
        {
            ADEBUG << "Distance with ADC is "
                   << std::fabs(lane_s - obstacle_lane_s);
        ...
        std::array<double, 5>* coefficients)
{
    ...
    // double min_end_speed = std::min(FLAGS_still_obstacle_speed_threshold,
    // ds0); double ds1 = std::max(min_end_speed, ds0 + dds0 *
    // time_to_end_state);
