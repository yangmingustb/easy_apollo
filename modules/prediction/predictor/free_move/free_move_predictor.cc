        ...
        Obstacle* obstacle, ObstaclesContainer* obstacles_container)
{
    ...
    if (feature.predicted_trajectory().empty())
    {
        ...
        Eigen::Vector2d position(feature.position().x(),
                                 feature.position().y());
        Eigen::Vector2d velocity(feature.velocity().x(),
                                 feature.velocity().y());
        Eigen::Vector2d acc(feature.acceleration().x(),
    ...
    else
    {
        for (int i = 0; i < feature.predicted_trajectory_size(); ++i)
        {
            ...
            DrawFreeMoveTrajectoryPoints(
                    ...
                    FLAGS_prediction_trajectory_time_resolution, &points);
            // The following for-loop starts from index 1 because the vector
            // points includes the last point in the existing predicted
            // trajectory
