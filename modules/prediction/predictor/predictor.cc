                                    ...
                                    Obstacle* obstacle_ptr)
{
    ...
    const auto prob =
            total_probability / static_cast<double>(num - start_index);
        ...
        Obstacle* obstacle, Trajectory* trajectory)
{
    ...
    double forward_length = std::fmax(
            vehicle_length / 2.0 - FLAGS_distance_beyond_junction, 0.0);
    ...
    while (index < num_of_point)
    {
        const PathPoint& point =
                trajectory->trajectory_point(index).path_point();
...
bool Predictor::SupposedToStop(const Feature& feature,
                               const double stop_distance, double* acceleration)
{
