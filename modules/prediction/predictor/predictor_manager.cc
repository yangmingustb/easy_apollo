        ...
        ObstaclesContainer* obstacles_container)
{
    ...
    {
        PredictObstaclesInParallel(perception_obstacles,
        ...
        ObstaclesContainer* obstacles_container)
{
         ...
         perception_obstacles.perception_obstacle())
    {
        ...
        if (obstacle != nullptr)
        {
            PredictObstacle(adc_trajectory_container, obstacle,
        ...
        ObstaclesContainer* obstacles_container)
{
         ...
         perception_obstacles.perception_obstacle())
    {
        ...
        if (obstacle == nullptr)
        {
            ...
            prediction_obstacle_ptr->set_timestamp(
                    perception_obstacle.timestamp());
        }
        else
        {
            GroupObstaclesByObstacleId(id, obstacles_container,
                                       &id_obstacle_map);
        }
    }
        ...
        PredictionObstacle* const prediction_obstacle)
{
    ...
    // AINFO << "traj number: "
        //   << obstacle->latest_feature().predicted_trajectory_size();

    int pred_traj_index = 0;

    int pred_traj_size = obstacle->latest_feature().predicted_trajectory_size();

    for (int i = 0; i < pred_traj_size; i++)
    {
        const prediction::Trajectory& cur_traj =
                obstacle->latest_feature().predicted_trajectory(i);

        if (i >= 1)
        {
            const prediction::Trajectory& last_traj =
                    obstacle->latest_feature().predicted_trajectory(i - 1);

            if (is_same_trajectory(last_traj, cur_traj))
            {
                AINFO << "same prediction trajectory. obs id: "
                      << obstacle->id();
                AINFO << "traj number: "
                  << obstacle->latest_feature().predicted_trajectory_size();
        ...
        prediction_obstacle->add_trajectory()->CopyFrom(cur_traj);

        pred_traj_index++;

        if (pred_traj_index >= 2)
        {
            break;
        }
    }
...
bool PredictorManager::is_same_trajectory(const prediction::Trajectory& left,
                                          const prediction::Trajectory& right)
{
    int left_size = left.trajectory_point_size();
    int right_size = right.trajectory_point_size();
    ...
    common::math::Vec2d left_vec;
    common::math::Vec2d right_vec;

    double dist;

    for (size_t i = 0; i < left_size; i++)
    {
        const common::TrajectoryPoint& left_point = left.trajectory_point(i);

        left_vec.set_x(left_point.path_point().x());
        left_vec.set_y(left_point.path_point().y());

        const common::TrajectoryPoint& right_point = right.trajectory_point(i);

        right_vec.set_x(right_point.path_point().x());
        right_vec.set_y(right_point.path_point().y());

        dist = left_vec.DistanceSquareTo(right_vec);
