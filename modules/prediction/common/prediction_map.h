...
public:
     ...
     */
    static std::shared_ptr<const hdmap::LaneInfo> LaneById(
            const std::string& id);
     ...
     */
    static void OnLane(
            const std::vector<std::shared_ptr<const hdmap::LaneInfo>>&
                    prev_lanes,
            const Eigen::Vector2d& point, const double heading,
            const double radius, const bool on_lane, const int max_num_lane,
     ...
     * @param The size of samples alone the lane to compute the average
     * curvature
