                                       ...
                                       const LaneGraph& lane_graph_successor)
{
    ...
    for (const auto& lane_sequence_pre : lane_graph_predecessor.lane_sequence())
    {
        ...
        for (const auto& lane_sequence_suc :
             lane_graph_successor.lane_sequence())
        {
            if (lane_sequence_suc.lane_segment(0).lane_id() ==
                ending_lane_segment.lane_id())
            {
                LaneSequence* lane_sequence =
                        final_lane_graph.add_lane_sequence();
        ...
        LaneGraph* const lane_graph_ptr) const
{
    ...
    double curr_s = curr_lane_seg_s >= 0.0 ? curr_lane_seg_s
    ...
    if (search_forward_direction)
    {
        ...
        // Based on other conditions, select what successor lanes should be
        // used.
        if (!consider_lane_split)
        {
            candidate_lanes = {PredictionMap::LaneWithSmallestAverageCurvature(
                    candidate_lanes)};
        }
    }
    else
    ...
    for (const auto& candidate_lane : candidate_lanes)
    {
        ConstructLaneSequence(
                search_forward_direction, new_accumulated_s, new_lane_seg_s,
