                                 ...
                                 const double end_t, const double end_v)
{
    ...
    switch (order)
    {
        ...
        case 3:
        {
            return (60.0 * coeffs[5] * t + 24.0 * coeffs[4]) * t +
                   6.0 * coeffs[3];
        }
        case 4:
        {
                                 ...
                                 const double end_t, const double end_v)
{
    if (t >= end_t)
    {
        switch (order)
        {
            ...
            {
                double end_value =
                        (((coeffs[4] * end_t + coeffs[3]) * end_t + coeffs[2]) *
                                 end_t +
                         coeffs[1]) *
                                end_t +
                        coeffs[0];
    ...
    switch (order)
    {
        ...
        case 2:
        {
            return (12.0 * coeffs[4] * t + 6.0 * coeffs[3]) * t +
                   2.0 * coeffs[2];
        }
        case 3:
        {
                               ...
                               const double end_t, const double end_v)
{
    if (t > end_t)
    {
        switch (order)
        {
            ...
            {
                double end_value =
                        ((coefs[3] * end_t + coefs[2]) * end_t + coefs[1]) *
                                end_t +
                        coefs[0];
                return end_value + (t - end_t) * end_v;
            }
    ...
    for (std::size_t i = 0; i < num; ++i)
    {
        ...
        if (speed > std::numeric_limits<double>::epsilon())
        {
            if (points->size() > 0)
            {
                ...
                PathPoint* prev_path_point =
                        prev_trajectory_point.mutable_path_point();
                theta = std::atan2(y - prev_path_point->y(),
                                   x - prev_path_point->x());
