        ...
        const std::unordered_map<int, ObstacleHistory>& obstacle_id_history_map)
{
    ...
    {
        DrawADCTrajectory(cv::Scalar(0, 255, 255), curr_base_x_, curr_base_y_,
                          &curr_img_);
    }
                              ...
                              const double base_x, const double base_y)
{
    ...
    common::PointENU center_point =
            common::util::PointFactory::ToPointENU(x, y);
                    ...
                    for (const auto& segment : edge.curve().segment())
                    {
                        for (const auto& point : segment.line_segment().point())
                        {
                            polygon.insert(polygon.begin(),
                                           std::move(GetTransPoint(
                                                   point.x(), point.y(), base_x,
                                                   base_y)));
                        }
                    }
                }
    ...
    for (const auto& lane : lanes)
    {
        ...
        for (const auto& segment : lane->lane().central_curve().segment())
        {
            for (int i = 0; i < segment.line_segment().point_size() - 1; ++i)
            {
                ...
                // cv::Scalar c =
                //     cv::Scalar(rgb.at<float>(0, 0) * 255, rgb.at<float>(0, 1)
                //     * 255,
                           ...
                           cv::Mat* img)
{
    ...
    polygon.push_back(std::move(GetTransPoint(
            ...
            obs_y + (sin(theta) * -obs_l + cos(theta) * obs_w) / 2, base_x,
            base_y)));
                           ...
                           cv::Mat* img)
{
    ...
    {
        polygon.push_back(std::move(GetTransPoint(
                polygon_point.x(), polygon_point.y(), base_x, base_y)));
    }
    cv::fillPoly(*img,
                 std::vector<std::vector<cv::Point>>({std::move(polygon)}),
                 color);
                              ...
                              const double base_y, cv::Mat* img)
{
    for (int i = history.feature_size() - 1; i >= 0; --i)
    {
        ...
        double time_decay =
                1.0 - ego_feature_.timestamp() + feature.timestamp();
        ...
        else
        {
            ...
            {
                std::cout << "No polygon points in feature, please check!";
                continue;
            }
                                    ...
                                    cv::Mat* img)
{
    ...
    {
        double time_decay =
                ego_feature_.adc_trajectory_point(i).relative_time() -
                                   ...
                                   const double base_y)
{
    ...
    const cv::Point2i& center_point =
            GetTransPoint(curr_feature.position().x(),
                          curr_feature.position().y(), base_x, base_y);
