...
DEFINE_double(prediction_test_duration, std::numeric_limits<double>::infinity(),
              "The runtime duration in test mode (in seconds). Negative value "
              "will not "
              "restrict the runtime duration.");

DEFINE_string(prediction_offline_bags, "",
              "a list of bag files or directories for offline mode. The items "
              "need to be "
              "separated by colon ':'.  If this value is not set, the "
              "prediction module "
