                                 ...
                                 IdObstacleListMap* const id_obstacle_map)
{
         ...
         obstacles_container->curr_frame_considered_obstacle_ids())
    {
        ...
        {
            caution_thread_idx =
                    caution_thread_idx % FLAGS_max_caution_thread_num;
        ...
        else
        {
            ...
            int id_mod = obstacle_id % normal_thread_num +
                         FLAGS_max_caution_thread_num;
            (*id_obstacle_map)[id_mod].push_back(obstacle_ptr);
            ADEBUG << "Normal obstacle [" << obstacle_id << "] for thread"
                   << id_mod;
        }
    }
}
    ...
    for (const auto& obstacle_conf : config.obstacle_conf())
    {
        ...
        if (obstacle_conf.has_obstacle_status())
        {
            switch (obstacle_conf.obstacle_type())
            {
                ...
                {
                    if (obstacle_conf.obstacle_status() ==
                        ObstacleConf::ON_LANE)
                    {
                        if (obstacle_conf.priority_type() ==
        ...
        ObstaclesContainer* obstacles_container)
{
    ...
    else
    {
        for (int id : obstacles_container->curr_frame_considered_obstacle_ids())
        {
            ...
            if (obstacle->IsStill())
            {
                ADEBUG << "Ignore still obstacle [" << id
    ...
    switch (obstacle->type())
    {
        case PerceptionObstacle::VEHICLE:
        {
            if (obstacle->IsCaution() && !obstacle->IsSlow())
            {
                ...
                {
                    evaluator =
                            GetEvaluator(vehicle_on_lane_caution_evaluator_);
                }
                else
                {
                    evaluator =
                            GetEvaluator(vehicle_default_caution_evaluator_);
                }
                CHECK_NOTNULL(evaluator);
                // Evaluate and break if success
                if (evaluator->GetName() ==
                    "JOINTLY_PREDICTION_PLANNING_EVALUATOR")
                {
                    if (evaluator->Evaluate(adc_trajectory_container, obstacle,
                                            obstacles_container))
                    {
                        break;
                    }
                ...
                else
                {
                    ...
                    {
                        AERROR << "Obstacle: " << obstacle->id()
                               << " caution evaluator failed, downgrade to "
                                  "normal level!";
                    }
                }
            }
            ...
            else
            {
                ADEBUG << "Obstacle: " << obstacle->id()
                       << " is neither on lane, nor in junction. Skip "
                          "evaluating.";
                break;
            }
            CHECK_NOTNULL(evaluator);
