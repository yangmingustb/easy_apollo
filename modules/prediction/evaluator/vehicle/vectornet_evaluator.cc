...
bool VectornetEvaluator::VectornetProcessObstaclePosition(
        ...
        torch::Tensor* ptr_obstacle_data, torch::Tensor* ptr_all_obs_p_id)
{
    ...
    for (int i = 0; i < obs_num; ++i)
    {
        ...
        for (int j = 0; j < 20; ++j)
        {
            ...
            all_obstacle_pos.index_put_({i, 19 - j, 0},
                                        all_obs_pos_history[i][j].first);
            all_obstacle_pos.index_put_({i, 19 - j, 1},
                                        all_obs_pos_history[i][j].second);
        }

        ptr_all_obs_p_id->index_put_(
                {i}, torch::from_blob(obs_p_id.data(), {2}, opts));
                                  ...
                                  ObstaclesContainer* obstacles_container)
{
    ...
    if (!VectornetProcessObstaclePosition(
                ...
                &target_obstacle_pos_step, &vector_mask, &obstacle_data,
                &all_obs_p_id))
    {
    ...
    AINFO << "obstacle vectors used time: " << diff_obs.count() * 1000
          << " ms.";
...
void VectornetEvaluator::LoadModel()
{
    ...
    else
    {
        torch_vehicle_model_ = torch::jit::load(
                FLAGS_torch_vehicle_vectornet_cpu_file, device_);
    }
