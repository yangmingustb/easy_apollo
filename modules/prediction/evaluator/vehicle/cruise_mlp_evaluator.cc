    ...
    for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i)
    {
            ...
            PredictionConstants::kDumpDataForLearning)
        {
            ...
            FeatureOutput::InsertDataForLearning(
                    *latest_feature_ptr, feature_values, "lane_scanning",
                    lane_sequence_ptr);
        ...
        int input_dim =
                static_cast<int>(OBSTACLE_FEATURE_SIZE +
        ...
        const Obstacle* obstacle_ptr, std::vector<double>* feature_values)
{
    ...
    for (std::size_t i = 0; i < obstacle_ptr->history_size(); ++i)
    {
        ...
        if (feature.has_lane() && feature.lane().has_lane_feature())
        {
            ...
            dist_lbs.push_back(
                    feature.lane().lane_feature().dist_to_left_boundary());
            dist_rbs.push_back(
                    feature.lane().lane_feature().dist_to_right_boundary());
            lane_types.push_back(
                    feature.lane().lane_feature().lane_turn_type());
        ...
        {
            pos_history[i] =
                    WorldCoordToObjCoord(std::make_pair(feature.position().x(),
                                                        feature.position().y()),
        ...
        {
            auto vel_end =
                    WorldCoordToObjCoord(std::make_pair(feature.velocity().x(),
                                                        feature.velocity().y()),
                                         obs_curr_pos, obs_curr_heading);
        ...
        if (feature.has_velocity_heading())
        {
            ...
            if (i != 0)
            {
                vel_heading_changing_rate_history[i] =
                        ...
                        (FLAGS_double_precision + feature.timestamp() -
                         prev_timestamp);
        ...
        delta_t > std::numeric_limits<double>::epsilon())
    {
        ...
        dist_lb_rate_curr =
                (dist_lb_curr - dist_lb_prev) / (curr_size * delta_t);
    }
        ...
        delta_t > std::numeric_limits<double>::epsilon())
    {
        ...
        dist_rb_rate_curr =
                (dist_rb_curr - dist_rb_prev) / (curr_size * delta_t);
    }
    ...
    for (int i = 0; i < lane_sequence_ptr->lane_segment_size(); ++i)
    {
        ...
        for (int j = 0; j < lane_segment.lane_point_size(); ++j)
        {
            ...
            std::pair<double, double> relative_s_l = WorldCoordToObjCoord(
                    ...
                    heading);
            double relative_ang =
                    WorldAngleToObjAngle(lane_point.heading(), heading);
...
void CruiseMLPEvaluator::LoadModels()
{
    ...
    AINFO << "CruiseMLPEvaluator" << FLAGS_torch_vehicle_cruise_go_file;
    torch::set_num_threads(1);

    torch_go_model_ =
            torch::jit::load(FLAGS_torch_vehicle_cruise_go_file, device_);
    AINFO << "CruiseMLPEvaluator";
        ...
        LaneSequence* lane_sequence_ptr)
{
    ...
    lane_sequence_ptr->set_probability(
            apollo::common::math::Sigmoid(static_cast<double>(
