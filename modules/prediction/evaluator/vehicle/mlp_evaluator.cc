                            ...
                            ObstaclesContainer* obstacles_container)
{
    ...
    for (int i = 0; i < lane_graph_ptr->lane_sequence_size(); ++i)
    {
        ...
        SetLaneFeatureValues(obstacle_ptr, lane_sequence_ptr,
                             &lane_feature_values);
...
void MLPEvaluator::SetObstacleFeatureValues(Obstacle* obstacle_ptr,
                                            std::vector<double>* feature_values)
{
    ...
    {
        angle_diff_rate =
                angle_diff / (delta_t * static_cast<double>(curr_size));
        lane_l_diff_rate =
                lane_l_diff / (delta_t * static_cast<float>(curr_size));
    }
        ...
        const std::vector<double>& feature_values)
{
    ...
    {
        ADEBUG << "Model feature size not consistent with model proto "
                  "definition. "
    ...
    for (int i = 0; i < model_ptr_->dim_input(); ++i)
    {
        ...
        layer_input.push_back(apollo::prediction::math_util::Normalize(
                feature_values[i], mean, std));
    }
    ...
    for (int i = 0; i < model_ptr_->num_layer(); ++i)
    {
        ...
        for (int col = 0; col < layer.layer_output_dim(); ++col)
        {
            ...
            {
                double weight =
                        layer.layer_input_weight().rows(row).columns(col);
                neuron_output += (layer_input[row] * weight);
            }
            if (layer.layer_activation_func() == Layer::RELU)
            {
                neuron_output =
                        apollo::prediction::math_util::Relu(neuron_output);
            }
