                                     ...
                                     ObstaclesContainer* obstacles_container)
{
    ...
    torch::Tensor img_tensor =
            torch::from_blob(img_float.data, {1, 224, 224, 3});
    ...
    for (int i = 0; i < 30; ++i)
    {
        ...
        {
            double sigma_xr =
                    std::abs(static_cast<double>(torch_output[0][i][2]));
            double sigma_yr =
                    std::abs(static_cast<double>(torch_output[0][i][3]));
            ...
            double corr = cov_matrix(0, 1) /
                          (sigma_x + FLAGS_double_precision) /
...
void SemanticLSTMEvaluator::LoadModel()
{
    if (FLAGS_use_cuda && torch::cuda::is_available())
    {
        ...
        torch_vehicle_model_ = torch::jit::load(
                FLAGS_torch_vehicle_semantic_lstm_file, device_);
        torch_pedestrian_model_ = torch::jit::load(
                FLAGS_torch_pedestrian_semantic_lstm_file, device_);
    }
    else
    {
        torch_vehicle_model_ = torch::jit::load(
                FLAGS_torch_vehicle_semantic_lstm_cpu_file, device_);
        torch_pedestrian_model_ = torch::jit::load(
    ...
    torch_default_output_tensor_ = torch_vehicle_model_.forward(torch_inputs)
                                           .toTensor()
                                           .to(torch::kCPU);
    torch_default_output_tensor_ = torch_pedestrian_model_.forward(torch_inputs)
                                           .toTensor()
                                           .to(torch::kCPU);
}
