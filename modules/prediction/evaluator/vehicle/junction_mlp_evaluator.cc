                                    ...
                                    ObstaclesContainer* obstacles_container)
{
        ...
        PredictionConstants::kDumpDataForLearning)
    {
        FeatureOutput::InsertDataForLearning(
                *latest_feature_ptr, feature_values, "junction", nullptr);
        ADEBUG << "Save extracted features for learning locally.";
        return true;  // Skip Compute probability for offline mode
    }
    std::vector<torch::jit::IValue> torch_inputs;
    int input_dim =
            static_cast<int>(OBSTACLE_FEATURE_SIZE + EGO_VEHICLE_FEATURE_SIZE +
    ...
    else
    {
        for (int i = 0; i < 12; ++i)
        {
            probability.push_back(
                    feature_values[OBSTACLE_FEATURE_SIZE +
        ...
        std::vector<double>* const feature_values)
{
    ...
    *feature_values = std::vector<double>(4, 0.0);
    ...
    const auto ego_position =
            ego_pose_obstacle_ptr->latest_feature().position();
    const auto ego_velocity =
            ego_pose_obstacle_ptr->latest_feature().velocity();
        ...
        Obstacle* obstacle_ptr, std::vector<double>* feature_values)
{
    ...
    int num_junction_exit =
            feature_ptr->junction_feature().junction_exit_size();
    for (int i = 0; i < num_junction_exit; ++i)
    {
        const JunctionExit& junction_exit =
                feature_ptr->junction_feature().junction_exit(i);
        double x =
                junction_exit.exit_position().x() - feature_ptr->position().x();
        double y =
                junction_exit.exit_position().y() - feature_ptr->position().y();
        double diff_x = std::cos(-heading) * x - std::sin(-heading) * y;
        double diff_y = std::sin(-heading) * x + std::cos(-heading) * y;
        double diff_heading = apollo::common::math::AngleDiff(
                heading, junction_exit.exit_heading());
