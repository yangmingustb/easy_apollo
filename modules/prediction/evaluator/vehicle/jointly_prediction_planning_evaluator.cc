...
bool JointlyPredictionPlanningEvaluator::VectornetProcessObstaclePosition(
        ...
        torch::Tensor* ptr_target_obs_pos_step, torch::Tensor* ptr_vector_mask,
        torch::Tensor* ptr_all_obstacle_pos, torch::Tensor* ptr_all_obs_p_id,
        torch::Tensor* ptr_obs_length)
{
    ...
    for (int j = 0; j < 20; ++j)
    {
        ptr_target_obs_pos->index_put_({19 - j, 0},
                                       target_pos_history[j].first);
        ptr_target_obs_pos->index_put_({19 - j, 1},
                                       target_pos_history[j].second);
        ...
        ptr_target_obs_pos_step->index_put_(
                {19 - j, 1}, target_pos_history[j].second -
                                     target_pos_history[j + 1].second);
    }
    ...
    for (int i = 0; i < obs_num; ++i)
    {
        ...
        for (int j = 0; j < 20; ++j)
        {
            ...
            ptr_all_obstacle_pos->index_put_({i, 19 - j, 0},
                                             all_obs_pos_history[i][j].first);
            ptr_all_obstacle_pos->index_put_({i, 19 - j, 1},
...
bool JointlyPredictionPlanningEvaluator::VectornetProcessMapData(
        FeatureVector* map_feature, PidVector* map_p_id, const int obs_num,
        torch::Tensor* ptr_map_data, torch::Tensor* ptr_all_map_p_id,
        torch::Tensor* ptr_vector_mask)
{
    ...
    {
        ptr_all_map_p_id->index_put_(
                {i}, torch::from_blob(map_p_id->at(i).data(), {2}, opts));

        int one_polyline_vector_size = map_feature->at(i).size();
        for (int j = 0; j < one_polyline_vector_size && j < 50; ++j)
        {
            ptr_map_data->index_put_(
                    {i, j},
                    torch::from_blob(map_feature->at(i)[j].data(), {9}, opts));
        }
    }
        ...
        Obstacle* obstacle_ptr, ObstaclesContainer* obstacles_container)
{
    ...
    if (!VectornetProcessObstaclePosition(
                obstacle_ptr, obstacles_container, &target_obstacle_pos,
                &target_obstacle_pos_step, &vector_mask, &all_obstacle_pos,
                &all_obs_p_id, &obs_length_tmp))
    {
    ...
    ADEBUG << "obstacle vectors used time: " << diff_obs.count() * 1000
           << " ms.";
    ...
    std::chrono::duration<double> diff_query =
            end_time_query - start_time_query;
    ADEBUG << "vectors query used time: " << diff_query.count() * 1000
           << " ms.";

    // process map data & map p id & v_mask for map polyline
    int map_polyline_num = map_feature.size();
    int data_length = ((obs_num + map_polyline_num) < 450)
                              ? (obs_num + map_polyline_num)
                              : 450;

    // Process input tensor
    auto start_time_data_prep = std::chrono::system_clock::now();
    int map_polyline_num_valid = ((obs_num + map_polyline_num) < 450)
                                         ? map_polyline_num
                                         : (450 - obs_num);
    ...
    if (!VectornetProcessMapData(&map_feature, &map_p_id, obs_num, &map_data,
    ...
    if (data_length < 450)
    {
        polyline_mask.index_put_(
                {torch::indexing::Slice(data_length, torch::indexing::None)},
                1);
    }
    ...
    // Add obs attribute
    torch::Tensor obs_attr_agent = torch::tensor({11.0, 4.0})
                                           .unsqueeze(0)
                                           .unsqueeze(0)
                                           .repeat({1, 19, 1});
    torch::Tensor obs_attr_other = torch::tensor({10.0, 4.0})
                                           .unsqueeze(0)
                                           .unsqueeze(0)
                                           .repeat({(obs_num - 1), 19, 1});
    torch::Tensor obs_attr = torch::cat({obs_attr_agent, obs_attr_other}, 0);
    // ADD obs id
    // add 500 to avoid same id as in map_info
    torch::Tensor obs_id = torch::arange(500, obs_num + 500)
                                   .unsqueeze(1)
                                   .repeat({1, 19})
                                   .unsqueeze(2);
    // Process obs data
    ...
    torch::Tensor obs_data_with_id =
            torch::cat({obs_data_with_attr, obs_id}, 2);
    torch::Tensor obs_data_final = torch::cat(
            {torch::zeros({obs_num, (50 - 19), 9}), obs_data_with_id}, 1);
    ...
    {
        double delta_time =
                time_interval - adc_traj.trajectory_point(0).relative_time();
        adc_traj_points.emplace_back(InterpolateUsingLinearApproximation(
                adc_traj.trajectory_point(i), adc_traj.trajectory_point(i + 1),
                delta_time));
    }
    if (!ExtractADCTrajectory(&adc_traj_points, obstacle_ptr,
                              &adc_traj_curr_pos))
    {
    ...
    for (size_t j = 0; j < 30; ++j)
    {
        if (j > traj_points_num - 1)
        {
            ...
            adc_trajectory[0][j][3] = adc_traj_points[traj_points_num - 1].v();
            adc_trajectory[0][j][4] = adc_traj_points[traj_points_num - 1].a();
            adc_trajectory[0][j][5] =
                    adc_traj_points[traj_points_num - 1].path_point().kappa();
        }
        else
        {
            adc_trajectory[0][j][0] = adc_traj_curr_pos[j].first;
            adc_trajectory[0][j][1] = adc_traj_curr_pos[j].second;
            adc_trajectory[0][j][2] = adc_traj_points[j].path_point().theta();
            adc_trajectory[0][j][3] = adc_traj_points[j].v();
            adc_trajectory[0][j][4] = adc_traj_points[j].a();
            adc_trajectory[0][j][5] = adc_traj_points[j].path_point().kappa();
        }
    }
    ...
    torch_output_tensor = torch_vehicle_model_.forward(torch_inputs)
                                  .toTensor()
                                  .to(torch::kCPU);
    ...
    for (int i = 0; i < 30; ++i)
    {
        ...
        if (i < 10)
        {  // use origin heading for the first second
        ...
        else
        {
            point->mutable_path_point()->set_theta(std::atan2(
                    trajectory->trajectory_point(i).path_point().y() -
                            trajectory->trajectory_point(i - 1)
                                    .path_point()
                                    .y(),
        ...
        torch::Tensor* vector_mask)
{
    ...
    all_obs_length->emplace_back(std::make_pair(obs_curr_feature.length(),
    ...
    for (int id : obstacles_container->curr_frame_considered_obstacle_ids())
    {
        ...
        if (obs_his_size > 1)
        {
            vector_mask->index_put_(
                    {cur_idx, torch::indexing::Slice(torch::indexing::None,
                                                     -(obs_his_size - 1))},
                    1);
        }
        else
        ...
        for (size_t i = 0; i < obs_his_size; ++i)
        {
            ...
            pos_history[i] = WorldCoordToObjCoordNorth(
                    std::make_pair(feature.position().x(),
                                   feature.position().y()),
                    obs_curr_pos, obs_curr_heading);
        }
...
bool JointlyPredictionPlanningEvaluator::ExtractADCTrajectory(
        std::vector<TrajectoryPoint>* trajectory_points, Obstacle* obstacle_ptr,
...
void JointlyPredictionPlanningEvaluator::LoadModel()
{
    if (FLAGS_use_cuda && torch::cuda::is_available())
    {
        ...
        torch_vehicle_model_ = torch::jit::load(
                FLAGS_torch_vehicle_jointly_model_file, device_);
    }
    else
    {
        torch_vehicle_model_ = torch::jit::load(
                FLAGS_torch_vehicle_jointly_model_cpu_file, device_);
    }
    ...
    torch_default_output_tensor_ = torch_vehicle_model_.forward(torch_inputs)
                                           .toTensor()
                                           .to(torch::kCPU);
    torch_default_output_tensor_ = torch_vehicle_model_.forward(torch_inputs)
                                           .toTensor()
