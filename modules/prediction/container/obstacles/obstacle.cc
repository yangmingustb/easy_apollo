...
bool Obstacle::Insert(const PerceptionObstacle& perception_obstacle,
                      const double timestamp, const int prediction_obstacle_id)
{
...
bool Obstacle::IsCloseToJunctionExit() const
{
         ...
         latest_feature.junction_feature().junction_exit())
    {
        ...
        if (IsClosed(position_x, position_y, raw_velocity_heading, exit_x,
                     exit_y, exit_heading))
        {
            return true;
        }
                           ...
                           Feature* feature)
{
    ...
    if (perception_obstacle.has_velocity())
    {
        if (perception_obstacle.velocity().has_x())
        {
            ...
            {
                AERROR << "Found unreasonable velocity_x from perception "
                          "obstacle";
            }
        }
        if (perception_obstacle.velocity().has_y())
        {
            ...
            {
                AERROR << "Found unreasonable velocity_y from perception "
                          "obstacle";
            }
        }
        if (perception_obstacle.velocity().has_z())
        {
            ...
            {
                AERROR << "Found unreasonable velocity_z from perception "
                          "obstacle";
            }
        }
    }
        ...
        history_size() > 0)
    {
        ...
        double obstacle_size = std::fmax(perception_obstacle.length(),
                                         perception_obstacle.width());
        double size_diff = std::abs(obstacle_size - prev_obstacle_size);
        double shift_thred = std::fmax(
        ...
        if (std::fabs(diff_x) > shift_thred &&
            std::fabs(diff_y) > shift_thred && size_diff < size_diff_thred)
        {
            double shift_heading = std::atan2(diff_y, diff_x);
            double angle_diff = common::math::NormalizeAngle(shift_heading -
                                                             velocity_heading);
...
void Obstacle::SetAcceleration(Feature* feature)
{
    ...
    if (feature_history_.size() > 0)
    {
        ...
        if (curr_ts > prev_ts)
        {
            ...
            acc_x = (curr_velocity.x() - prev_velocity.x()) /
                    (curr_ts - prev_ts);
            acc_y = (curr_velocity.y() - prev_velocity.y()) /
                    (curr_ts - prev_ts);
            acc_z = (curr_velocity.z() - prev_velocity.z()) /
                    (curr_ts - prev_ts);
...
void Obstacle::SetCurrentLanes(Feature* feature)
{
    ...
    for (std::shared_ptr<const LaneInfo> current_lane : current_lanes)
    {
        ...
        double angle_diff =
                common::math::AngleDiff(heading, nearest_point_heading);
        double left = 0.0;
        double right = 0.0;
...
void Obstacle::SetNearbyLanes(Feature* feature)
{
    ...
    for (std::shared_ptr<const LaneInfo> nearby_lane : nearby_lanes)
    {
        ...
        {
            angle_diff =
                    common::math::AngleDiff(nearest_point.heading(), heading);
        }
...
void Obstacle::BuildLaneGraph()
{
    ...
    double road_graph_search_distance =
            std::fmax(estimated_move_distance,
    ...
    for (auto& lane : feature->lane().current_lane_feature())
    {
        ...
        {
            if (is_in_junction &&
                !HasJunctionExitLane(lane_seq, exit_lane_id_set))
            {
                continue;
            }
...
void Obstacle::BuildLaneGraphFromLeftToRight()
{
    ...
    const std::vector<std::string> lane_ids_ordered(
            lane_ids_ordered_list.begin(), lane_ids_ordered_list.end());
    ...
    for (const std::string& lane_id : lane_ids_ordered)
    {
        ...
        bool vehicle_is_on_lane =
                (lane_id == center_lane_info->lane().id().id());
        std::shared_ptr<const LaneInfo> curr_lane_info =
                PredictionMap::LaneById(lane_id);
        LaneGraph local_lane_graph =
                clusters_ptr_->GetLaneGraphWithoutMemorizing(
                             ...
                             LaneGraph* const lane_graph)
{
    ...
    for (int i = 0; i < lane_graph->lane_sequence_size(); ++i)
    {
        ...
        if (is_bidirection)
        {
            int lane_index = 0;
            double lane_seg_s =
                    lane_sequence->lane_segment(lane_index).start_s();
            while (lane_index < lane_sequence->lane_segment_size())
            {
                    ...
                    lane_seg_s > lane_segment->adc_s())
                {
                    lane_segment->set_adc_lane_point_idx(
                            lane_segment->lane_point_size());
                    break;
                }

                if (lane_seg_s > lane_segment->end_s())
                {
                    // If already exceeds the current lane_segment, then go to
                ...
                else
                {
                    ...
                    Eigen::Vector2d lane_point_pos =
                            PredictionMap::PositionOnLane(lane_info,
                                                          lane_seg_s);
                    double lane_point_heading =
                            PredictionMap::HeadingOnLane(lane_info, lane_seg_s);
                    double lane_point_width = PredictionMap::LaneTotalWidth(
                            lane_info, lane_seg_s);
                    double lane_point_angle_diff = common::math::AngleDiff(
                            lane_point_heading, heading);
...
void Obstacle::SetLaneSequencePath(LaneGraph* const lane_graph)
{
    ...
    for (int i = 0; i < lane_graph->lane_sequence_size(); ++i)
    {
        ...
        for (int j = 0; j + 1 < num_path_point; ++j)
        {
            ...
            double kappa =
                    std::abs(delta_theta / (delta_s + FLAGS_double_precision));
...
void Obstacle::SetNearbyObstacles()
{
    ...
    for (int i = 0; i < lane_graph->lane_sequence_size(); ++i)
    {
        ...
        if (clusters_ptr_->ForwardNearbyObstacle(*lane_sequence, id_,
                                                 obstacle_s, obstacle_l,
                                                 &forward_obstacle))
        {
            lane_sequence->add_nearby_obstacle()->CopyFrom(forward_obstacle);
        }
        NearbyObstacle backward_obstacle;
        if (clusters_ptr_->BackwardNearbyObstacle(*lane_sequence, id_,
                                                  obstacle_s, obstacle_l,
                                                  &backward_obstacle))
        {
...
void Obstacle::DiscardOutdatedHistory()
{
    ...
    {
        ADEBUG << "Obstacle [" << id_ << "] discards "
               << num_of_discarded_frames << " historical features";
    }
}
...
void Obstacle::SetInteractiveTag()
{
    CHECK_GT(feature_history_.size(), 0U);
    Feature* feature = mutable_latest_feature();
    feature->mutable_interactive_tag()->set_interactive_tag(
