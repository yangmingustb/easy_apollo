...
ObstaclesContainer::ObstaclesContainer() :
    ptr_obstacles_(FLAGS_max_num_obstacles), clusters_(new ObstacleClusters())
{
}
...
void ObstaclesContainer::Insert(const ::google::protobuf::Message& message)
{
    ...
    switch (FLAGS_prediction_offline_mode)
    {
        case 1:
        {
            if (std::fabs(timestamp - timestamp_) >
...
const std::vector<int>& ObstaclesContainer::curr_frame_unmovable_obstacle_ids()
{
    return curr_frame_unmovable_obstacle_ids_;
}

const std::vector<int>& ObstaclesContainer::curr_frame_considered_obstacle_ids()
{
...
void ObstaclesContainer::BuildJunctionFeature()
{
    ...
    for (const int id : curr_frame_considered_obstacle_ids_)
    {
        ...
        {
            ADEBUG << "Build junction feature for obstacle ["
                   << obstacle_ptr->id() << "] in junction [" << junction_id
                   << "]";
