...
bool MapService::CheckRoutingPointLaneId(const double x, const double y,
                                         std::vector<std::string> idsArr) const
{
    if (idsArr.empty())
    {
        return false;
...
bool MapService::CheckRoutingPointLaneType(LaneInfoConstPtr lane) const
{
    ...
    {
        AERROR << "Failed to construct LaneWayPoint for RoutingRequest: "
                  "Expected lane "
                                        ...
                                        std::vector<Path> *paths) const
{
    ...
    {
        AERROR << " routing segment is null";
        return false;
    }
        ...
        const routing::Passage &passage_region, std::vector<Path> *paths) const
{
    ...
    for (const auto &segment : passage_region.segment())
    {
        ...
        auto points = MapPathPoint::GetPointsFromLane(
                lane_ptr, segment.start_s(), segment.end_s());
    ...
    {
        AERROR << "path_points size < 2: " << path_points.size();
        return false;
    }
