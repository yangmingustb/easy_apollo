        ...
        std::vector<PassageInfo>* const passages)
{
    ...
    {
        auto edge = nodes.at(i - 1).GetTopoNode()->GetOutEdgeTo(
                nodes.at(i).GetTopoNode());
        if (edge == nullptr)
        {
            AERROR << "Get null pointer to edge from "
                   << nodes.at(i - 1).LaneId() << " to "
                   << nodes.at(i).LaneId();
            return false;
        }
...
bool ResultGenerator::IsReachableToWithChangeLane(const TopoNode* to_node,
                                     ...
                                     PassageInfo* const curr_passage)
{
    ...
    while (allowed_to_explore)
    {
             ...
             curr_passage->nodes.front().GetTopoNode()->InFromPreEdge())
        {
                                            ...
                                            &reachable_node))
            {
                ...
                if (pred_range != nullptr && !pred_range->empty())
                {
                    ...
                    {
                        pred_set.emplace_back(pred_node, black_s_end,
                                              pred_node->Length());
                    }
                }
                else
        ...
        else
        {
            ...
            curr_passage->nodes.insert(curr_passage->nodes.begin(),
                                       node_to_insert);
    ...
    if (!IsCloseEnough(back_node.EndS(), back_node.FullLength()))
    {
        if (!range_manager.Find(back_node.GetTopoNode()))
        {
            ...
            else
            {
                ...
                {
                    adjusted_end_s =
                            std::min(adjusted_end_s, back_node.FullLength());
             ...
             curr_passage->nodes.back().GetTopoNode()->OutToSucEdge())
        {
                                              ...
                                              &reachable_node))
            {
                ...
                else
                {
                    ...
                    {
                        succ_set.emplace_back(succ_node, 0.0,
                                              succ_node->Length());
                    }
                    else
                    {
                                     ...
                                     std::vector<PassageInfo>* const passages)
{
    ...
    for (int i = 0; i < passage_num; ++i)
    {
        if (i < passage_num - 1)
        {
            ExtendForward(range_manager, passages->at(i + 1),
                          &(passages->at(i)));
        }
        if (i > 0)
        {
            ExtendBackward(range_manager, passages->at(i - 1),
                           &(passages->at(i)));
        }
    }
        ...
        RoutingResponse* result)
{
    ...
    road->set_id(
            passages[start_index.first].nodes[start_index.second].RoadId());
        ...
        const std::vector<PassageInfo>& passages, RoutingResponse* result)
{
        ...
        for (std::size_t j = 0; j < curr_nodes.size(); ++j)
        {
            if ((i + 1 < passages.size() &&
                 IsReachableToWithChangeLane(curr_nodes[j].GetTopoNode(),
                                             passages[i + 1],
                                             &fake_node_range)) ||
                (i > 0 && IsReachableFromWithChangeLane(
                                  curr_nodes[j].GetTopoNode(), passages[i - 1],
                                  &fake_node_range)))
            {
                if (!in_change_lane)
                {
            ...
            else
            {
                if (in_change_lane)
                {
                    ADEBUG << "start_index(" << start_index.first << ", "
                           << start_index.second << ") end_index(" << i << ", "
                           << j - 1 << ")";
                    AddRoadSegment(passages, start_index, {i, j - 1}, result);
                }
                ADEBUG << "start_index(" << i << ", " << j << ") end_index("
                       << i << ", " << j << ")";
    ...
    {
        ADEBUG << "start_index(" << start_index.first << ", "
               << start_index.second << ") end_index(" << passages.size() - 1
