...
{
    AINFO << "Route lane id\tstart s\tend s";
    for (const auto& node : nodes)
    {
        AINFO << node.GetTopoNode()->LaneId() << "\t" << node.StartS() << "\t"
              << node.EndS();
        // node.GetTopoNode()->CentralCurve().segment(0).line_segment().point(0).x()
    }
}
                            ...
                            RoutingResponse* const response)
{
    ...
    if (!IsReady())
    {
        SetErrorCode(ErrorCode::ROUTING_ERROR_NOT_READY,
                     "Navigator is not ready!", response->mutable_status());
        return false;
    }
    ...
    {
        SetErrorCode(ErrorCode::ROUTING_ERROR_RESPONSE,
                     "Failed to result nodes!", response->mutable_status());
        return false;
    }
